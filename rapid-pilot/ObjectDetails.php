...
final class RapidPilotObjectDetails
{
    public static function enhance(string $html, string $path): string
    {
        ...
        $html=str_replace('>Изменяющее распоряжение подготовлено</span>','>Требуется изменение</span>',$html);
        ...
        $action['html']=preg_replace('#<label for="actualStartDate">Фактическая дата начала работ</label><input id="actualStartDate"([^>]*)><small>.*?</small>#s','<label class="fm2-open-date" for="actualStartDate"><span>Фактическая дата начала работ</span><input class="shlz-input" id="actualStartDate"$1></label>',$action['html'],1)??$action['html'];
    ...
    private static function render(array $data,string $action,string $dashboard,array $identity): string
    {
        ...
        $engineer=$teamFacts['Инженер стройконтроля']??trim((string)($fields['responsstroicontrol']['display']??''));$engineer=preg_replace('/\s+·\s+.*$/u','',$engineer)??$engineer;$roster='';foreach($data['installers']??[]as$installer){$employed=($installer['employment_status']??$installer['employment_status_snapshot']??'')==='employed';$tab=str_pad((string)$installer['installer_tab_id'],6,'0',STR_PAD_LEFT);$roster.='<div class="fm2-object-installer"><strong>'.self::e(trim((string)$installer['fio_snapshot'])).'</strong><span>Табельный № '.self::e($tab).'</span><span class="shlz-status '.($employed?'shlz-status--bright-green':'shlz-status--neutral').'">'.($employed?'Работает':'Уволен').'</span></div>';}$nativeInstallers=self::panelValues($existing['Команда объекта']??'','Монтажник');if($nativeInstallers!==[]){$roster='';foreach($nativeInstallers as$installer)$roster.='<div class="fm2-object-installer"><strong>'.self::e($installer).'</strong></div>';}$participants=self::teamRow($engineer,trim((string)($fields['contact_phone_itn']['display']??'')),$roster)
        ...
        $orderHistory=self::assignmentDocumentHistory($data['assignmentDocuments']??[],(int)($data['objectId']??0));$documents.=($existing['Распоряжение']??$existing['Распоряжение и 1С ДО']??'').$orderHistory;$control.=($existing['Проблемы']??'').($existing['Последние события']??'');
