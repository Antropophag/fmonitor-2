...
try{
 ...
 $projection=$sync->projection(1103);$ids=array_column($projection['crew'],'tabId');if($ids!==['202'])throw new RuntimeException('Expected only latest registered order crew, got '.json_encode($ids));$historicalInstallers=$projection['items']['28']['installers']??[];$historical=array_column($historicalInstallers,'tabId');if($historical!==['101'])throw new RuntimeException('Expected historical item installer snapshot 101, got '.json_encode($historical));if(($historicalInstallers[0]['currentlyAssigned']??null)!==false)throw new RuntimeException('Detached historical installer must be explicitly projected as historical, without changing the accepted work fact.');echo "Checklist current crew contract OK.\n";
