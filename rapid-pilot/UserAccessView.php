...
final class RapidPilotUserAccessView
{
    ...
    {
        $actorId=(int)($_SERVER['FMONITOR_AUTH_USER_ID']??0);$csrf=(string)($_POST['csrfToken']??'');$expected=(string)($_SERVER['FMONITOR_AUTH_CSRF']??'');$action=(string)($_POST['action']??'');if($actorId<1||$userId<1||$userId===$actorId||$csrf===''||$expected===''||!hash_equals($expected,$csrf)||!in_array($action,['block','unblock'],true))self::statusError(403);require_once dirname(__DIR__).'/app/PilotHttp/AccessPolicy.php';$db=self::db();$p=self::prefix();if(!\FMonitor2\PilotHttp\AccessPolicy::grants(\FMonitor2\PilotHttp\AccessPolicy::forUser($db,$p,$actorId),'access.administer'))self::statusError(403,$db);$db->begin_transaction();try{$q=$db->prepare("SELECT status,activation_state FROM `{$p}fm2_pilot_users` WHERE user_id=? FOR UPDATE");$q->bind_param('i',$userId);$q->execute();$target=$q->get_result()->fetch_assoc();if(!is_array($target)||($action==='block'&&((int)$target['status']!==1||$target['activation_state']!=='active'))||($action==='unblock'&&$target['activation_state']!=='blocked'))self::statusError(400,$db);if($action==='block'){$s=$db->prepare("SELECT COUNT(*) n FROM `{$p}fm2_pilot_user_roles` ur JOIN `{$p}fm2_pilot_roles` r ON r.role_id=ur.role_id WHERE ur.user_id=? AND r.code='superadministrator'");$s->bind_param('i',$userId);$s->execute();if((int)$s->get_result()->fetch_assoc()['n']>0){$count=(int)$db->query("SELECT COUNT(DISTINCT u.user_id) n FROM `{$p}fm2_pilot_users` u JOIN `{$p}fm2_pilot_user_roles` ur ON ur.user_id=u.user_id JOIN `{$p}fm2_pilot_roles` r ON r.role_id=ur.role_id WHERE u.status=1 AND u.activation_state='active' AND r.code='superadministrator'")->fetch_assoc()['n'];if($count<=1)self::statusError(400,$db);}}$next=$action==='block'?0:1;$state=$action==='block'?'blocked':'active';$now=(new DateTimeImmutable('now',new DateTimeZone('Europe/Moscow')))->format(DATE_ATOM);$u=$db->prepare("UPDATE `{$p}fm2_pilot_users` SET status=?,activation_state=?,session_version=session_version+1,source_updated_at=? WHERE user_id=?");$u->bind_param('issi',$next,$state,$now,$userId);$u->execute();$db->query("CREATE TABLE IF NOT EXISTS `{$p}fm2_pilot_user_status_events`(id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,user_id BIGINT UNSIGNED NOT NULL,action VARCHAR(40) NOT NULL,occurred_at VARCHAR(40) NOT NULL,actor_user_id BIGINT UNSIGNED NOT NULL,KEY(user_id,id)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");$e=$db->prepare("INSERT INTO `{$p}fm2_pilot_user_status_events`(user_id,action,occurred_at,actor_user_id) VALUES(?,?,?,?)");$event=$action==='block'?'user_blocked':'user_unblocked';$e->bind_param('issi',$userId,$event,$now,$actorId);$e->execute();$db->commit();}catch(Throwable$error){try{$db->rollback();}catch(Throwable){}throw$error;}$db->close();header('Location: /pilot/admin/users',true,303);header('Cache-Control: no-store');exit;
    }
