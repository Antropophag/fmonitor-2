...
$source->query('SET TRANSACTION ISOLATION LEVEL REPEATABLE READ');$source->query('START TRANSACTION WITH CONSISTENT SNAPSHOT, READ ONLY');try{$snapshot=(new LegacyChecklistTemplateMySqlSource($source))->extract($capturedAt);$source->commit();}catch(Throwable$error){$source->rollback();throw$error;}finally{$source->close();}$result=['mode'=>isset($options['apply'])?'apply':'dry-run','capturedAt'=>$capturedAt,'contentSha256'=>$snapshot['contentSha256'],'counts'=>$snapshot['counts'],'validity'=>'active_baseline_and_future_native_only'];
