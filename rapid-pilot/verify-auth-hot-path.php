...
$queueReader = file_get_contents(dirname(__DIR__) . '/app/PilotHttp/MariaDbObjectQueue.php');
if (!is_string($auth) || !is_string($bootstrap) || !is_string($identityBootstrap) || !is_string($identityBootstrapApplication) || !is_string($launcher) || !is_string($queue) || !is_string($queueReader)) throw new RuntimeException('Auth sources unavailable');
...
$fail(str_contains($queue, 'new \\FMonitor2\\PilotHttp\\MariaDbObjectQueue('), 'rapid operational queue does not use the MariaDB read owner');
$fail(!str_contains($queue, 'fm2_installation_cases') && !str_contains($queue, 'fm2_checklist_operations') && !str_contains($queue, 'fm2_pilot_completion_facts'), 'rapid operational queue still owns its process projection SQL');
$fail(str_contains($queueReader, 'public function read('), 'MariaDB object queue has no public read seam');
