...
После клонирования репозитория скопируйте `.env.example` в `.env` и заполните
пароль администратора, вебхук Bitrix и ID отделов. Затем в корне проекта:

```bash
make up
```

Штатный запуск поднимает пилот вместе с часовой синхронизацией монтажников.
Настройки Bitrix задаются явно; соседний legacy-репозиторий и PHP на хосте
не нужны. Подготовка выполняется в контейнере, приватный файл `.local/` имеет
права `0600`. См. [инструкцию Bitrix](../docs/bitrix-startup.md).
...
После `make up` в уже заполненном `.env` укажите реквизиты read-only
пользователя legacy MariaDB и выполните одну команду:

```bash
# откройте существующий .env в редакторе и замените FMONITOR_SOURCE_USER / FMONITOR_SOURCE_PASSWORD
...
Rapid pilot использует только локальные учётные записи и системные роли FMonitor. Саморегистрации нет: обычная учётная запись заранее создаётся администратором и активируется одноразовым приглашением. Учётки из `FMONITOR_BOOTSTRAP_SUPERADMIN_EMAILS` создаются сразу активными с паролем из `FMONITOR_BOOTSTRAP_SUPERADMIN_PASSWORD` и ролями `user` и `superadministrator`; удаление email из env не отзывает роль. Пароли хранятся только как Argon2id-хэши.
