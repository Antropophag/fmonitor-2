...
Каталог пользователей и роли переносятся из legacy FMonitor командой `import-production-users.php`. Вход разрешён активным пользователям с корпоративным email `@shlz.ru`. При первом входе пользователь задаёт локальный пароль; исходные legacy-пароли не импортируются. Пароль хранится только как Argon2id-хэш (bcrypt используется как fallback, если Argon2id недоступен в сборке PHP).
