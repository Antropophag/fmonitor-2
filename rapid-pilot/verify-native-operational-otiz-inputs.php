...
$db=new mysqli(getenv('FMONITOR_VERIFY_DB_HOST')?:'mariadb','fmonitor2_demo','fmonitor2_demo_local','fmonitor2_demo',(int)(getenv('FMONITOR_VERIFY_DB_PORT')?:3306));$db->set_charset('utf8mb4');$p='native_otiz_'.bin2hex(random_bytes(4)).'_';
...
try{
 ...
 $db->query("CREATE TABLE `{$p}fm2_checklist_operations`(id BIGINT AUTO_INCREMENT PRIMARY KEY,installation_case_id BIGINT,client_operation_id CHAR(36),operation_type VARCHAR(40),item_id INT,device_time VARCHAR(40),server_received_at VARCHAR(40),template_snapshot_id BIGINT,template_content_sha256 CHAR(64))");
 $db->query("CREATE TABLE `{$p}fm2_checklist_operation_installers`(client_operation_id CHAR(36),installer_tab_id BIGINT)");
 ...
 $db->query("INSERT INTO `{$p}fm2_checklist_operations`(installation_case_id,client_operation_id,operation_type,item_id,device_time,server_received_at,template_snapshot_id,template_content_sha256)VALUES(7,'11111111-1111-4111-8111-111111111111','item_completed',1,'2026-08-10T10:00:00+03:00','2026-08-10T10:01:00+03:00',3,'$hash'),(7,'22222222-2222-4222-8222-222222222222','item_completed',2,'2026-09-10T10:00:00+03:00','2026-09-10T10:01:00+03:00',3,'$hash')");$db->query("INSERT INTO `{$p}fm2_checklist_operation_installers` VALUES('11111111-1111-4111-8111-111111111111',42),('22222222-2222-4222-8222-222222222222',42)");
