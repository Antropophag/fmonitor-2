...
final class MigratedEvidenceDecisionLedger
{
    ...
    private function projection(int $snapshotId):array{foreach(MigratedEvidenceReconciliation::load($this->db,$this->prefix)as$projection)if((int)$projection['snapshotId']===$snapshotId)return $projection;throw new DomainException('Migrated evidence projection is stale or unavailable.');}
}
