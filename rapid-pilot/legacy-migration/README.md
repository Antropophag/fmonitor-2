...
For the Docker pilot, use the execution-context runner. It reads the git-ignored
source environment from `.env`, joins the running pilot container to the local
read-only tunnel network, and resolves the active pilot manifest inside its
volume:

```bash
rapid-pilot/run-legacy-history-import.sh --object-id=123 --cutoff='2026-08-30 23:59:59'
rapid-pilot/run-legacy-history-import.sh --object-id=123 --cutoff='2026-08-30 23:59:59' --apply
```

The network, container, and database defaults can be overridden with the
`FMONITOR_SOURCE_TUNNEL_*` and `FMONITOR_PILOT_*` environment variables.
