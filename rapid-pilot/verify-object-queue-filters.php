...
foreach(['name="q"','name="status"','type="search"','Рег. номер, адрес, подъезд или № объекта','Все статусы','>Статус</span>']as$needle)$check(str_contains($source,$needle),'missing application filter: '.$needle);
...
foreach(['needs_assignment_order','ready_to_open','installation','document_closeout','completed','needs_assignment_change']as$status)$check(str_contains($source,$status),'status missing: '.$status);
$check(!str_contains($source,"'assignment_order_prepared'=>'Распоряжение подготовлено'"),'prepared order remains a user-visible status');
$check(!str_contains($source,"'working'=>'В работе'"),'obsolete combined working status remains in select');
$check(str_contains($source,"'installation'=>'Монтажные работы'")&&str_contains($source,"'document_closeout'=>'Документарное закрытие'")&&str_contains($source,"'completed'=>'Работы завершены'"),'completion statuses are not exposed in select');
