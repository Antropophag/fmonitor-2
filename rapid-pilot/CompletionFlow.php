...
final class RapidPilotCompletionFlow
{
    ...
    public static function enhanceCard(string $html,int $objectId):string
    {
        ...
        $html=preg_replace_callback('#<section class="fm2-next-action"[^>]*>.*?</section>#s',static function(array$match)use($objectId,$progress,$facts):string{
            ...
            return self::panel($objectId,$progress,$facts,(string)($_SERVER['FMONITOR_AUTH_CSRF']??''),$actions[0]??'');
        },$html,1)??$html;
        return$html;
    }
    ...
    private static function panel(int$id,int$progress,array$facts,string$csrf,string$actions):string
    {
        ...
        return'<section class="fm2-next-action fm2-completion" id="completion" aria-labelledby="completion-title"><header><div><h2 id="completion-title">Текущее действие</h2><p>'.($complete?'Работы по объекту завершены: подтверждены акт ПТО и декларация.':($progress>=85?'Монтаж завершён. Добавьте закрывающие документы.':'Продолжайте чеклист монтажа. Последние 15% закроются актом ПТО и декларацией.')).'</p></div><span class="shlz-status '.($complete?'shlz-status--bright-green':($progress>=85?'shlz-status--orange':'shlz-status--blue')).'">'.$status.' · '.$shown.'%</span></header><div class="fm2-completion-track" role="progressbar" aria-label="Готовность работ" aria-valuemin="0" aria-valuemax="100" aria-valuenow="'.$shown.'"><progress class="fm2-completion-track__segment fm2-completion-track__segment--installation" max="85" value="'.min(85,$progress).'" aria-hidden="true"></progress><progress class="fm2-completion-track__segment fm2-completion-track__segment--documents" max="15" value="'.($complete?'15':'0').'" aria-hidden="true"></progress></div><ol class="fm2-completion-steps"><li class="'.($progress>=85?'is-done':'is-current').'"><span>1</span><div><strong>Чеклист монтажных работ</strong><small>'.min(85,$progress).' из 85%</small></div></li><li class="'.($pto?'is-done':($progress>=85?'is-current':'')).'"><span>2</span><div>'.$ptoState.'</div></li><li class="'.($declaration?'is-done':($pto?'is-current':'')).'"><span>3</span><div>'.$declarationState.'</div></li></ol>'.$form.$actions.'</section>';
    }
