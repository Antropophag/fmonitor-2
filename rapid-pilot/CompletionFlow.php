...
final class RapidPilotCompletionFlow
{
    ...
    {
        try{$db=self::db();$prefix=self::prefix();self::ensureSchema($db,$prefix);$case=self::case($db,$prefix,$objectId,false);if($case===null)return$html;$progress=self::installationProgress($db,$prefix,(int)$case['id']);$facts=self::facts($db,$prefix,(int)$case['id']);}
        catch(Throwable){return$html;}
        $complete=isset($facts['pto_act'],$facts['declaration']);$status=self::status($progress,$complete);$panel=self::panel($objectId,$progress,$facts,(string)($_SERVER['FMONITOR_AUTH_CSRF']??''));
        ...
        $needle='<div class="fm2-object-workspace">';return str_replace($needle,$needle.$panel,$html);
    }
    ...
    private static function panel(int$id,int$progress,array$facts,string$csrf):string
    {
        ...
        $form=$progress<85||$complete?'':($pto===null?'<form method="post" action="/pilot/objects/'.$id.'/completion" class="fm2-completion-form"><input type="hidden" name="csrfToken" value="'.$e($csrf).'"><input type="hidden" name="action" value="record_pto"><label class="shlz-field"><span class="shlz-field__label">Дата акта ПТО</span><span class="shlz-field__control"><input class="shlz-input" type="date" name="ptoActDate" max="'.substr(self::now(),0,10).'" required></span></label><button class="shlz-button shlz-button--primary">Зафиксировать акт ПТО</button></form>':'<form method="post" action="/pilot/objects/'.$id.'/completion" class="fm2-completion-form"><input type="hidden" name="csrfToken" value="'.$e($csrf).'"><input type="hidden" name="action" value="record_declaration"><label class="shlz-field"><span class="shlz-field__label">Дата декларации</span><span class="shlz-field__control"><input class="shlz-input" type="date" name="declarationDate" max="'.substr(self::now(),0,10).'" required></span></label><label class="shlz-field fm2-completion-details"><span class="shlz-field__label">Реквизиты декларации</span><span class="shlz-field__control"><input class="shlz-input" name="declarationDetails" maxlength="500" placeholder="Номер, кем и когда выдана" required></span></label><button class="shlz-button shlz-button--primary">Завершить работы</button></form>');
        $mountFill=min(100,(int)round($progress/85*100));$status=self::status($progress,$complete);
        return'<section class="fm2-completion" id="completion" aria-labelledby="completion-title"><header><div><h2 id="completion-title">Завершение работ</h2><p>'.($complete?'Работы по объекту завершены: подтверждены акт ПТО и декларация.':'Монтаж даёт 85%. Последние 15% закрываются документами.').'</p></div><span class="shlz-status '.($complete?'shlz-status--bright-green':($progress>=85?'shlz-status--orange':'shlz-status--blue')).'">'.$status.' · '.$shown.'%</span></header><div class="fm2-completion-track" role="progressbar" aria-label="Прогресс завершения работ" aria-valuemin="0" aria-valuemax="100" aria-valuenow="'.$shown.'"><span class="fm2-completion-track__segment fm2-completion-track__segment--installation"><i style="width:'.$mountFill.'%"></i></span><span class="fm2-completion-track__segment fm2-completion-track__segment--documents"><i style="width:'.($complete?'100':'0').'%"></i></span></div><ol class="fm2-completion-steps"><li class="'.($progress>=85?'is-done':'is-current').'"><span>1</span><div><strong>Монтажные работы</strong><small>'.min(85,$progress).' из 85%</small></div></li><li class="'.($pto?'is-done':($progress>=85?'is-current':'')).'"><span>2</span><div>'.$ptoState.'</div></li><li class="'.($declaration?'is-done':($pto?'is-current':'')).'"><span>3</span><div>'.$declarationState.'</div></li></ol>'.$form.'</section>';
    }
