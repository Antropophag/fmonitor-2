...
final class RapidPilotOtiz
{
    ...
    private function queue(): never
    {
        ...
        $cards .= '<nav class="fm2-otiz-subnav" aria-label="Раздел ОТиЗ"><a class="shlz-link" aria-current="page" href="/pilot/otiz">Очередь расчёта</a><a class="shlz-link" href="/pilot/otiz/objects">Объекты</a><a class="shlz-link" href="/pilot/otiz/history">История срезов</a></nav>';
    ...
    {
        return '<section class="fm2-otiz-scenarios"><header><h2>Native operational mode</h2><p>В очередь входят только открытые native cases с подтверждённым classification provenance.</p></header><ol><li><strong>Прогресс на дату</strong><span>Только принятые сервером checklist events и неизменяемая версия весов</span></li><li><strong>Финансовые operands</strong><span>Только датированные подтверждённые факты; отсутствие создаёт блокер</span></li><li><strong>Накопление</strong><span>Предыдущий принятый срез и append-only закрытия участвуют в следующем расчёте</span></li></ol></section>';
    }
