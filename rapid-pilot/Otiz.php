...
final class RapidPilotOtiz
{
    ...
    public function handle(string $path): never
    {
        ...
        if ($path === '/pilot/otiz/objects') $this->objects();
    ...
    private function command(string $path): never
    {
        ...
        if (preg_match('#^/pilot/otiz/snapshots/(\d+)/payments/complete$#D', $path, $m) === 1) {
            ...
            $date=substr($this->now(),0,10);$discipline=0;$deadline=0;$basis='Выплаты по срезу выполнены по подтверждению ОТиЗ';$artifact='';$now=$this->now();$count=0;$total=0;
    ...
    private function calculate(string $date): int
    {
        ...
        foreach ($objects as $o) {
            ...
            foreach ($members as $member) {
                ...
                $zero = 0;$contribution=(int)($member['contribution']??$member['weight']); $a->bind_param('iisssiiiiiiss', $snapshotId, $o['id'], $member['tab'], $member['name'], $member['position'], $contribution, $member['weight'], $zero, $member['weight'], $share, $amount, $employment, $basis); $a->execute();
            }
    ...
    private function queue(): never
    {
        ...
        $cards .= '<nav class="fm2-otiz-subnav" aria-label="Раздел ОТиЗ"><a class="shlz-link" aria-current="page" href="/pilot/otiz">Подготовка выплат</a><a class="shlz-link" href="/pilot/otiz/objects">Объекты</a><a class="shlz-link" href="/pilot/otiz/history">Архив расчётов</a></nav>';
        if (is_array($latest)) $cards .= '<section class="fm2-otiz-current"><div><span class="shlz-status ' . ($latest['status'] === 'accepted' ? 'shlz-status--green' : 'shlz-status--orange') . '">' . ($latest['status'] === 'accepted' ? 'Готов к выплате' : 'Требует проверки') . '</span><h2>Расчёт на ' . $this->date($latest['report_date']) . '</h2><p>' . ($latest['status'] === 'accepted' ? 'Начисления проверены. Можно выгрузить реестр и зафиксировать выплату.' : 'Проверьте объекты и устраните замечания перед оформлением реестра.') . '</p></div><div><strong>' . $this->rub((int) $latest['total_pool_cents']) . '</strong><span>к распределению</span><a class="shlz-button shlz-button--primary" href="/pilot/otiz/snapshots/' . (int) $latest['id'] . '">' . ($latest['status'] === 'accepted' ? 'Открыть реестр' : 'Продолжить проверку') . '</a></div></section>';
    ...
    private function snapshot(int $id): never
    {
        ...
        $payable=0;$deadlinePenalty=0;foreach($objects as$object){$closed=(int)$this->db->query("SELECT COALESCE(SUM(paid_cents+discipline_cents+deadline_cents),0) n FROM `{$this->prefix}fm2_pilot_otiz_payment_closures` WHERE snapshot_id={$id} AND object_id=".(int)$object['object_id'])->fetch_assoc()['n'];if($object['calculation_state']!=='blocked')$payable+=max(0,(int)$object['pool_cents']-$closed);$deadlinePenalty+=$this->deadlinePenalty($object);}
        $flash = isset($_GET['created']) ? 'Черновик рассчитан по датированным фактам.' : (isset($_GET['accepted']) ? 'Срез принят и защищён от изменения.' : (($_GET['paid']??'')==='1' ? 'Выплата отмечена выполненной. Сумма записана в историю объекта.' : (($_GET['paid']??'')==='duplicate' ? 'Выплата по объекту уже была отмечена.' : (isset($_GET['closed']) ? 'Дисциплинарное удержание добавлено в историю объекта.' : (isset($_GET['reversed']) ? 'Создана сторнирующая запись.' : '')))));
        $body = '<nav class="fm2-breadcrumb" aria-label="Хлебные крошки"><ol><li><a class="fm2-breadcrumb-link" href="/pilot/otiz">Подготовка выплат</a></li><li><span aria-current="page">Расчёт на ' . $this->date($s['report_date']) . '</span></li></ol></nav>';
        ...
        $acceptedActions='<a class="shlz-link" href="/pilot/otiz/snapshots/'.$id.'/export.xlsx">Скачать рабочий комплект XLSX</a>'.($payable>0?'<form method="post" action="/pilot/otiz/snapshots/'.$id.'/payments/complete"><input type="hidden" name="csrfToken" value="'.$this->e($this->csrf).'"><button class="shlz-button shlz-button--primary" type="submit">Отметить выплаты по срезу выполненными</button><small>Будет выплачено '.$this->rub($payable).'</small></form>':'<span class="shlz-status shlz-status--green">Выплаты отмечены</span>');
        $body .= '<header class="fm2-otiz-snapshot-head"><div><div class="fm2-otiz-title-line"><h1>Выплаты на ' . $this->date($s['report_date']) . '</h1><span class="shlz-status ' . ($s['status'] === 'accepted' ? 'shlz-status--green' : 'shlz-status--orange') . '">' . ($s['status'] === 'accepted' ? 'Готовы к выплате' : 'На проверке') . '</span></div><p>Расчёт подготовлен ' . $this->dateTime($s['calculated_at']) . '. Проверьте начисления и замечания по каждому объекту.</p></div><div class="fm2-otiz-actions">' . ($s['status'] === 'draft' ? '<form method="post" action="/pilot/otiz/snapshots/' . $id . '/accept"><input type="hidden" name="csrfToken" value="' . $this->e($this->csrf) . '"><button class="shlz-button shlz-button--primary" type="submit"' . ($blockers > 0 ? ' disabled' : '') . '>Подтвердить расчёт</button></form>' : $acceptedActions) . '</div></header>';
        $body .= '<section class="fm2-otiz-summary" aria-label="Итоги среза"><div><span>К выплате</span><strong>' . $this->rub($payable) . '</strong></div><div><span>Штраф за срыв сроков</span><strong>' . $this->rub($deadlinePenalty) . '</strong></div><div><span>Блокеры</span><strong>' . $blockers . '</strong></div><div><span>Объекты</span><strong>' . count($objects) . '</strong></div></section>';
        $body .= '<section class="fm2-list-surface"><div class="fm2-list-toolbar"><h2>Очередь проверки</h2><span class="fm2-result-count">' . count($objects) . ' объектов</span></div><div class="fm2-table-wrap"><table class="shlz-table fm2-queue-table fm2-otiz-table"><thead><tr><th>Объект</th><th>Производственная динамика</th><th>Ксс</th><th>Штраф за сроки</th><th>Закрыто ранее</th><th>Новый пул</th><th>Статус</th></tr></thead><tbody>';
        foreach ($objects as $o) {
    ...
    private function history(): never
    {
        ...
        $body = '<header class="fm2-page-header"><div><h1>История срезов</h1><p>Каждая принятая версия открывается с исходным hash и неизменными результатами.</p></div></header><nav class="fm2-otiz-subnav" aria-label="Раздел ОТиЗ"><a class="shlz-link" href="/pilot/otiz">Очередь расчёта</a><a class="shlz-link" href="/pilot/otiz/objects">Объекты</a><a class="shlz-link" aria-current="page" href="/pilot/otiz/history">История срезов</a></nav><section class="fm2-list-surface"><div class="fm2-table-wrap"><table class="shlz-table fm2-queue-table fm2-otiz-history"><thead><tr><th>Отчётная дата</th><th>Версия</th><th>Принят</th><th>Новый пул</th><th>Закрыто ранее</th><th>Контроль</th></tr></thead><tbody>';
    ...
    private function objects(): never
    {
        ...
        $body = '<header class="fm2-page-header fm2-otiz-register-head"><div><h1>Экономика объектов</h1><p>Весь адресный перечень глазами ОТиЗ: расчётный фонд каждого объекта и фактическая экономика начатых работ.</p></div><p class="fm2-otiz-asof">План — по карточкам объектов · факт — по последнему срезу</p></header>';
        $body .= '<nav class="fm2-otiz-subnav" aria-label="Раздел ОТиЗ"><a class="shlz-link" href="/pilot/otiz">Очередь расчёта</a><a class="shlz-link" aria-current="page" href="/pilot/otiz/objects">Объекты</a><a class="shlz-link" href="/pilot/otiz/history">История срезов</a></nav>';
        ...
        foreach ($rows as $row) {
            ...
            $snapshotDate=$row['report_date']!==null?'Срез '.$this->date($row['report_date']):'Расчётных срезов нет';$progressDate=$row['progress_fact_date']!==null?'Факт на '.$this->date($row['progress_fact_date']):'Подтверждённого прогресса нет';
            $money=fn(?int$value):string=>$value===null?'<span class="shlz-table__empty">—</span>':'<strong>'.$this->rub($value).'</strong>';
            $body .= '<tr class="shlz-table__row" data-otiz-row data-state="'.$this->e($state).'" data-search="'.$this->e(mb_strtolower($row['regnumber'].' '.$row['address'])).'"><td class="shlz-table__cell"><a class="shlz-link" href="/pilot/objects/'.(int)$row['object_id'].'"><strong>'.$this->e($row['regnumber']?:'Объект №'.$row['object_id']).'</strong></a><small>'.$this->e($row['address']?:'Адрес не указан').'</small><small>'.$snapshotDate.'</small></td><td class="shlz-table__cell shlz-table__cell--numeric"><strong>'.$this->percent((int)($row['current_progress_bp']??0)).'</strong><small>'.$progressDate.'</small></td><td class="shlz-table__cell shlz-table__cell--numeric">'.$money($row['fund_cents']).($row['premium_cents']!==null?'<small>База '.$this->rub((int)$row['premium_cents']).'</small>':'<small>Не найдена норма</small>').'</td><td class="shlz-table__cell shlz-table__cell--numeric">'.($row['shaft_bp']!==null?'<strong>'.number_format((int)$row['shaft_bp']/10000,2,',',' ').'</strong>':'<span class="shlz-table__empty">—</span>').'</td><td class="shlz-table__cell shlz-table__cell--numeric">'.$money($row['snapshot_id']!==null?(int)$row['accrued_cents']:null).($row['snapshot_id']!==null?'<small>Ксс '.number_format((int)$row['kss_bp']/10000,2,',',' ').'</small>':'').'</td><td class="shlz-table__cell shlz-table__cell--numeric fm2-money-positive"><strong>'.$this->rub($rowPaid).'</strong></td><td class="shlz-table__cell shlz-table__cell--numeric fm2-money-negative"><strong>'.$this->rub($rowPenalty).'</strong><small>'.($rowPenalty>0?'Дисциплина '.$this->rub((int)$row['discipline_cents']).' · Ксс '.$this->rub((int)($row['deadline_penalty_cents']??0)):'Удержаний нет').'</small></td><td class="shlz-table__cell shlz-table__cell--numeric">'.$money($row['fund_cents']!==null?$rowBalance:null).($row['fund_cents']!==null?'<span class="fm2-fund-track" aria-label="Закрыто '.number_format($settled,0,',',' ').'% фонда"><i style="width:'.number_format($settled,2,'.','').'%"></i></span>':'').'</td><td class="shlz-table__cell"><span class="shlz-status '.$status[0].'">'.$status[1].'</span></td></tr>';
        }
    ...
    private function export(int $id): never
    {
        ...
        $sheets = ['Объекты' => [['Регномер','Адрес','Квып','Ксс','Закрыто ранее','Новый пул','Статус']], 'Работники' => [['Регномер объекта','Табельный номер','ФИО','Должность','Доля','Сумма']], 'Контроль' => [['Объект','Уровень','Код','Проблема','Владелец']], 'Приложение к приказу' => [['Табельный номер','ФИО','Сумма','Основание']], 'Метаданные' => [['Поле','Значение']]];
    ...
    private function closureForm(array $snapshot, array $object): string
    {
        ...
        $deadlineReduction=$this->deadlinePenalty($object);$disciplineForm=$available>0?'<details class="fm2-otiz-close"><summary>Есть дисциплинарное удержание</summary><form method="post" action="/pilot/otiz/snapshots/' . (int) $snapshot['id'] . '/closures"><input type="hidden" name="csrfToken" value="' . $this->e($this->csrf) . '"><input type="hidden" name="objectId" value="' . (int) $object['object_id'] . '"><label class="shlz-field"><span class="shlz-field__label">Дисциплинарное удержание, ₽</span><input class="shlz-input" name="discipline" inputmode="decimal" value="0" required></label><label class="shlz-field"><span class="shlz-field__label">Основание удержания</span><input class="shlz-input" name="basis" required placeholder="ПВР или служебная записка"></label><label class="shlz-field"><span class="shlz-field__label">Документ</span><input class="shlz-input" name="artifact" placeholder="Номер документа (необязательно)"></label><button class="shlz-button" type="submit">Добавить удержание</button><small>Можно удержать не более ' . $this->rub($available) . '</small></form></details>':'<p class="fm2-otiz-paid-note">Выплата по объекту отмечена в реестре среза.</p>';
        return '<div class="fm2-otiz-settlement"><div class="fm2-otiz-system-money" aria-label="Суммы, определённые системой"><div><span>Начислено до Ксс</span><strong>'.$this->rub((int)$object['accrued_cents']+$deadlineReduction).'</strong></div><div><span>Штраф за срыв сроков</span><strong>'.$this->rub($deadlineReduction).'</strong></div><p>К выплате после Ксс — '.$this->rub((int)$object['accrued_cents']).'. Выплаты подтверждаются одной кнопкой по всему срезу.</p></div>'.$disciplineForm.'</div>';
    }
