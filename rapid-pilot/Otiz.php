...
final class RapidPilotOtiz
{
    ...
    private function reconciliation(): never
    {
        ...
        $pageSize=50;$requestedPage=$this->requestedPage();$pageCount=max(1,(int)ceil(count($filtered)/$pageSize));if($requestedPage>$pageCount)$this->fail(404,'Страница не найдена.');$page=$requestedPage;$visible=array_slice($filtered,($page-1)*$pageSize,$pageSize);
    ...
    {
        $all=HistoricalPremiumReplayReadModel::load($this->db,$this->prefix);$page=$this->requestedPage();$pageSize=50;$pages=max(1,(int)ceil(count($all)/$pageSize));if($page>$pages)$this->fail(404,'Страница не найдена.');$rows=array_slice($all,($page-1)*$pageSize,$pageSize);
