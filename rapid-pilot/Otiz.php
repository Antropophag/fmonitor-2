...
final class RapidPilotOtiz
{
    ...
    private function command(string $path): never
    {
        ...
        if(preg_match('#^/pilot/otiz/cases/(\d+)/premium-facts$#D',$path,$m)===1){
            $caseId=(int)$m[1];$effective=(string)($_POST['effectiveDate']??'');$premium=filter_var($_POST['premiumCents']??null,FILTER_VALIDATE_INT,['options'=>['min_range'=>1]]);$shaft=filter_var($_POST['shaftBp']??null,FILTER_VALIDATE_INT,['options'=>['min_range'=>1,'max_range'=>20000]]);$source=$this->validatedSource();if(!$this->validDate($effective)||$premium===false||$shaft===false||$source===null)$this->fail(422,'Не заполнено подтверждённое основание premium/Кшах.');$now=$this->now();$s=$this->db->prepare("INSERT INTO `{$this->prefix}fm2_operational_premium_facts`(installation_case_id,effective_date,premium_cents,shaft_bp,source_label,source_locator,source_sha256,recorded_by_user_id,recorded_at) SELECT ?,?,?,?,?,?,?,?,? WHERE EXISTS(SELECT 1 FROM `{$this->prefix}fm2_migration_classification_provenance` mp WHERE mp.output_kind='operational_case' AND mp.output_id=? AND mp.category='native_candidate')");$s->bind_param('isiisssisi',$caseId,$effective,$premium,$shaft,$source['label'],$source['locator'],$source['sha256'],$this->userId,$now,$caseId);$s->execute();if($s->affected_rows!==1)$this->fail(422,'Финансовый факт разрешён только для native operational case.');$this->event(null,null,'operational_premium_fact_recorded',['caseId'=>$caseId,'factId'=>$s->insert_id,'sourceSha256'=>$source['sha256']]);$this->redirect('/pilot/otiz?fact=recorded');
        }
        if(preg_match('#^/pilot/otiz/cases/(\d+)/ktu-facts$#D',$path,$m)===1){
            $caseId=(int)$m[1];$tab=filter_var($_POST['installerTabId']??null,FILTER_VALIDATE_INT,['options'=>['min_range'=>1]]);$effective=(string)($_POST['effectiveDate']??'');$ktu=filter_var($_POST['ktuBp']??null,FILTER_VALIDATE_INT,['options'=>['min_range'=>1,'max_range'=>10000]]);$source=$this->validatedSource();if($tab===false||!$this->validDate($effective)||$ktu===false||$source===null)$this->fail(422,'Не заполнено подтверждённое основание КТУ.');$now=$this->now();$s=$this->db->prepare("INSERT INTO `{$this->prefix}fm2_operational_ktu_facts`(installation_case_id,installer_tab_id,effective_date,ktu_bp,source_label,source_locator,source_sha256,recorded_by_user_id,recorded_at) SELECT ?,?,?,?,?,?,?,?,? WHERE EXISTS(SELECT 1 FROM `{$this->prefix}fm2_assignment_orders` o JOIN `{$this->prefix}fm2_order_installers` oi ON oi.assignment_order_id=o.id JOIN `{$this->prefix}fm2_migration_classification_provenance` mp ON mp.output_kind='operational_case' AND mp.output_id=o.installation_case_id AND mp.category='native_candidate' WHERE o.installation_case_id=? AND o.status='registered' AND oi.installer_tab_id=?)");$s->bind_param('iisisssisii',$caseId,$tab,$effective,$ktu,$source['label'],$source['locator'],$source['sha256'],$this->userId,$now,$caseId,$tab);$s->execute();if($s->affected_rows!==1)$this->fail(422,'КТУ разрешён только для монтажника native operational case с зарегистрированным распоряжением.');$this->event(null,null,'operational_ktu_fact_recorded',['caseId'=>$caseId,'factId'=>$s->insert_id,'installerTabId'=>$tab,'sourceSha256'=>$source['sha256']]);$this->redirect('/pilot/otiz?fact=recorded');
        }
    ...
    private function queue(): never
    {
        ...
        $cards .= '<nav class="fm2-otiz-subnav" aria-label="Раздел ОТиЗ"><a class="shlz-link" aria-current="page" href="/pilot/otiz">Очередь расчёта</a><a class="shlz-link" href="/pilot/otiz/history">История срезов</a></nav>';
    ...
    private function history(): never
    {
        ...
        $body = '<header class="fm2-page-header"><div><h1>История срезов</h1><p>Каждая принятая версия открывается с исходным hash и неизменными результатами.</p></div></header><nav class="fm2-otiz-subnav" aria-label="Раздел ОТиЗ"><a class="shlz-link" href="/pilot/otiz">Очередь расчёта</a><a class="shlz-link" aria-current="page" href="/pilot/otiz/history">История срезов</a></nav><section class="fm2-list-surface"><div class="fm2-table-wrap"><table class="shlz-table fm2-queue-table fm2-otiz-history"><thead><tr><th>Отчётная дата</th><th>Версия</th><th>Принят</th><th>Новый пул</th><th>Закрыто ранее</th><th>Контроль</th></tr></thead><tbody>';
    ...
    public static function bootstrap(mysqli $db, string $prefix): void
    {
        ...
        $queries = [
            ...
            ,"CREATE TABLE IF NOT EXISTS `{$prefix}fm2_operational_premium_facts`(id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,installation_case_id BIGINT UNSIGNED NOT NULL,effective_date DATE NOT NULL,premium_cents BIGINT UNSIGNED NOT NULL,shaft_bp INT UNSIGNED NOT NULL,source_label VARCHAR(160) NOT NULL,source_locator VARCHAR(300) NOT NULL,source_sha256 CHAR(64) NOT NULL,recorded_by_user_id BIGINT UNSIGNED NOT NULL,recorded_at VARCHAR(40) NOT NULL,KEY(installation_case_id,effective_date,id)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4"
            ,"CREATE TABLE IF NOT EXISTS `{$prefix}fm2_operational_ktu_facts`(id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,installation_case_id BIGINT UNSIGNED NOT NULL,installer_tab_id BIGINT UNSIGNED NOT NULL,effective_date DATE NOT NULL,ktu_bp INT UNSIGNED NOT NULL,source_label VARCHAR(160) NOT NULL,source_locator VARCHAR(300) NOT NULL,source_sha256 CHAR(64) NOT NULL,recorded_by_user_id BIGINT UNSIGNED NOT NULL,recorded_at VARCHAR(40) NOT NULL,KEY(installation_case_id,installer_tab_id,effective_date,id)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4"
    ...
    private function validatedSource():?array{$label=trim((string)($_POST['sourceLabel']??''));$locator=trim((string)($_POST['sourceLocator']??''));$sha=strtolower(trim((string)($_POST['sourceSha256']??'')));return$label!==''&&mb_strlen($label)<=160&&$locator!==''&&mb_strlen($locator)<=300&&preg_match('/^[a-f0-9]{64}$/D',$sha)===1?compact('label','locator')+['sha256'=>$sha]:null;}
