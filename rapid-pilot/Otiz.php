...
final class RapidPilotOtiz
{
    ...
    private function queue(): never
    {
        ...
        if (is_array($latest)) {
            $closed = (int)$this->db->query("SELECT COALESCE(SUM(paid_cents+discipline_cents+deadline_cents),0) n FROM `{$this->prefix}fm2_pilot_otiz_payment_closures` WHERE object_id IN(SELECT object_id FROM `{$this->prefix}fm2_pilot_otiz_snapshot_objects` WHERE snapshot_id=".(int)$latest['id'].")")->fetch_assoc()['n'];
    ...
    private function snapshot(int $id): never
    {
        ...
        $payable=0;$deadlinePenalty=0;$closedByObject=[];foreach($objects as$object){$closed=(int)$this->db->query("SELECT COALESCE(SUM(paid_cents+discipline_cents+deadline_cents),0) n FROM `{$this->prefix}fm2_pilot_otiz_payment_closures` WHERE object_id=".(int)$object['object_id'])->fetch_assoc()['n'];$closedByObject[(int)$object['object_id']]=$closed;if($object['calculation_state']!=='blocked')$payable+=max(0,(int)$object['accrued_cents']-$closed);$deadlinePenalty+=$this->deadlinePenalty($object);}
    ...
    private function closureForm(array $snapshot, array $object): string
    {
        ...
        $closed = (int) $this->db->query("SELECT COALESCE(SUM(paid_cents+discipline_cents+deadline_cents),0) n FROM `{$this->prefix}fm2_pilot_otiz_payment_closures` WHERE object_id={$objectId}")->fetch_assoc()['n'];
