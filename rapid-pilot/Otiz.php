...
final class RapidPilotOtiz
{
    ...
    {
        $category=(string)($_GET['category']??'');$code=(string)($_GET['code']??'');$page=filter_var($_GET['page']??1,FILTER_VALIDATE_INT,['options'=>['min_range'=>1]])?:1;$model=(new MigrationQuarantineReadModel($this->db,$this->prefix))->read(['category'=>$category,'code'=>$code],$page);$category=$model['filters']['category']??'';$code=$model['filters']['code']??'';
        ...
        if($model['rows']===[])$body.='<section class="fm2-empty"><h2>Записей не найдено</h2><p>Registry пуст или выбранные фильтры не совпали. Никакие import-операции не выполнялись.</p></section>';else{$body.='<section class="fm2-recon-list" aria-label="Pre-import quarantine registry">';foreach($model['rows']as$r){$reasons=implode(' · ',array_map($this->reasonLabel(...),$r['reasonCodes']));$codes='<ul>'.implode('',array_map(fn(string$c):string=>'<li><code>'.$this->e($c).'</code> — '.$this->e($this->reasonLabel($c)).'</li>',$r['quarantineCodes'])).'</ul>';$body.='<article class="fm2-recon-item"><header><div><h2>'.$this->e($r['reference']).'</h2><p>Стабильная redacted reference; raw legacy ID не отображается</p></div><div class="fm2-recon-state"><span class="shlz-status shlz-status--orange">'.$this->e($r['status']).'</span><strong>'.$this->e($r['category']).'</strong></div></header><div class="fm2-recon-grid"><section><h3>Классификация</h3><p>'.$this->e($reasons).'</p><small>'.$this->e($r['classificationVersion']).'</small></section><section><h3>Source provenance</h3><dl><div><dt>Cutoff</dt><dd>'.$this->e($r['sourceCutoffAt']).'</dd></div><div><dt>Source digest</dt><dd title="'.$this->e($r['sourceDigest']).'">'.$this->e(substr($r['sourceDigest'],0,16)).'…</dd></div><div><dt>Locator hash</dt><dd title="'.$this->e($r['locatorHash']).'">'.$this->e(substr($r['locatorHash'],0,16)).'…</dd></div></dl></section><section><h3>Наблюдение</h3><dl><div><dt>Статус</dt><dd>'.$this->e($r['status']).'</dd></div><div><dt>Наблюдений</dt><dd>'.(int)$r['observationCount'].'</dd></div><div><dt>Последнее</dt><dd>'.$this->e($r['lastObservedAt']!==''?$r['lastObservedAt']:'—').'</dd></div><div><dt>Run</dt><dd>'.$this->e($r['runReference']!==''?$r['runReference']:'—').'</dd></div></dl></section><section class="fm2-recon-conflicts"><h3>Quarantine reasons</h3>'.$codes.'</section></div><p class="fm2-active-readonly">Read-only registry. Snapshot decision ledger неприменим: отдельный quarantine-decision contract в этом slice не создаётся.</p></article>';}$body.='</section>'.$this->quarantinePager($model,$category,$code);}$this->page('Pre-import quarantine',$body);
    }
