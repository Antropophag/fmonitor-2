...
3. Container delivery использует unique temporary regular file, fsync/complete write и atomic rename в tmpfs. Wrapper запускает child асинхронно, пересылает TERM/INT/QUIT, ждёт child и только затем очищает файлы, сохраняя importer/interruption status. Make задаёт one-shot stop signal, поддерживаемый wrapper, не меняя production PHP-FPM service policy. Secrets не входят в command text или environment values: только paths.
