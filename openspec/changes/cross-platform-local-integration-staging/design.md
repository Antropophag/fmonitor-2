...
2. Make/Compose выполняют привилегированный только этап копирования host snapshot в private container storage и устанавливают container ownership/mode; сам PHP importer запускается стандартным non-root user. Host `.env` и snapshot не изменяются. Прямой bind mount к importer и `chmod 0644` отвергнуты.
3. Container delivery использует unique temporary regular file, fsync/complete write и atomic rename внутри container-owned storage. Target устанавливает trap/finalizer, удаляет delivered file на всех выходах и возвращает сохранённый importer status. Secrets не входят в command text или environment values: только paths.
