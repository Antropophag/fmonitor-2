...
- [x] 2.2 Добавить проверки server pagination, full-set totals, stable ordering, out-of-range behavior, saved-versus-current labels, GET и доступных source transitions; подтвердить RED и записать команду/вывод в `reviews/tests/OTIZ-OBJECT-LEDGER-HISTORY-001.md`.
...
- [x] 3.1 Отдельному executor реализовать SQL-filtered consistent read одной страницы, count/full signed totals, source/reversal relations и object-in-snapshot validation; focused read-model/HTTP tests SHALL стать GREEN.
- [x] 3.2 Отдельному executor добавить lazy transition и локальное представление в существующем snapshot/object context с escaped basis, distinct saved/current copy, pagination, empty/error states и без financial actions; focused HTTP tests SHALL стать GREEN.
- [x] 3.3 Выполнить только planner-selected bounded local checks, обязательный architecture check и HTTP qualification при касании `app/PilotHttp/*.php`; полный локальный `make test`/`make verify` не запускать и сохранить точные результаты.
