...
- [ ] 1.6 Поручить fresh independent Gate 1 reviewer проверить v4 lease-conflict amendment и весь current contract: CAS loser holds lease through fingerprint/current-lineage rereads, releases exactly once, release failure preserves selected result и не пропускает audit; verification: новый immutable review имеет explicit `APPROVED` и reviewer не редактировал artifacts
