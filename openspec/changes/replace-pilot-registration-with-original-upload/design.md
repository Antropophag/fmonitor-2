...
Gate 2 descriptor-integrity proof использует named public `AssignmentOrderOriginalProductionFactoryVerification::create(mysqli, config, observer)` той же production logger composition. Exact enum `FINAL_PATH_VALIDATED_BEFORE_OPEN` публикуется один раз после final non-following pathname regular/current-EUID/`0600` validation и device/inode capture, непосредственно перед real append open; observer получает только phase. Task-owned observer ordinary owner `chmod` меняет тот же inode `0600→0640`, после чего real retained-descriptor `fstat` обязан увидеть same identity/actual `0640`. Unchanged inert control и отдельные setup assertions доказывают harness; expected failure фиксирует exact factory error, zero mismatch DB calls/private-root touch/write, unchanged bytes/only deliberate mode metadata и отсутствие retained descriptor через live-child before/after integer-keyed `get_resources('stream')` inventories с обязательным successful `fstat` каждого entry. Observer Throwable fixed-redacted до open/resources. Production factory всегда использует final inert observer без selector. Cleanup revalidates/restores/removes only enumerated task-owned artifacts under monotonic bounded terminate/reap. Native interposition/interception/loader injection, privilege changes, external targets, sleeps и probabilistic loops запрещены.
