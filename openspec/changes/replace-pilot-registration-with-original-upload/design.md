...
Production command construction получает обязательный `safeLogFile` через `AssignmentOrderOriginalProductionConfig`. Самым первым resource-sensitive шагом factory канонизирует путь и доказывает, что configured entry уже существует, сама не symlink, является regular file, принадлежит effective current user и имеет exact mode `0600`. Любое несоответствие даёт один fixed redacted construction exception до DB operation и до private-storage validation/access; factory не создаёт, не заменяет, не chmod/chown и не очищает файл. После успешной проверки real safe-log observer открывает canonical identity только для append и сохраняет cleanup/release diagnostics, не включая path, secret или exception detail. Worker/evidence-reader `safeLogFile` остаётся отдельным ранее утверждённым serializable config contract и не подменяет production-config field.
...
- [Production cleanup/release diagnostics могут потеряться при неверной конфигурации] → обязательный pre-created owned regular `0600` safe log проверяется fail-closed до DB/private storage и используется только append-only; auto-create/repair запрещены.
