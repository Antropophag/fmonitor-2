...
## Current-line continuation — 2026-09-08

База новой проверяемой цепочки — `b5fca7d4df56404a4ac4eba802d497066a42b6c8`;
рабочий runtime отдельно зафиксирован на `4990cf1` с literal VERIFY_OK,
backup/migration19/restart/isolated image golden proof. Источник готового QG
пакета — immutable local ref `f07548135fe930e7a8fb9bb97271c9f05a8ebfc1`.
Переносится один vertical governance seam с новыми текущими RED, независимыми
Gate3 и Gate5, GREEN и receipt; merge105 старых commits и выдача старых reviews
за current approvals не выполняются. Executable v0.6 spec остаётся побайтно прежней.

Current exact evidence envelope использует named parity/final-verification files:
...
Это конкретизация разрешённых v0.6 категорий; произвольные operations files,
source, tests, spec и graph после reviewed implementation по-прежнему запрещены.
Именованные phase-B evidence не означают, что phase-B задача завершена.

Current runner обязан подготовить зависимости существующего `make verify` в
чистом Linux CI: PHP/extensions, Node/headless browser, exact TCPDF6.11.4 и public
SHLZ export из уже pinned revision. Подготовка остаётся repository-owned setup,
не копией проверок внутри graph. Две исторические test-fixture привязки к
абсолютному Mac home устраняются в test-owned setup, без изменения runtime/domain
поведения и без создания фиктивного Mac home на CI. Generated runner/manifest
создаются только pinned compiler; publisher privileges не расширяются.
