...
- Использовать repository-owned минимальный trusted publisher поверх upstream publish/watch runtime: без `issue_comment`, command/approval job и лишних write permissions, поскольку compiler v0.1.7 генерирует этот интерфейс даже при отключённых approvals.
...
- Delivery surface: Quality Graph declaration/generated runner/manifest, repository-owned minimal publisher workflow, governance checker, provenance manifest/receipts и документация операций.
