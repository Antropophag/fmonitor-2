...
Явные non-goals: новые формулы/справки №66, пересчёт принятой истории, изменение полномочий или separation of duties, миграция данных, произвольные новые виды выплат, общий cutover, общая переработка Compose/startup/recovery и deployment. В срез входит только минимальная production composition, необходимая самому Yii2 OTIZ runtime: явный Yii login, ключи и подготовка durable session directory. Неопределённые целевые semantics принятия из `CHARACTERIZE-OTIZ-SNAPSHOT-ACCEPTANCE-001` остаются `NEEDS_GRILL`; этот refactor не превращает наблюдаемое pilot-поведение в новую продуктовую норму и не закрывает соответствующий redesign slice.
