...
### 8. Динамика использует фиксированное окно 42 дня
...
Dashboard принимает необязательный allowlisted `utilizationTo=YYYY-MM-DD`. Конец окна включителен, начало вычисляется сервером как `to - 41 day`; default — дата общего dashboard-среза. Кнопки public `shlz-button` сдвигают окно ровно на 42 дня назад или вперёд. Следующий период отключён, когда его конец оказался бы позднее даты среза. URL хранит выбранное окно, поэтому reload/back сохраняют контекст. Read model выбирает только наблюдения внутри границ; отсутствие наблюдения не превращается в ноль. На desktop 42 дневные пары образуют плотную горизонтальную шкалу с подписями недельных границ, на mobile — тот же scrollable plot без page overflow. Каждая существующая точка остаётся keyboard-link на сохранённый drill-down. Виджет содержит только заголовок, легенду, период с навигацией и plot; summary-плашки удаляются.
