...
Local `make up` SHALL явно выбирать один IdentityAccess-owned local startup mode. Под существующим per-database/prefix lock этот mode SHALL создать initial owner существующим strict create только когда canonical identity полностью пуста; если identity непуста, он SHALL перейти только к read-only resume. Resume SHALL вернуть success только когда requested normalized email однозначно указывает на одного active/unblocked owner, его credential существует, а его действующие назначения `user` и `superadministrator` имеют bootstrap provenance с null actor и обеспечивают необходимые текущие owner permissions. Наличие других пользователей, приглашений, auth/status/role history, изменённых profile/session/credential данных либо дополнительных non-bootstrap ролей само по себе SHALL NOT блокировать resume. Resume MUST NOT сравнивать или изменять текущий password secret и MUST NOT изменять users, credentials, sessions, profile, grants, invitations или историю.
...
#### Scenario: Первый локальный запуск создаёт владельца
- **WHEN** local `make up` вызывает explicit local mode на полностью пустой canonical identity
- **THEN** тот же IdentityAccess owner под тем же lock выполняет strict create и возвращает `created`; следующий identical local-mode вызов возвращает `already_provisioned` без новых facts
...
CLI SHALL проверить canonical readiness без DDL, работать под explicit provisioning authority, получить per-database/prefix lock timeout 0 и всегда release. Web/FPM, migration runner и production deployment startup SHALL NOT неявно выбирать local mode. Только repository-owned local `make up` SHALL вызывать явный local mode после migrations; выбор clean-create/read-only-resume SHALL принадлежать IdentityAccess внутри lock, без SQL в Make/CLI. Прямой CLI без этого явного режима сохраняет production clean-create/exact-replay semantics.
...
#### Scenario: Первый real local-up
- **WHEN** оператор запускает `make up` после migrations с полностью пустой canonical identity
- **THEN** тот же explicit local-mode CLI создаёт initial owner до запуска php/web; Make не классифицирует identity и не подавляет отказ
