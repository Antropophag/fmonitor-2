...
1. Owning module остаётся `YiiRuntime`: отдельный локальный adapter использует application-owned `yii\db\Connection` и `yii\db\Query`, не создаёт mysqli lifecycle и не зависит от Jobs command owners. Альтернатива — расширить controller private methods — отвергнута, потому что event correlation и receipt validation образуют самостоятельный read contract и раздуют HTTP coordination seam.
...
7. Persistence owner — существующий Jobs module; migration/backup/restore/readiness не меняются. `rapid-pilot` adapter не нужен. Architecture check должен подтвердить отсутствие новых global calls и command dependencies; HTTP qualification обязателен из-за изменения controller.
