...
function demoProvision(array $config, int $generation): array
{
    ...
    try {
        ...
        demoWriteJson($directory . '/ready.json', ['fingerprint'=>$config['fingerprint'], 'generation'=>$generation, 'schemaVersion'=>25, 'objectId'=>4512]);
...
function demoGeneration(array $config, int $generation): ?array
{
    ...
    if (($owner['fingerprint'] ?? null) !== $config['fingerprint'] || ($ready['fingerprint'] ?? null) !== $config['fingerprint'] || ($ready['schemaVersion'] ?? null) !== 25) return null;
