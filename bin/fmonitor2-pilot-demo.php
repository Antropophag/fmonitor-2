...
function demoProvision(array $config, int $generation): array
{
    ...
    try {
        ...
        demoWriteJson($directory . '/ready.json', ['fingerprint'=>$config['fingerprint'], 'generation'=>$generation, 'schemaVersion'=>23, 'objectId'=>4512]);
...
function demoGeneration(array $config, int $generation): ?array
{
    ...
    if (($owner['fingerprint'] ?? null) !== $config['fingerprint'] || ($ready['fingerprint'] ?? null) !== $config['fingerprint'] || ($ready['schemaVersion'] ?? null) !== 23) return null;
