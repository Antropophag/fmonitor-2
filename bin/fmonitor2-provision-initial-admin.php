...
$exit=0;$db=null;
try{
    $resumeExistingLocal=$argc===4&&$argv[1]==='--resume-existing-local'&&$argv[2]==='--email'&&is_string($argv[3]);
    if(!$resumeExistingLocal&&($argc!==3||$argv[1]!=='--email'||!is_string($argv[2])))throw new InvalidArgumentException();
    $email=$resumeExistingLocal?$argv[3]:$argv[2];
