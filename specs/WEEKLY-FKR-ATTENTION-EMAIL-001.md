...
Каждый понедельник руководитель получает письмо только по доступным ему объектам: планы открытия/закрытия, изменение подтверждённого прогресса и фактическую просрочку. Отчёт ничего не исправляет в монтажных делах.
...
HTML и plain text строятся из одной immutable view model в порядке: `Плановые открытия`, `Плановые закрытия`, `Прогресс за прошедшую неделю`, `Просрочка`. Каждый HTML-раздел — Outlook-safe таблица с явными заголовками колонок.
...
4. Основной текст не мельче `13px`; смысл статуса не кодируется одним цветом.
5. Визуальный язык наследует FMonitor: тёмно-синий заголовок, спокойный светлый фон, белый content canvas, тонкие нейтральные разделители и красный только для «Просрочка». Иерархия не зависит от gradient, shadow или декоративных карточек.
...
1. Открытия: незавершённое открытие с canonical plan date внутри planPeriod, дата и current native статус монтажного дела (например, `Требуется распоряжение` или `Готов к открытию`), без fallback `Не открыт`.
...
Максимум subject+HTML+text до SMTP encoding — `5_242_880` UTF-8 bytes. Measured focused fixture из 1 000 табличных строк во всех четырёх секциях SHALL оставаться ниже этого предела. Строки не обрезаются: overflow даёт permanent `REPORT_TOO_LARGE` без сети, с safe byte count/limit.

## 8. Delivery и concurrency
...
| A3 | Четыре табличных секции, headers/order, empty, escaping, HTTPS links, sorting; image-free Outlook/web markup и narrow layout |
| A4 | Current-week boundary dates и corrected plan date as-of |
| A5 | Historical 85/15, correction, UNKNOWN≠0, fixed delta |
| A6 | Old/today overdue, явные opening+closing types, exact days, no cutoff |
| A7 | Current native opening status без fallback `Не открыт` |
