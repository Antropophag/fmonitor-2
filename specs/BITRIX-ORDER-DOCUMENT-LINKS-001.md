...
Оператор SHALL задавать кадровый и document webhook отдельными private `.env` ключами `FMONITOR_BITRIX_WEBHOOK_URL` и `FMONITOR_BITRIX_ORDER_DOCUMENT_WEBHOOK_URL`. Host staging SHALL объединить их в один атомарный private config с разными `baseUrl` и `documentBaseUrl`; document delivery MUST читать origin/user/token только из `documentBaseUrl` и MUST NOT fallback на кадровые credentials. Отсутствующий, malformed либо credential-identical document webhook SHALL fail closed до Bitrix request. Existing workforce temporary token cleanup MUST сохраняться.
...
Секция SHALL отображаться в существующей object card после `objects.read`. Для одного номера заказа SHALL существовать не более одной effective ссылки; неоднозначность SHALL fail soft без выбора произвольной ссылки. В construction-control queue available effective ссылка SHALL отображаться отдельной круглой primary `shlz-ui/folder-file-open` action рядом с inspection action; missing/empty/unavailable status SHALL отображаться disabled action без URL. Checklist заводской номер SHALL находиться в правой колонке hero над процентом монтажа и быть прямой ссылкой на effective документацию; при отсутствии ссылки номер остаётся обычным текстом. Ссылочный номер SHALL сохранять brand-blue без подчёркивания в default state, получать подчёркивание при pointer hover и сохранять заметный keyboard focus; нессылочный номер SHALL оставаться чёрным. Отсутствие или недоступность ссылки MUST NOT ломать queue, checklist или карточку. Queue presentation в этом абзаце superseded owner decision 2026-09-25; safe exact mapping и ambiguity behavior не меняются.
