...
- Actor: прогнозные counts/details наследуют доступ общего dashboard и доступны любому аутентифицированному активному пользователю. Capability/scope не превращают исправный прогноз в unavailable.
...
6. Существующий единственный widget «Загрузка монтажников и динамика» SHALL показывать шестинедельный forecast; отдельный второй forecast widget и historical chart на dashboard запрещены. Каждая week/group value ведёт на live detail с bounds, denominator, людьми и основаниями, сортировка `fio,tabId`; новый authoritative fact меняет прогноз, но не observations.
7. Guest получает canonical redirect. Любой аутентифицированный активный actor, включая scoped/без `installers.read`, получает forecast dashboard/detail `200`. HEAD повторяет status/headers GET и имеет пустое body. Чтение не создаёт jobs, observations, audits или domain facts.
...
| J | guest/denied/scoped GET|HEAD | guest redirect; все authenticated actors dashboard/detail 200; empty HEAD |
...
| N | dashboard composition | ровно один utilization widget с forecast; отдельного forecast/history widget нет |
