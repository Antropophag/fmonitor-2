...
Версия — server-owned build identity длиной 1–80 ASCII `[A-Za-z0-9._-]`, прочитанная из заранее сформированного immutable runtime build-файла, либо явное `unknown`; это не маркетинговое имя `FMonitor 2.0`, не Git SHA по догадке и не пользовательское поле. HTTP feedback не вызывает source hashing, Git, Docker API или внешние запросы. Отсутствующий, недоступный, изменяемый, ссылочный или некорректный build-файл даёт `unknown` без недоступности feedback и соседних экранов; строгая readiness-проверка остаётся отдельной и fail closed. Вводимое пользователем описание сохраняется как текст, UI предупреждает не указывать пароли и содержимое документов; автоматическое содержимое страницы не собирается.
...
### A8 — Актуальные исходные экраны

К закрытому allowlist A2 добавляются только user-facing GET-экраны актуального router, на которых существует штатная feedback-ссылка: `/pilot/calendar`, `/pilot/dashboard`, `/pilot/users`, `/pilot/otiz`, `/pilot/otiz/objects`, `/pilot/otiz/payments`, `/pilot/otiz/history`; `/pilot/objects/<objectId>/assignment-order/prepare`, `/pilot/objects/<objectId>/deadline-certificates`; `/pilot/otiz/snapshots/<snapshotId>`. В форме, сохранённой записи, подтверждении, ссылке возврата и административном списке используется один и тот же нормализованный path без query/fragment.

Snapshot ID, feedback ID, order ID и любые другие идентификаторы не становятся object ID. `object_id` извлекается только из явно разрешённых объектных путей: карточки, checklist, selection/prepare/execution, deadline certificates, стройконтроля и original submit/history. Например `/pilot/otiz/snapshots/731` сохраняет path и `object_id=null`, а `/pilot/objects/1450/deadline-certificates` — path и `object_id=1450`.

Не разрешаются произвольный `/pilot/*`, login/activation, sync-context, photo/original download, `.xlsx` export, mutating endpoints, внешние/сетевые URL, backslash, управляющие символы, percent-encoded paths и oversized IDs. Query и fragment всегда отбрасываются; если сам path не разрешён, сохраняется `/pilot/objects` и `object_id=null`. Старые упомянутые в issue пути не восстанавливаются без действующего user-facing GET route.

### A9 — Build identity, история и replay

Для нового обращения server-owned build identity фиксируется в существующем `app_version VARCHAR(80)` при первой вставке и возвращается listing из сохранённой строки. Полный 64-символьный digest не сокращается. Клиентский POST не имеет доверенного version input; добавленное клиентом поле игнорируется. Административное представление явно и экранированно показывает «Исходный экран» и «Сборка» из persisted item, не вычисляя текущую сборку при чтении.

Для сборок A=`aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa` и B=`bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb` новые request IDs сохраняют соответственно полный A и B. Обращение A после обновления остаётся A. Точный actor-scoped replay исходного requestId после потери ответа или обновления возвращает прежний id, одну исходную запись, прежние context/build и не выполняет UPDATE; build identity не входит в fingerprint пользовательского содержания. Иной description или нормализованный context с тем же actor/requestId остаётся conflict без новых фактов. Те же свойства сохраняются при конкурентных повторах.
