...
def register_test(repo, path, category, suite='unit', runtime='python3'):
    catalog = repo / 'tools/verification/suites.tsv'
...
class DeliveryHarnessCiCompleteness(unittest.TestCase):
    ...
    def fixture_repo(self):
        ...
        for directory in ['InstallationProcess', 'AssignmentOrderComposition', 'Otiz', 'Runtime', 'Jobs']:
            (repo / 'tests' / directory).mkdir(parents=True, exist_ok=True)
        for line in (repo / 'tools/verification/suites.tsv').read_text().splitlines():
            if not line:
                continue
    ...
    def test_pr98_inventory_is_complete_before_publication(self):
        ...
        self.assertIn(f'UNREGISTERED_TEST: {synthetic}', planned.stderr)
        register_test(repo, synthetic, 'e2e', suite='e2e')
        planned, plan_path, plan_name = self.plan(repo, environment, base, input_name)
        ...
        self.assertEqual({'GENERATED_SOURCE_DRIFT', 'UNDECLARED_TEST_DEPENDENCY'},
    ...
    def test_pr100_db_in_unit_is_rejected_in_unit_environment(self):
        ...
        register_test(repo, synthetic, 'unit', runtime='php')
    ...
    def test_preflight_observes_services_and_dependencies_instead_of_copying_profile(self):
        ...
        register_test(repo, synthetic, 'integration', suite='db')
    ...
    def test_preflight_accepts_repository_local_sibling_python_import(self):
        ...
        register_test(repo, synthetic, 'unit')
        input_name = self.write_input(repo, test=synthetic)
        planned, _, plan_name = self.plan(repo, environment, base, input_name)
        self.assertEqual(0, planned.returncode, planned.stderr)
        preflight = self.command(repo, environment, sys.executable,
    ...
    def test_preflight_accepts_canonical_node_builtin_imports(self):
        ...
        register_test(repo, synthetic, 'unit', runtime='node')
        input_name = self.write_input(repo, test=synthetic)
        planned, _, plan_name = self.plan(repo, environment, base, input_name)
        self.assertEqual(0, planned.returncode, planned.stderr)
        preflight = self.command(repo, environment, sys.executable,
    ...
    def test_reviewer_package_accepts_plan_owned_category_evidence_only(self):
        ...
        register_test(repo, acceptance, 'governance', suite='characterization')
        register_test(repo, category_test, 'governance', suite='characterization')
    ...
    def test_gate3_test_delta_links_historical_red_and_rejects_mismatch(self):
        ...
        planned, plan_path, _plan_name = self.plan(repo, environment, base, input_name)
        self.assertEqual(0, planned.returncode, planned.stderr)
        command_environment = next(item['environment'] for item in json.loads(plan_path.read_text())['commands']
                                   if item['argv'][-1] == test)
        encoded_environment = json.dumps(command_environment, sort_keys=True, separators=(',', ':'))
        historical = self.command(repo, environment, sys.executable, 'tools/delivery/harness.py', 'run',
                                  ...
                                  '--acceptance-id', 'fixture', '--command-environment', encoded_environment,
                                  '--intended-red', 'EXPECTED_DELTA_RED',
                                  '--', sys.executable, test)
