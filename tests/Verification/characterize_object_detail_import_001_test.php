...
function odciExactContainerAbsent(string $container): bool
{
    $inspect=odciDocker(['container','inspect',$container]);if($inspect['status']===0)return false;if(trim($inspect['stdout'])!=='[]'||trim($inspect['stderr'])!=='Error response from daemon: No such container: '.$container)odciFail('SETUP_FAILURE','container absence was not exact not-found');return true;
}

function odciAcquire(string $token,string $artifactRoot,?string $fault=null): array
{
    ...
    $child=$root.'/object-detail-'.$token;$container='fm2-odci-'.$token;$image=odciDocker(['image','inspect','--format','{{.Id}}',ODCI_IMAGE]);$imageId=trim($image['stdout']);if($image['status']!==0||preg_match('/^sha256:[a-f0-9]{64}$/D',$imageId)!==1)odciFail('SETUP_FAILURE','expected local MariaDB image unavailable');if(@lstat($child)!==false||!odciExactContainerAbsent($container))odciFail('SETUP_FAILURE','owned namespace occupied');
    $owned=['child'=>$child,'container'=>$container,'containerId'=>'','imageId'=>$imageId,'volumes'=>[]];$childOwned=false;$createAttempt=false;
    try {
        if(!mkdir($child,0700))odciFail('SETUP_FAILURE','artifact child unavailable');$childOwned=true;if(!chmod($child,0700))odciFail('SETUP_FAILURE','artifact child mode unavailable');if($fault==='after-child')odciFail('SETUP_FAILURE','injected post-child acquisition fault');
        $createAttempt=true;$create=odciDocker(['create','--name',$container,'--label','fmonitor2.object-detail-token='.$token,'--tmpfs','/var/lib/mysql','-e','MARIADB_ROOT_PASSWORD='.ODCI_ROOT_PASSWORD,'-p','127.0.0.1::3306',$imageId]);if($fault==='after-create-response-loss')odciFail('SETUP_FAILURE','injected Docker create response loss');if($create['status']!==0)odciFail('SETUP_FAILURE','private container create failed');$owned['containerId']=trim($create['stdout']);
        $storage=odciDocker(['inspect','--format','{{json .Mounts}}|{{json .HostConfig.Tmpfs}}',$container]);if($storage['status']!==0)odciFail('SETUP_FAILURE','container storage inspect failed');[$mountJson,$tmpfsJson]=explode('|',trim($storage['stdout']),2);$mounts=json_decode($mountJson,true,flags:JSON_THROW_ON_ERROR);$tmpfs=json_decode($tmpfsJson,true,flags:JSON_THROW_ON_ERROR);foreach($mounts as $mount)if(($mount['Type']??null)==='volume'&&is_string($mount['Name']??null))$owned['volumes'][]=$mount['Name'];if($mounts!==[]||!is_array($tmpfs)||array_keys($tmpfs)!==['/var/lib/mysql'])odciFail('SETUP_FAILURE','container storage proof failed');
        $start=odciDocker(['start',$container]);$portResult=odciDocker(['port',$container,'3306/tcp']);if($start['status']!==0||$portResult['status']!==0||preg_match('/127\.0\.0\.1:(\d+)/',$portResult['stdout'],$m)!==1)odciFail('SETUP_FAILURE','private container start/port failed');$identity=odciDocker(['inspect','--format','{{.Id}} {{.Image}} {{index .Config.Labels "fmonitor2.object-detail-token"}}',$container]);if($identity['status']!==0||trim($identity['stdout'])!==$owned['containerId'].' '.$imageId.' '.$token)odciFail('SETUP_FAILURE','container identity failed');return $owned+['port'=>(int)$m[1]];
    } catch(Throwable $error) {
        $recovery=[];if($createAttempt&&$owned['containerId']==='')try{$probe=odciDocker(['inspect','--format','{{.Id}}|{{.Image}}|{{index .Config.Labels "fmonitor2.object-detail-token"}}|{{json .Mounts}}',$container]);if($probe['status']===0){[$id,$actualImage,$label,$mountJson]=explode('|',trim($probe['stdout']),4);if($actualImage!==$imageId||$label!==$token)throw new RuntimeException();$owned['containerId']=$id;$mounts=json_decode($mountJson,true,flags:JSON_THROW_ON_ERROR);foreach($mounts as $mount)if(($mount['Type']??null)==='volume'&&is_string($mount['Name']??null))$owned['volumes'][]=$mount['Name'];}elseif(!odciExactContainerAbsent($container))throw new RuntimeException();}catch(Throwable){$recovery[]='ambiguous-create-recovery';}
        $cleanup=$owned['containerId']!==''?odciRelease($owned,$token):odciAttemptAll(['artifact-child'=>function()use($child,$childOwned):void{if($childOwned&&is_dir($child)&&!rmdir($child))throw new RuntimeException();}]);$decision=odciFailureDecision($error,array_merge($recovery,$cleanup));throw new TestFailure($decision['message']);
    }
}
...
function odciRelease(array $owned,string $token): array
{
    ...
    return odciAttemptAll([
        ...
        'container-remove'=>function()use($containerId,&$authorized):void{if(!$authorized||$containerId==='')throw new RuntimeException();if(odciDocker(['rm','-f',$containerId])['status']!==0)throw new RuntimeException();},
        'container-absence'=>function()use($container):void{if(!odciExactContainerAbsent($container))throw new RuntimeException();},
...
function odciAcquisitionFaultProbes(string $artifactRoot,string $decoyPath): void
{
    $decoy=hash_file('sha256',$decoyPath);
    foreach(['after-child','after-create-response-loss'] as $fault){$token=bin2hex(random_bytes(6));$failed=false;try{odciAcquire($token,$artifactRoot,$fault);}catch(TestFailure $error){$failed=str_contains($error->getMessage(),'SETUP_FAILURE: injected');}assertSameValue(true,$failed,'live acquisition fault is observed: '.$fault);assertSameValue(false,is_dir($artifactRoot.'/object-detail-'.$token),'acquisition fault removes exact owned child: '.$fault);assertSameValue(true,odciExactContainerAbsent('fm2-odci-'.$token),'acquisition fault leaves exact container absent: '.$fault);assertSameValue($decoy,hash_file('sha256',$decoyPath),'acquisition fault preserves ambient decoy: '.$fault);}
}
...
try {
    ...
    odciAcquisitionFaultProbes($artifactRoot,$decoyPath);
