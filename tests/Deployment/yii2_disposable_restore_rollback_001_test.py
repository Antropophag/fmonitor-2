...
class Rollback(unittest.TestCase):
 ...
 def test_authorized_real_rollback(self):
  ...
  password=Path(auth["credential_files"]["database"]).read_text().strip();compose=["docker","compose","--project-name",auth["project"],"--file",auth["compose_file"]];prefix=auth["runtime"]["process_table_prefix"];db=subprocess.run([*compose,"exec","-T","-e","MYSQL_PWD","db","mariadb","-N","-B","-u",auth["database_user"],auth["database"],"-e",f"SELECT id,fact FROM {prefix}fm2_restore_rehearsal_probe ORDER BY id; SELECT status,idempotency_key FROM {prefix}fm2_jobs WHERE idempotency_key='restore-rehearsal-job'; SELECT status FROM {prefix}fm2_outbox_intents WHERE domain_event_id='restore-rehearsal-outbox' AND channel='rehearsal'"],env={**os.environ,"MYSQL_PWD":password},capture_output=True,text=True,timeout=60);self.assertEqual((0,"7\tknown-history\nready\trestore-rehearsal-job\npending"),(db.returncode,db.stdout.strip()));ready=subprocess.run(["curl","--fail","--silent","--show-error",auth["health"]["ready"]],capture_output=True,text=True,timeout=30);self.assertEqual(0,ready.returncode,ready.stderr)
  ...
  for volume,command in (("artifacts",f"test \"$(cat /state/{artifact_path}/rehearsal/original.pdf)\" = PDF-known && test \"$(stat -c %a /state/{artifact_path}/rehearsal/original.pdf)\" = 600"),("sessions",f"test -s /state/{session_path}/rehearsal-session")):
