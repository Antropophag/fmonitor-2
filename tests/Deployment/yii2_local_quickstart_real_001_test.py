...
try:
    ...
    query="SELECT CONCAT((SELECT COUNT(*) FROM fm2_fm2_pilot_users),'|',(SELECT COALESCE(MIN(user_id),0) FROM fm2_fm2_pilot_users),'|',(SELECT COUNT(*) FROM information_schema.tables WHERE table_schema=DATABASE()))"
    ...
    subprocess.run([*compose,'exec','-T','php','sh','-c','printf preserved > /home/fmonitor/.local/state/fmonitor2/quickstart-real-sentinel'],cwd=root,check=True)
    ...
    sentinel=subprocess.check_output([*compose,'exec','-T','php','cat','/home/fmonitor/.local/state/fmonitor2/quickstart-real-sentinel'],cwd=root,text=True)
    assert before and after and before_db==after_db and sentinel=='preserved','REAL_REPLAY_MUTATED_STATE'
    ...
    assert subprocess.check_output([*compose,'exec','-T','php','cat','/home/fmonitor/.local/state/fmonitor2/quickstart-real-sentinel'],cwd=root,text=True)=='preserved','DOWN_REMOVED_STATE'
finally:
