...
try:
    ...
    subprocess.run([*compose,'exec','-T','db','sh','-c','MYSQL_PWD="$MARIADB_ROOT_PASSWORD" exec mariadb -uroot "$MARIADB_DATABASE" -e "CREATE TABLE quickstart_acceptance_sentinel(marker VARCHAR(64) NOT NULL); INSERT INTO quickstart_acceptance_sentinel VALUES (\"domain-preserved-001\")"'],cwd=root,check=True)
