...
with tempfile.TemporaryDirectory() as raw:
    ...
    bitrix=private/'bitrix-workforce.json';bitrix.write_text('{"baseUrl":"https://example.invalid/rest/7/SYNTHETIC","departments":[71],"documentBaseUrl":"https://example.invalid/rest/8/DOCUMENT_SYNTHETIC"}');bitrix.chmod(0o600)
    for relative in ('app/autoload.php','app/Jobs/SmtpConfiguration.php'):
        target=checkout/relative;target.parent.mkdir(parents=True,exist_ok=True);shutil.copy2(root/relative,target)
    values={'COMPOSE_PROJECT_NAME':'fm2-local-contract','FMONITOR_RUNTIME_IMAGE':'fmonitor2-runtime:contract','FMONITOR_HTTP_PORT':'18093','FMONITOR_DB_NAME':'fmonitor2','FMONITOR_DB_USER':'fmonitor_runtime','FMONITOR_DB_PASSWORD':'db-secret-contract','FMONITOR_MIGRATION_DB_USER':'root','FMONITOR_MIGRATION_DB_PASSWORD':'migration-secret-contract','FMONITOR_PROCESS_TABLE_PREFIX':'fm2_','FMONITOR_LEGACY_TABLE_PREFIX':'fm2_','FMONITOR_SESSION_INSTANCE':'local-contract','FMONITOR_YII_COOKIE_VALIDATION_KEY':'c'*32,'FMONITOR_YII_IDENTITY_KEY':'i'*32,'FMONITOR_TRUSTED_REQUEST_HOST':'127.0.0.1:18093','FMONITOR_TRUSTED_REQUEST_SCHEME':'http','FMONITOR_INITIAL_OWNER_EMAIL':'owner@example.test','FMONITOR_BOOTSTRAP_SUPERADMIN_PASSWORD':'owner-secret-contract','FMONITOR_RUNTIME_ENV':'development','FMONITOR_PUBLIC_BASE_URL':'https://fmonitor.example.invalid','FMONITOR_SMTP_HOST':'smtp.example.invalid','FMONITOR_SMTP_PORT':'587','FMONITOR_SMTP_ENCRYPTION':'tls','FMONITOR_SMTP_USERNAME':'fmonitor@example.invalid','FMONITOR_SMTP_PASSWORD':'smtp-secret-contract','FMONITOR_SMTP_FROM_ADDRESS':'fmonitor@example.invalid','FMONITOR_SMTP_FROM_NAME':'FMonitor','FMONITOR_SMTP_TIMEOUT_SECONDS':'10','FMONITOR_SMTP_VERIFY_PEER':'true','FMONITOR_ERP_HOST':'erp.example.invalid','FMONITOR_ERP_DATABASE':'legacy-stage','FMONITOR_ERP_USER':'reader','FMONITOR_ERP_PASSWORD':'erp-secret-contract','FMONITOR_ERP_EQUIPMENT_FACTS_HMAC_KEY':'h'*40,'FMONITOR_ERP_EQUIPMENT_FACTS_MAX_ROWS':'500','FMONITOR_ERP_EQUIPMENT_FACTS_TIMEOUT_SECONDS':'5','FMONITOR_ERP_EQUIPMENT_FACTS_CHUNK_SIZE':'100','FMONITOR_BITRIX_ORDER_DOCUMENT_ROOT_ID':'1809812','FMONITOR_BITRIX_ORDER_DOCUMENT_WEBHOOK_URL':'https://example.invalid/rest/8/DOCUMENT_SYNTHETIC/'}
...
''');curl.chmod(0o755)
    ...
    for secret in ('db-secret-contract','migration-secret-contract','owner-secret-contract','smtp-secret-contract','hostile-secret'):assert secret not in combined,('SECRET_EXPOSED',secret)
    ...
    marker=sandbox/'ENV_MUST_NOT_EXECUTE';literal=f'$(shell touch {marker})';write_env({'FMONITOR_DB_PASSWORD':literal});clear();literal_run=make('up');assert literal_run.returncode==0 and not marker.exists(),'DOTENV_EXECUTED_AS_MAKE';literal_observation=literal_run.stdout+literal_run.stderr+trace.read_text();assert literal not in literal_observation and str(marker) not in literal_observation,'DOTENV_SECRET_DISCLOSED';assert events()[0]['dbPasswordDigest']==hashlib.sha256(literal.encode()).hexdigest(),'DOTENV_VALUE_CHANGED';write_env()
    ...
    for changes in invalid_cases:
        current={k:v for k,v in {**values,**changes}.items() if v is not None};(checkout/'.env').write_text(''.join(f'{k}={v}\n' for k,v in current.items()));clear();rejected=make('up');assert rejected.returncode!=0,('INVALID_ACCEPTED',changes);assert events()==[],('INVALID_EFFECT',changes);assert 'LOCAL_CONFIG_INVALID' in rejected.stdout+rejected.stderr,('INVALID_REASON_MASKED',changes);assert 'replace_with_a_strong_unique_password' not in rejected.stdout+rejected.stderr
