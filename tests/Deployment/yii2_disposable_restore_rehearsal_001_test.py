...
class Rehearsal(unittest.TestCase):
 ...
 def test_authorized_real_roundtrip(self):
  ...
  artifact=subprocess.run(["docker","run","--rm","--network","none","--mount",f"type=volume,source={auth['volumes']['artifacts']['name']},target=/state,readonly",auth["image"],"sh","-c","test \"$(cat /state/fmonitor2/artifacts/rehearsal/original.pdf)\" = PDF-known && test \"$(stat -c %a /state/fmonitor2/artifacts/rehearsal/original.pdf)\" = 600"],capture_output=True,text=True,timeout=60);self.assertEqual(0,artifact.returncode,artifact.stderr)
  session=subprocess.run(["docker","run","--rm","--network","none","--mount",f"type=volume,source={auth['volumes']['sessions']['name']},target=/state,readonly",auth["image"],"sh","-c","test -s /state/fmonitor2/sessions/rehearsal-session"],capture_output=True,text=True,timeout=60);self.assertEqual(0,session.returncode,session.stderr)
  jobs=subprocess.run([*compose,"exec","-T","-e","MYSQL_PWD","db","mariadb","-N","-B","-u",auth["database_user"],auth["database"],"-e","SELECT state,deduplication_key FROM fm2_jobs WHERE deduplication_key='restore-rehearsal-job'; SELECT state FROM fm2_outbox WHERE idempotency_key='restore-rehearsal-outbox'"],env={**os.environ,"MYSQL_PWD":password},capture_output=True,text=True,timeout=60);self.assertEqual((0,"queued\trestore-rehearsal-job\npending"),(jobs.returncode,jobs.stdout.strip()))
