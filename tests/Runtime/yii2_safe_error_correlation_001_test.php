...
try{
    $base=array_replace(getenv(),$fixture->environment(),['FMONITOR_DB_PASSWORD'=>SRC_CANARY,'HTTP_COOKIE'=>SRC_CANARY]);
    ...
    $token=$fixture->token($cookies);$conflictBody=json_encode(['type'=>'item_completed','itemId'=>42,'clientOperationId'=>'173-conflict'],JSON_THROW_ON_ERROR);$conflict=$fixture->request('POST','/pilot/objects/4512/checklist/operations',[],$cookies,['Content-Type: application/json; charset=UTF-8','X-FM2-CSRF: '.$token],$conflictBody);srcNoFailureRecord($before,$yiiLog,$conflict,409,"{\"status\":\"rejected\",\"message\":\"Последние 15% закрываются актом ПТО и декларацией в карточке объекта.\"}",'409 domain');
    $unsupported=$fixture->request('POST','/pilot/objects/4512/assignment-orders/81/originals',[],$cookies,['Content-Type: text/plain'],SRC_CANARY);srcNoFailureRecord($before,$yiiLog,$unsupported,415,"{\"error\":\"UNSUPPORTED_MEDIA_TYPE\"}\n",'415 media');
    $tooLarge=$fixture->wire('POST','/pilot/objects/4512/assignment-orders/81/originals',['Content-Type: application/pdf','Content-Length: 20971521'],'',$cookies);[$rawHead,$rawBody]=array_pad(explode("\r\n\r\n",$tooLarge['reply'],2),2,'');assertSameValue([413,true,false],[$tooLarge['status'],str_contains($rawBody,'{"error":"REQUEST_TOO_LARGE"}'),preg_match('/^X-FMonitor-Error-ID:/mi',$rawHead)===1],'413 exact body/no ID');assertSameValue(true,preg_match('/^Content-Type: application\/json; charset=UTF-8$/mi',$rawHead)===1,'413 content type');assertSameValue(srcFailureCount($before),srcFailureCount((string)file_get_contents($yiiLog)),'413 no diagnostic record');
    ...
    $fixture->start([],srcControllerRouter($root,SRC_CANARY,'unexpected'),true);$execution=$fixture->request('GET','/pilot/objects/4512/execution?query='.SRC_CANARY,[],$cookies);srcControllerExpectation($execution,$fixture->artifacts.'/server.log','execution_controller','unexpected','execution','html');
    $fixture->start([],srcControllerRouter($root,SRC_CANARY,'storage'),true);$metadata=$fixture->metadata($cookies);$metadata['originalFilename']='secret-'.SRC_CANARY.'.pdf';$original=$fixture->upload($cookies,$metadata,"%PDF-1.4\n".SRC_CANARY);srcControllerExpectation($original,$fixture->artifacts.'/server.log','original_controller','storage','original','json');
    $fixture->start([],srcControllerRouter($root,SRC_CANARY,'unexpected'),true);$checklist=$fixture->request('GET','/pilot/objects/4512/checklist',[],$cookies);srcControllerExpectation($checklist,$fixture->artifacts.'/server.log','checklist_controller','unexpected','checklist','plain');
    $marker=$fixture->artifacts.'/command-attempts.log';$facts=$fixture->facts();$open=['_csrf'=>$fixture->token($cookies),'action'=>'open_confirmed','requestId'=>'33333333-3333-4333-8333-000000000173','orderId'=>'81','revisionId'=>$accepted->currentRevisionId(),'sequence'=>'0','actualStartDate'=>'2026-09-02'];$fixture->start([],srcControllerRouter($root,SRC_CANARY,'unexpected',$marker,true),true);$failedCommand=$fixture->form('/pilot/objects/4512/execution',$open,$cookies);srcControllerExpectation($failedCommand,$fixture->artifacts.'/server.log','execution_controller','unexpected','broken sink command','html',false);
    assertSameValue("attempt\n",is_file($marker)?file_get_contents($marker):'','failed command boundary invoked once');assertSameValue($facts,$fixture->facts(),'failed logger/command preserves all domain facts and history');
    $fixture->start([],srcControllerRouter($root,SRC_CANARY,'sql'),true);$sqlCookies=[];$login=$fixture->request('GET','/pilot/login',[],$sqlCookies);$sql=$fixture->request('POST','/pilot/login',['_csrf'=>$fixture->csrf($login['body']),'email'=>'sql-'.SRC_CANARY.'@example.test','password'=>SRC_CANARY],$sqlCookies);assertSameValue(503,$sql['status'],'SQL parameter global failure');$sqlId=srcId($sql,'SQL parameter global failure');$sqlRecord=srcRecord((string)file_get_contents($fixture->artifacts.'/server.log'),$sqlId,'SQL parameter global failure');assertSameValue('yii_error_handler',$sqlRecord['component'],'SQL parameter global component');assertSameValue(false,str_contains(json_encode($sql,JSON_THROW_ON_ERROR).json_encode($sqlRecord,JSON_THROW_ON_ERROR),SRC_CANARY),'SQL/request-body canary private');
...
{
    assertSameValue(503,$response['status'],$label.' status');srcSecurity($response,$label);if($envelope==='html'){$semantic=[preg_match('/<title>Сервис временно недоступен · FMonitor 2\.0<\/title>/u',$response['body']),substr_count($response['body'],'Результат операции неизвестен. Проверьте карточку объекта перед повтором.'),preg_match('#href="/pilot/objects/4512"[^>]*>Вернуться к карточке<#u',$response['body']),substr_count($response['body'],'Повторите действие после проверки актуального состояния.')];assertSameValue([1,1,1,1],$semantic,$label.' complete error-page semantics');}elseif($envelope==='json')assertSameValue("{\"error\":\"SERVICE_UNAVAILABLE\"}\n",$response['body'],$label.' JSON body');else assertSameValue("Service unavailable.\n",$response['body'],$label.' plain body');$id=srcId($response,$label);usleep(100000);$bytes=(string)file_get_contents($log);if(!$expectRecord){assertSameValue(0,substr_count($bytes,'"errorId":"'.$id.'"'),$label.' failed sink writes no record');return;}$record=srcRecord($bytes,$id,$label);
