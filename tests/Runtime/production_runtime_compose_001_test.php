...
try {
    ...
    $sequentialStarted=microtime(true);for($probe=0;$probe<10;$probe++)assertSameValue(200,runtimeHttp($port,'/health/ready')['status'],'sequential measured readiness');$sequentialSeconds=microtime(true)-$sequentialStarted;
    $parallelStarted=microtime(true);$children=[];for($probe=0;$probe<4;$probe++){if(($pid=pcntl_fork())===0)exit(runtimeHttp($port,'/health/ready')['status']===200?0:1);$children[]=$pid;}foreach($children as$pid){pcntl_waitpid($pid,$status);assertSameValue(0,pcntl_wexitstatus($status),'parallel measured readiness');}$parallelSeconds=microtime(true)-$parallelStarted;
    $measurement=['method'=>'exact candidate nginx/FPM Compose; global counters include sampling and background healthchecks','controlDelta'=>runtimeCounterDelta($controlBefore,$controlAfter),'singleDelta'=>runtimeCounterDelta($singleBefore,$singleAfter),'singleSeconds'=>$singleSeconds,'sequential10Seconds'=>$sequentialSeconds,'parallel4Seconds'=>$parallelSeconds];
