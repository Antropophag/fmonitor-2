...
try {
    ...
    assertSameValue(range(1, 28), array_keys($catalogue), 'canonical production catalogue has a contiguous v28 frontier');
    $v22 = $catalogue[22];
    ...
    assertSameValue([0, true, 28, range(1, 28)], [$first['exitCode'], $first['result']['ok'] ?? null, $first['result']['schemaVersion'] ?? null, $first['result']['appliedVersions'] ?? null], 'clean production migration creates the full runtime schema');
    ...
    assertSameValue([0, true, 28, []], [$repeat['exitCode'], $repeat['result']['ok'] ?? null, $repeat['result']['schemaVersion'] ?? null, $repeat['result']['appliedVersions'] ?? null], 'production migration replay is a no-op at v28');
