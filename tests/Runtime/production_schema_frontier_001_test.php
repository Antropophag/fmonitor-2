...
try {
    ...
    assertSameValue(range(1, 24), array_keys($catalogue), 'INTENTIONAL_RED: canonical production catalogue has a contiguous v24 frontier');
    $v22 = $catalogue[22];
    ...
    assertSameValue([0, true, 24, range(1, 24)], [$first['exitCode'], $first['result']['ok'] ?? null, $first['result']['schemaVersion'] ?? null, $first['result']['appliedVersions'] ?? null], 'clean production migration creates the full runtime schema');
    ...
    assertSameValue([0, true, 24, []], [$repeat['exitCode'], $repeat['result']['ok'] ?? null, $repeat['result']['schemaVersion'] ?? null, $repeat['result']['appliedVersions'] ?? null], 'production migration replay is a no-op at v24');
