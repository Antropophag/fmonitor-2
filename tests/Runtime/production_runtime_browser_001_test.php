...
$environment = array_replace($processEnvironment, [
    ...
    'FMONITOR_YII_COOKIE_VALIDATION_KEY' => str_repeat('production-browser-cookie-', 2),
    'FMONITOR_YII_IDENTITY_KEY' => str_repeat('production-browser-identity-', 2),
...
function prbSeed(SelectedOriginalFixture $fixture,string $password):void
{
    ...
    $expectedCurrent=CurrentProductionSchemaContract::cleanApplicationResult();assertSameValue([$expectedCurrent[0],$expectedCurrent[1],$expectedCurrent[2],true],[$migration['exitCode'],$migration['result']['ok']??null,$migration['result']['schemaVersion']??null,InspectionPhotoContentIndexSchemaMigration::isCompleteCompatible($db,$p)],'fixture canonical current schema');
...
try{
    ...
    $factsBefore=$facts();
    $stateBefore=prbCommand([...$base,'--profile','deployment','run','--rm','--entrypoint','sh','prepare','-c','find /home/fmonitor/.local/state/fmonitor2/yii-sessions -type f -name "sess_*" -exec sha256sum {} + | sort; find /home/fmonitor/.local/state/fmonitor2/artifacts -type f -exec sha256sum {} + | sort'],$environment,$repo,30);prbOk($stateBefore,'persistent state snapshot');
    $ownerSession=prbCommand([...$base,'--profile','deployment','run','--rm','--entrypoint','sh','prepare','-c','for f in /home/fmonitor/.local/state/fmonitor2/yii-sessions/sess_*; do grep -aq "__id|i:18;" "$f" && basename "$f" | sed -e "s/^sess_//" && exit 0; done; exit 1'],$environment,$repo,30);prbOk($ownerSession,'owner Yii session identity');$ownerSessionId=trim($ownerSession['stdout']);
    $ownerHashBefore=prbCommand([...$base,'--profile','deployment','run','--rm','--entrypoint','sha256sum','prepare','/home/fmonitor/.local/state/fmonitor2/yii-sessions/sess_'.$ownerSessionId],$environment,$repo,30);prbOk($ownerHashBefore,'owner session pre-restart hash');
    prbOk(prbCommand([...$base,'restart','php','web'],$environment,$repo,90),'production process restart');
    $stateRestarted=prbCommand([...$base,'--profile','deployment','run','--rm','--entrypoint','sh','prepare','-c','find /home/fmonitor/.local/state/fmonitor2/yii-sessions -type f -name "sess_*" -exec sha256sum {} + | sort; find /home/fmonitor/.local/state/fmonitor2/artifacts -type f -exec sha256sum {} + | sort'],$environment,$repo,30);prbOk($stateRestarted,'immediate post-restart state snapshot');assertSameValue($stateBefore['stdout'],$stateRestarted['stdout'],'process restart preserves session and private artifact bytes exactly');
    ...
    $ownerHashAfter=prbCommand([...$base,'--profile','deployment','run','--rm','--entrypoint','sha256sum','prepare','/home/fmonitor/.local/state/fmonitor2/yii-sessions/sess_'.$ownerSessionId],$environment,$repo,30);prbOk($ownerHashAfter,'owner session post-read hash');assertSameValue($ownerHashBefore['stdout'],$ownerHashAfter['stdout'],'authorized restart read preserves exact committed owner session bytes');
    ...
    $factsAfter=$facts();
    $stateAfter=prbCommand([...$base,'--profile','deployment','run','--rm','--entrypoint','sh','prepare','-c','find /home/fmonitor/.local/state/fmonitor2/yii-sessions -type f -name "sess_*" -exec sha256sum {} + | sort; find /home/fmonitor/.local/state/fmonitor2/artifacts -type f -exec sha256sum {} + | sort'],$environment,$repo,30);prbOk($stateAfter,'post-restart state snapshot');
    ...
    // OTIZ consumes the applied native 4512 case completed through the browser above.
    // The pre-backfill 4520 order remains historical setup, without a synthetic eligibility projection.
    $reportDate=(new DateTimeImmutable('now',new DateTimeZone('Europe/Moscow')))->format('Y-m-d');$otizResult=$artifacts.'/otiz-result.json';$otizBrowser=prbCommand([$node,$repo.'/tests/Otiz/snapshot_publication_browser_001_test.mjs',(string)$httpPort,$playwright,$artifacts,$otizResult,$reportDate],$environment,$repo,90);$otizLogs=$otizBrowser['exit']===0?'':prbCommand([...$base,'logs','--no-color','--tail','80','php','web'],$environment,$repo,30)['stdout'];assertSameValue(0,$otizBrowser['exit'],'OTIZ calculate/accept/XLSX through production runtime '.(is_file($otizResult)?file_get_contents($otizResult):$otizBrowser['stderr']).' runtime='.preg_replace('/[0-9a-f]{12,}/i','<ID>',$otizLogs));
