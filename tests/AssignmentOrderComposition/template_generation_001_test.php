...
$tests=[
    'today repeat failure and direct original'=>static function(F $f):void {
        ...
        $r=$app->generateAssignmentOrderTemplate(4512,81,18);templateSuccess($r,'2026-09-06');assertSameValue(1,$f->clockCalls,'one clock read for first attempt');F::pdfMarker($r['bytes'],'сентября');F::pdfMarker($r['bytes'],'06');
        ...
        $f->at='2026-09-06T21:30:00Z';$r=$app->generateAssignmentOrderTemplate(4512,81,18);templateSuccess($r,'2026-09-07');assertSameValue(2,$f->clockCalls,'one clock read for repeated attempt');F::pdfMarker($r['bytes'],'07');$after2=$f->original->selection->rows();templateOnlyAudit($before,$after2);assertSameValue(2,count($after2['fm2_process_events']),'repeat creates a new audit, not a PDF version');assertSameValue($event,$after2['fm2_process_events'][0],'prior audit immutable');assertSameValue(['status'=>'found','date'=>'2026-09-07'],$dates->find(4512,81),'next-day date replaces prefill projection');assertSameValue(2,count($f->inputs),'real renderer called again');
        $f->at='2026-09-07T21:30:00Z';$f->renderFails=true;$failure=$app->generateAssignmentOrderTemplate(4512,81,18);assertSameValue(3,$f->clockCalls,'one clock read before failed render');
