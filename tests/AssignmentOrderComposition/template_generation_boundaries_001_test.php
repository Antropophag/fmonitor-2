...
$tests=[];
foreach(['invalid','denied','missing','other_case','stale','completed','pto','missing_object_date','malformed_adjusted'] as $axis)$tests[$axis]=static function(F $f)use($axis):void {
    ...
    if($axis==='missing_object_date')$db->query('UPDATE fm_maintable SET plan_finish_date=NULL');
    if($axis==='malformed_adjusted')$db->query("UPDATE fm_maintable SET workdateendadjusted='not-a-date'");
    ...
    templateRefusal($r,$status,$reason);assertSameValue($before,$f->original->selection->rows(),'refusal never creates generation/domain facts');assertSameValue([], $f->inputs,'refusal before renderer');assertSameValue([], $f->original->privateFiles(),'refusal stores no PDF');
};
foreach(['empty','multiple','extra','wrong_type','bad_bytes'] as $axis)$tests['renderer_'.$axis]=static function(F $f)use($axis):void {
    ...
    $app=C\AssignmentOrderTemplateVerificationFactory::create($f->original->selection->db,'',$f,$render);templateRefusal($app->generateAssignmentOrderTemplate(4512,81,18),'failed','render_failure');assertSameValue($before,$f->original->selection->rows(),'invalid renderer response not audited as success');
};
$tests['last event ID and matching event integrity']=static function(F $f):void {
    ...
    $dates=$f->dates();assertSameValue(['status'=>'found','date'=>'2026-09-06'],$dates->find(4512,81),'greatest event ID, not MAX(date)');
...
$tests['adjusted finish takes precedence']=static function(F $f):void {
    $f->original->selection->db->query("UPDATE fm_maintable SET workdateendadjusted='2026-12-15'");assertSameValue('generated',$f->app()->generateAssignmentOrderTemplate(4512,81,18)['status'],'valid adjusted date');assertSameValue('2026-12-15',$f->inputs[0]['installationObjectSnapshot']['plannedFinishDate'],'existing adapter precedence');
};
$tests['audit SQL failure rolls back']=static function(F $f):void {
    $f->original->selection->db->query('RENAME TABLE fm2_process_events TO fixture_missing_events');$before=$f->original->selection->rows();templateRefusal($f->app()->generateAssignmentOrderTemplate(4512,81,18),'failed','persistence_failure');assertSameValue(1,count($f->inputs),'real render before failed audit');assertSameValue($before,$f->original->selection->rows(),'confirmed rollback changes nothing');
};
$tests['event capacity no acknowledged PDF']=static function(F $f):void {
    $f->original->selection->db->query('ALTER TABLE fm2_process_events AUTO_INCREMENT=9223372036854775808');$before=$f->original->selection->rows();templateRefusal($f->app()->generateAssignmentOrderTemplate(4512,81,18),'failed','allocation_capacity_exhausted');assertSameValue($before,$f->original->selection->rows(),'overflow rollback no business facts');
};
...
$tests['ambient transaction is not taken over']=static function(F $f):void {
    ...
    try{$db->query("INSERT INTO other_prefix_marker VALUES(2,'caller-owned')");templateRefusal($app->generateAssignmentOrderTemplate(4512,81,18),'failed','dependency_unavailable');assertSameValue(['status'=>'unavailable','date'=>null],$dates->find(4512,81),'date reader rejects ambient snapshot');assertSameValue('1',(string)$db->query('SELECT @@in_transaction')->fetch_row()[0],'caller transaction retained');}finally{$db->rollback();}assertSameValue($before,$f->original->selection->rows(),'caller controls rollback');
};
