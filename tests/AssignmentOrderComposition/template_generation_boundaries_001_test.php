...
foreach(['invalid','denied','missing','other_case','stale','completed','pto','missing_object_date','malformed_adjusted'] as $axis)$tests[$axis]=static function(F $f)use($axis):void {
    ...
    templateRefusal($r,$status,$reason);assertSameValue(in_array($axis,['missing_object_date','malformed_adjusted'],true)?1:0,$f->clockCalls,'clock ordering follows source phase');assertSameValue($before,$f->original->selection->rows(),'refusal never creates generation/domain facts');assertSameValue([], $f->inputs,'refusal before renderer');assertSameValue([], $f->original->privateFiles(),'refusal stores no PDF');
};
