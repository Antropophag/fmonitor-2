...
function pdbCookie(string $header):array{$tokens=explode(';',$header);$pair=trim((string)array_shift($tokens));if(preg_match('/^([^=]+)=([^=]+)$/D',$pair,$match)!==1)throw new TestFailure('one exact cookie name/value pair');$attributes=[];foreach($tokens as$token){$token=trim($token);if($token===''||substr_count($token,'=')>1)throw new TestFailure('canonical cookie attribute token');[$name,$value]=array_pad(explode('=',$token,2),2,null);$key=strtolower($name);if(array_key_exists($key,$attributes))throw new TestFailure('duplicate cookie attribute '.$name);$attributes[$key]=$value;}ksort($attributes);return ['name'=>$match[1],'value'=>$match[2],'attributes'=>$attributes];}
...
function pdbRouterStart(array $environment,int $port):array{$command=['/usr/bin/env','-i'];foreach($environment as$name=>$value)$command[]=$name.'='.$value;$command=[...$command,PHP_BINARY,'-d','expose_php=0','-S',"127.0.0.1:$port",dirname(__DIR__,2).'/public/router.php'];$pipes=[];$process=proc_open($command,[0=>['pipe','r'],1=>['pipe','w'],2=>['pipe','w']],$pipes,dirname(__DIR__,2));if(!is_resource($process))throw new TestFailure('normal router starts');fclose($pipes[0]);stream_set_blocking($pipes[1],false);stream_set_blocking($pipes[2],false);$deadline=microtime(true)+5;do{$socket=@stream_socket_client("tcp://127.0.0.1:$port",$errno,$error,.1);if($socket!==false){fclose($socket);return compact('process','pipes','port');}usleep(20000);}while(microtime(true)<$deadline);pdbStop(compact('process','pipes','port'));throw new TestFailure('normal router listens');}
...
try {
    ...
    $legacyPrefix='fm2l_'.$fingerprint.'_g1_';assertSameValue([['id'=>'4512','regnumber'=>'77-000123','ordadr_address'=>'Москва, ул. Примерная, д. 10','entrance'=>'2','workdatestart'=>'2026-10-05','workdateendadjusted'=>'2026-12-20','responsstroicontrol'=>'73'],['id'=>'4999','regnumber'=>'77-000999','ordadr_address'=>'Москва, ул. Непилотная, д. 1','entrance'=>'1','workdatestart'=>'2026-09-30','workdateendadjusted'=>'2026-12-01','responsstroicontrol'=>'73']],pdbRows($db,"SELECT id,regnumber,ordadr_address,entrance,workdatestart,workdateendadjusted,responsstroicontrol FROM `{$legacyPrefix}fm_maintable` ORDER BY id"),'exact legacy-shaped fixture contents');assertSameValue([['id'=>'18','name'=>'Сидоров Сергей Сергеевич','email'=>'sidorov@shlz.ru','role_id'=>'5','status'=>'1'],['id'=>'73','name'=>'Анна Волкова','email'=>'volkova@shlz.ru','role_id'=>'8','status'=>'1']],pdbRows($db,"SELECT * FROM `{$legacyPrefix}users` ORDER BY id"),'exact legacy user fixture');assertSameValue([['installer_tab_id'=>'1042','fio'=>'Иванов Иван Иванович','employment_status'=>'employed','workforce_source_updated_at'=>'2026-08-27T18:15:00+03:00'],['installer_tab_id'=>'2088','fio'=>'Петров Пётр Петрович','employment_status'=>'employed','workforce_source_updated_at'=>'2026-08-27T18:15:00+03:00']],pdbRows($db,"SELECT installer_tab_id,fio,employment_status,workforce_source_updated_at FROM `{$prefix}fm2_workforce_catalog` ORDER BY installer_tab_id"),'exact deployment workforce fixture');assertSameValue([['user_id'=>'18','capability'=>'assignment_order.confirm_registration','position_snapshot'=>null],['user_id'=>'18','capability'=>'assignment_order.prepare','position_snapshot'=>null],['user_id'=>'18','capability'=>'installation.open','position_snapshot'=>null],['user_id'=>'73','capability'=>'construction_control_engineer','position_snapshot'=>'Инженер строительного контроля']],pdbRows($db,"SELECT * FROM `{$prefix}fm2_process_user_capabilities` ORDER BY user_id,capability"),'exact deployment capability fixture');$artifactRoot=$home.'/.local/state/fmonitor2/pilot-demo/'.$fingerprint.'/generations/1/artifacts';$artifactInfo=lstat($artifactRoot);assertSameValue(true,$artifactInfo!==false&&!is_link($artifactRoot)&&is_dir($artifactRoot),'artifact root is real directory');assertSameValue(posix_geteuid(),$artifactInfo['uid']??null,'artifact root effective owner');assertSameValue(0755,($artifactInfo['mode']??0)&0777,'artifact root exact secure deployment mode');

    pdbStop($server);$server=null;$server=pdbStart($environment);$restartQueue=pdbRequest($port,'GET','/pilot/objects');assertSameValue($queue['body'],$restartQueue['body'],'initial restart preserves exact queue projection');$restartCard=pdbRequest($port,'GET','/pilot/objects/4512');foreach(['Требуется распоряжение','77-000123','Москва, ул. Примерная, д. 10','2026-10-05','2026-12-20']as$text)pdbText($restartCard,$text,'full initial restart card');$restartForm=pdbRequest($port,'GET','/pilot/objects/4512/assignment-order/prepare');$restartXpath=pdbXpath($restartForm['body']);assertSameValue([1,1,1,1],[(int)$restartXpath->evaluate("count(//input[@name='installerTabIds[]' and @value='1042'])"),(int)$restartXpath->evaluate("count(//input[@name='installerTabIds[]' and @value='2088'])"),(int)$restartXpath->evaluate("count(//input[@name='controlEngineerUserId' and @value='73' and @checked])"),(int)$restartXpath->evaluate("count(//button[@type='submit' and not(@disabled)] | //input[@type='submit' and not(@disabled)])")],'initial restart preserves complete actionable form');
    $normalPort=pdbPort();$account=posix_getpwuid(posix_geteuid());$normalEnvironment=['HOME'=>(string)($account['dir']??''),'PATH'=>'/usr/bin:/bin','REMOTE_USER'=>'sidorov@shlz.ru','FMONITOR_DB_HOST'=>$adminHost,'FMONITOR_DB_PORT'=>(string)$adminPort,'FMONITOR_DB_NAME'=>$database,'FMONITOR_DB_USER'=>$demoUser,'FMONITOR_DB_PASSWORD'=>$demoPassword,'FMONITOR_PROCESS_TABLE_PREFIX'=>$prefix,'FMONITOR_LEGACY_TABLE_PREFIX'=>$legacyPrefix,'FMONITOR_ARTIFACT_STORAGE_ROOT'=>$artifactRoot,'FMONITOR_SHLZ_CSS_PATH'=>dirname(__DIR__,3).'/shlz-ui/packages/styles/shlz.css','FMONITOR_PILOT_CSS_PATH'=>dirname(__DIR__,2).'/app/PilotHttp/pilot.css','FMONITOR_NOW'=>'2026-08-29T12:00:00+03:00','FMONITOR_TRUSTED_REQUEST_HOST'=>"127.0.0.1:$normalPort"];$normal=pdbRouterStart($normalEnvironment,$normalPort);try{$normalForm=pdbRequest($normalPort,'GET','/pilot/objects/4512/assignment-order/prepare');$normalXpath=pdbXpath($normalForm['body']);$normalSetCookie=(string)pdbHeader($normalForm,'set-cookie');$normalParsed=pdbCookie($normalSetCookie);assertSameValue('fm2pilot',$normalParsed['name'],'ordinary production exact cookie name');assertSameValue(1,preg_match('/^[A-Za-z0-9,-]{16,128}$/D',$normalParsed['value']),'ordinary production exact opaque cookie value grammar');assertSameValue(['httponly'=>null,'path'=>'/pilot','samesite'=>'Strict','secure'=>null],$normalParsed['attributes'],'ordinary production exact secure cookie attribute set');$normalCookie='fm2pilot='.$normalParsed['value'];$normalBody=http_build_query(['csrfToken'=>pdbValue($normalXpath,'csrfToken'),'processRevision'=>pdbValue($normalXpath,'processRevision'),'installerTabIds'=>['1042'],'controlEngineerUserId'=>'73','controlEngineerConfirmed'=>'yes'],'','&',PHP_QUERY_RFC3986);$normalNull=pdbRequest($normalPort,'POST','/pilot/objects/4512/assignment-order/prepare',['Cookie'=>$normalCookie,'Origin'=>'null','Sec-Fetch-Site'=>'same-origin','Content-Type'=>'application/x-www-form-urlencoded; charset=UTF-8','Content-Length'=>(string)strlen($normalBody)],$normalBody);assertSameValue([403,"Invalid request.\n"],[$normalNull['status'],$normalNull['body']],'ordinary production rejects literal null Origin');$normalPost=pdbRequest($normalPort,'POST','/pilot/objects/4512/assignment-order/prepare',['Cookie'=>$normalCookie,'Origin'=>"http://127.0.0.1:$normalPort",'Sec-Fetch-Site'=>'same-origin','Content-Type'=>'application/x-www-form-urlencoded; charset=UTF-8','Content-Length'=>(string)strlen($normalBody)],$normalBody);assertSameValue([403,"Invalid request.\n"],[$normalPost['status'],$normalPost['body']],'ordinary built-in/router composition remains HTTPS-origin-only');}finally{pdbStop($normal);}assertSameValue($restartQueue['body'],pdbRequest($port,'GET','/pilot/objects')['body'],'rejected ordinary Origin variants create no process change');
    ...
    $attackerHost='127.0.0.1:'.pdbPort();$attackerForm=pdbRequest($port,'GET',$preparePath,['Host'=>$attackerHost]);assertSameValue(200,$attackerForm['status'],'valid but noncanonical loopback Host reaches normal production policy');$attackerXpath=pdbXpath($attackerForm['body']);$attackerParsed=pdbCookie((string)pdbHeader($attackerForm,'set-cookie'));assertSameValue('fm2pilot',$attackerParsed['name'],'noncanonical Host exact cookie name');assertSameValue(1,preg_match('/^[A-Za-z0-9,-]{16,128}$/D',$attackerParsed['value']),'noncanonical Host opaque cookie value grammar');assertSameValue(['httponly'=>null,'path'=>'/pilot','samesite'=>'Strict','secure'=>null],$attackerParsed['attributes'],'demo nonce never relaxes cookie for noncanonical Host');$attackerCookie='fm2pilot='.$attackerParsed['value'];$attackerBody=http_build_query(['csrfToken'=>pdbValue($attackerXpath,'csrfToken'),'processRevision'=>pdbValue($attackerXpath,'processRevision'),'installerTabIds'=>['1042'],'controlEngineerUserId'=>'73','controlEngineerConfirmed'=>'yes'],'','&',PHP_QUERY_RFC3986);$attackerNull=pdbRequest($port,'POST',$preparePath,['Host'=>$attackerHost,'Cookie'=>$attackerCookie,'Origin'=>'null','Sec-Fetch-Site'=>'same-origin','Content-Type'=>'application/x-www-form-urlencoded; charset=UTF-8','Content-Length'=>(string)strlen($attackerBody)],$attackerBody);assertSameValue([403,"Invalid request.\n"],[$attackerNull['status'],$attackerNull['body']],'noncanonical trusted Host rejects literal null Origin');$attackerPost=pdbRequest($port,'POST',$preparePath,['Host'=>$attackerHost,'Cookie'=>$attackerCookie,'Origin'=>'http://'.$attackerHost,'Sec-Fetch-Site'=>'same-origin','Content-Type'=>'application/x-www-form-urlencoded; charset=UTF-8','Content-Length'=>(string)strlen($attackerBody)],$attackerBody);assertSameValue([403,"Invalid request.\n"],[$attackerPost['status'],$attackerPost['body']],'matching attacker Origin cannot use trusted demo nonce on noncanonical Host');assertSameValue($restartQueue['body'],pdbRequest($port,'GET','/pilot/objects')['body'],'noncanonical Host rejection creates no process fact');
    $setCookie=pdbHeader($prepare,'set-cookie');if(!is_string($setCookie))throw new TestFailure('browser receives session cookie');$demoParsed=pdbCookie($setCookie);assertSameValue('fm2pilot',$demoParsed['name'],'trusted demo exact cookie name');assertSameValue(1,preg_match('/^[A-Za-z0-9,-]{16,128}$/D',$demoParsed['value']),'trusted demo exact opaque cookie value grammar');assertSameValue(['httponly'=>null,'path'=>'/pilot','samesite'=>'Strict'],$demoParsed['attributes'],'trusted demo exact browser-usable cookie attribute set');$cookie='fm2pilot='.$demoParsed['value'];$nullFields=['csrfToken'=>pdbValue($px,'csrfToken'),'processRevision'=>pdbValue($px,'processRevision'),'installerTabIds'=>['1042'],'controlEngineerUserId'=>'73'];$nullBody=http_build_query($nullFields,'','&',PHP_QUERY_RFC3986);$nullPost=pdbRequest($port,'POST',$preparePath,['Cookie'=>$cookie,'Origin'=>'null','Sec-Fetch-Site'=>'same-origin','Content-Type'=>'application/x-www-form-urlencoded; charset=UTF-8','Content-Length'=>(string)strlen($nullBody)],$nullBody);assertSameValue([303,$preparePath],[$nullPost['status'],pdbHeader($nullPost,'location')],'canonical trusted demo accepts literal null Origin through PRG');$prepare=pdbRequest($port,'GET',$preparePath,['Cookie'=>$cookie]);$px=pdbXpath($prepare['body']);
    ...
    $partial=$checkoutRoots[0].'/generations/2';if(!mkdir($partial.'/artifacts',0755,true))throw new TestFailure('interrupted inactive generation fixture');$partialOwner=['fingerprint'=>$fingerprint,'generation'=>2,'nonce'=>bin2hex(random_bytes(16)),'processPrefix'=>'fm2d_'.$fingerprint.'_g2_','legacyPrefix'=>'fm2l_'.$fingerprint.'_g2_'];file_put_contents($partial.'/owner.json',json_encode($partialOwner,JSON_UNESCAPED_SLASHES));chmod($partial.'/owner.json',0600);file_put_contents($partial.'/partial','INTERRUPTED-'.$token);$generationOneTree=pdbTree($stateRoot);$server=pdbStart($environment,'reset');
