...
try {
    ...
    $setCookie=pdbHeader($prepare,'set-cookie');if(!is_string($setCookie))throw new TestFailure('browser receives session cookie');assertSameValue(true,str_starts_with($setCookie,'fm2pilot='),'trusted demo emits named pilot session cookie');assertSameValue(false,str_contains($setCookie,'Secure'),'trusted loopback HTTP demo cookie omits Secure for real browser POST');assertSameValue(true,str_contains($setCookie,'HttpOnly')&&str_contains($setCookie,'SameSite=Strict')&&str_contains($setCookie,'Path=/pilot'),'trusted demo cookie retains non-transport protections');$cookie=explode(';',$setCookie,2)[0];
