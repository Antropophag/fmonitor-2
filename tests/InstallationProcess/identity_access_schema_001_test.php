...
try {
    ...
    assertSameValue(['ok' => true, 'schemaVersion' => 6, 'appliedVersions' => [1, 2, 3, 4, 5, 6]], iaJson($clean), 'Clean literal canonical result.');
    ...
    assertSameValue(['ok' => true, 'schemaVersion' => 6, 'appliedVersions' => []], iaJson($repeat), 'Complete repeat literal result.');
    ...
    assertSameValue(['ok' => true, 'schemaVersion' => 6, 'appliedVersions' => []], iaJson(iaRun($database, 'pop_')), 'Populated compatible result.');
    ...
    assertSameValue(['ok' => true, 'schemaVersion' => 6, 'appliedVersions' => [6]], iaJson(iaRun($database, 'partial_')), 'Exact-compatible partial literal result.');
    ...
    assertSameValue(['ok' => true, 'schemaVersion' => 6, 'appliedVersions' => []], iaJson(iaRun($database, 'partial_')), 'Interrupted recovery repeat is a no-op.');
    ...
    assertSameValue(['ok'=>true,'schemaVersion'=>6,'appliedVersions'=>[6]], iaJson(iaRun($database, 'deps_')), 'Dependency subset recovery literal result.');
    ...
    assertSameValue(['ok'=>true,'schemaVersion'=>6,'appliedVersions'=>[]], iaJson(iaRun($database, 'deps_')), 'Dependency recovery is restartable.');
