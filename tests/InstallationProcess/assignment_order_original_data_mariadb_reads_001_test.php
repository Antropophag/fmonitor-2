...
try{
    ...
    integrityCase('real-revision-owner-exact-and-historical',function()use($f,$repo){
        ...
        assertSameValue(O\AssignmentOrderOriginalLookupStatus::NOT_FOUND,$repo->findLineageForRevision('revision-0777')->status(),'globally absent target');assertSameValue($before,$f->facts(),'owner query has no mutation');
    });
    integrityCase('real-revision-owner-dangling-root',function()use($f,$repo){if(!method_exists($repo,'findLineageForRevision'))throw new TestFailure('INTENDED_RED: real revision-owner query absent');$f->resetOriginals();$f->seedAccepted();$f->corrupt(fn(mysqli $db)=>$db->query('DELETE FROM `'.$f->prefix.'fm2_assignment_order_original_roots`'));$before=$f->facts();assertSameValue(O\AssignmentOrderOriginalLookupStatus::UNAVAILABLE,$repo->findLineageForRevision('revision-0001')->status(),'existing revision with missing owner is not NOT_FOUND');assertSameValue($before,$f->facts(),'dangling target not repaired');});
    integrityCase('real-revision-owner-invalid-argument-no-SQL',function(){if(!method_exists(O\AssignmentOrderOriginalMariaDbRepository::class,'findLineageForRevision'))throw new TestFailure('INTENDED_RED: real revision-owner query absent');$db=new S\OriginalIntegrityNoSql();$repo=new O\AssignmentOrderOriginalMariaDbRepository($db,'data_');assertSameValue(O\AssignmentOrderOriginalLookupStatus::UNAVAILABLE,$repo->findLineageForRevision('bad/revision')->status(),'invalid target lookup unavailable');assertSameValue([],$db->calls,'invalid target before SQL');});
    ...
    integrityCase('real-duplicate-assignment-root-unavailable',function()use($f,$repo){
        ...
        assertSameValue(O\AssignmentOrderOriginalLookupStatus::UNAVAILABLE,$repo->findLineageForAssignmentOrder(4512,81)->status(),'duplicate assignment ownership cannot be hidden by LIMIT1');assertSameValue($before,$f->facts(),'duplicate roots not repaired');assertSameValue($catalog,$f->catalog(),'read never restores dropped fixture index');
    });
