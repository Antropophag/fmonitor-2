...
use FMonitor2\RuntimeRestore\{RuntimeRecoverySchemaV25 as V25,RuntimeRecoverySchemaV26 as V26,RuntimeRecoverySchemaV27 as V27,RuntimeRecoverySchemaV28 as V28,RuntimeRecoverySchemaV29 as V29,RuntimeRecoverySchemaV31 as V31};
...
try{
 ...
 $upgrade=Migration::run($db,$p,Catalogue::migrations());assertSameValue([0,31],[$upgrade['exitCode'],$upgrade['result']['schemaVersion']??null],'v25 forward through28');
 ...
 assertSameValue($auto,array_values(array_filter(Fixture::counters($db),static fn($r)=>!in_array($r['TABLE_NAME'],[$p.'fm2_deadline_certificate_revisions',$p.'fm2_control_engineer_assignments',$p.'fm2_legacy_identity_links',$p.'fm2_legacy_identity_link_events',$p.'fm2_equipment_fact_diagnostics',$p.'fm2_equipment_fact_history'],true))),'every old counter preserved');
 ...
 assertSameValue([77,42],[count(V26::tables($p)),count(V26::autoIncrement($p))],'exact certificate v26 recovery inventory');assertSameValue([78,42],[count(V27::tables($p)),count(V27::autoIncrement($p))],'exact document-links v27 recovery inventory');assertSameValue([79,43],[count(V28::tables($p)),count(V28::autoIncrement($p))],'exact historical v28 recovery inventory');assertSameValue([82,45],[count(V29::tables($p)),count(V29::autoIncrement($p))],'exact historical v29 recovery inventory');assertSameValue([87,47],[count(V31::tables($p)),count(V31::autoIncrement($p))],'exact current v31 recovery inventory');assertSameValue(V31::tables($p),Fixture::tables($db),'database exact current table set');
 $definitions=[
 ...
 $fresh=$f->database();assertSameValue([0,31],(static function($r){return[$r['exitCode'],$r['result']['schemaVersion']??null];})(Migration::run($fresh,$p,Catalogue::migrations())),'clean canonical29');
