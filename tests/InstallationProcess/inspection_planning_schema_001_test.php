...
try{$admin=new mysqli($host,$user,$pass,'',$port);$admin->query('CREATE DATABASE '.ipsQ($name).' DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci');$db=ipsDb($name);
    assertSameValue([36,28,29,30],[strlen('fm2_pilot_inspection_schedule_events'),64-strlen('fm2_pilot_inspection_schedule_events'),65-strlen('fm2_pilot_inspection_schedule_events')],'family-local identifier inventory is evidence only');
