...
try{$admin=new mysqli($host,$user,$pass,'',$port);$admin->query('CREATE DATABASE '.ipsQ($name).' DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci');$db=ipsDb($name);
    ...
    foreach([['schedules','events'],['events','schedules']]as[$present,$missing]){
        ...
        $before=ipsState($partialDb,'');ipsAssertRunnerSuccess($partial,range(1,35),"populated $present-only partial");$after=ipsState($partialDb,'');
