...
try {
    ...
    $runWorkerControl=static function(string$control)use($host,$port,$user,$password,$databases,$prefix,$controlParent,$admin):void{
        $process=null;$pipes=[];
        try{
            ...
            else{@proc_terminate($process,15);$termDeadline=hrtime(true)+300_000_000;do{usleep(10_000);$status=proc_get_status($process);}while(!$status['running']&&hrtime(true)<$termDeadline);$status=proc_get_status($process);assertSameValue(true,$status['running'],'Hang worker ignores TERM through bounded poll.');@proc_terminate($process,9);$killDeadline=hrtime(true)+5_000_000_000;do{usleep(10_000);$status=proc_get_status($process);}while($status['running']&&hrtime(true)<$killDeadline);assertSameValue(false,$status['running'],'KILL terminates hung pre-seam worker within bound.');foreach($pipes as$pipe)if(is_resource($pipe))fclose($pipe);$pipes=[];proc_close($process);$process=null;}
        ...
        }finally{foreach($pipes as$pipe)if(is_resource($pipe))fclose($pipe);if(is_resource($process)){$status=proc_get_status($process);if($status['running']){@proc_terminate($process,15);$deadline=hrtime(true)+300_000_000;do{usleep(10_000);$status=proc_get_status($process);}while($status['running']&&hrtime(true)<$deadline);if($status['running'])@proc_terminate($process,9);}proc_close($process);}}
    };
    $runWorkerControl('fail_before_seam');$runWorkerControl('hang_before_seam');$controlParent->close();
