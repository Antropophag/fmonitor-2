...
try {
    ...
    $actualOwnedTables=array_values(array_filter($tableNames($clean),static fn(string$name):bool=>str_starts_with($name,$prefix.'fm2_assignment_order_original_')));
    assertSameValue($expectedOwnedTables, $actualOwnedTables, 'Only exact original-owned manifest exists alongside prerequisites.');
    ...
    assertSameValue([...array_slice(Contract::TABLES, 1),'fm2_process_user_capabilities'], $partialResult->affectedTables(), 'Missing trailing tables then V4 capability are affected.');
    ...
    $populatedInitial=AssignmentOrderOriginalSchemaMigration::apply($populated, $prefix);assertSameValue([...Contract::TABLES,'fm2_process_user_capabilities'],$populatedInitial->affectedTables(),'Populated axis first V4 apply reports schema then capability.');
    ...
    $nearPrefix='near_';$prepareV4($conflict,$nearPrefix);$nearInitial=AssignmentOrderOriginalSchemaMigration::apply($conflict,$nearPrefix);assertSameValue([...Contract::TABLES,'fm2_process_user_capabilities'],$nearInitial->affectedTables(),'Near axis initial V4 apply reports capability last.');
    ...
    $checkPrefix='check_';$prepareV4($conflict,$checkPrefix);$checkInitial=AssignmentOrderOriginalSchemaMigration::apply($conflict,$checkPrefix);assertSameValue([...Contract::TABLES,'fm2_process_user_capabilities'],$checkInitial->affectedTables(),'CHECK axis initial V4 apply reports capability last.');
    ...
    $fixtureMigration=AssignmentOrderOriginalSchemaMigration::apply($fixture, $prefix);assertSameValue([...Contract::TABLES,'fm2_process_user_capabilities'],$fixtureMigration->affectedTables(),'Fixture setup V4 apply reports schema then capability.');
    ...
    $contendedFixtureCall=static function(string$mode,string$lockSql)use($fixture,$connect,$databases,$prefix,$host,$port,$user,$password,$admin):void{
        ...
        try{
            ...
            if($mode==='cleanup'){do{$wait=$observer->prepare("SELECT requesting.trx_state,requesting.trx_isolation_level,requesting.trx_query FROM information_schema.INNODB_LOCK_WAITS waits JOIN information_schema.INNODB_TRX requesting ON requesting.trx_id=waits.requesting_trx_id JOIN information_schema.INNODB_TRX blocking ON blocking.trx_id=waits.blocking_trx_id WHERE requesting.trx_mysql_thread_id=? AND blocking.trx_mysql_thread_id=?");$wait->bind_param('ii',$workerConnectionId,$blockingConnectionId);$wait->execute();$waitRow=$wait->get_result()->fetch_assoc();$wait->close();if(is_array($waitRow))break;usleep(10_000);}while(hrtime(true)<$deadline);assertSameValue(true,is_array($waitRow),'Cleanup exact worker is independently observed in InnoDB lock wait.');assertSameValue('LOCK WAIT',$waitRow['trx_state'],'Cleanup transaction actually waits.');assertSameValue('SERIALIZABLE',$waitRow['trx_isolation_level'],'Cleanup transaction is SERIALIZABLE.');assertSameValue(true,str_contains((string)$waitRow['trx_query'],$prefix.'fm2_process_tasks'),'Cleanup wait targets exact task identity table.');}
    ...
    $fieldOctets=static function(mysqli$db,string$table,string$where,string$column)use($prefix):string{$row=$db->query("SELECT HEX(CAST(`{$column}` AS BINARY)) octets FROM `{$prefix}{$table}` WHERE {$where}")->fetch_assoc();return(string)$row['octets'];};
    $collationDrifts=[
        ['trailing-space user','fm2_pilot_users','user_id=18','full_name','Тестовый Оператор ФКР','Тестовый Оператор ФКР '],
        ['case-equivalent role','fm2_pilot_roles','role_id=5301','description','TEST-USER','test-user'],
        ['composed accent email','fm2_pilot_users','user_id=18','email','test-fkr@example.invalid',"test-fkr@\u{00E9}xample.invalid"],
        ['decomposed accent email','fm2_pilot_users','user_id=31','email','test-engineer@example.invalid',"test-engineer@e\u{0301}xample.invalid"],
        ['trailing-space order','fm2_assignment_orders','id=81','object_address_snapshot','Тестовая улица, 1','Тестовая улица, 1 '],
        ['trailing-space installer','fm2_order_installers','assignment_order_id=81 AND installer_tab_id=7001','fio_snapshot','Тестовый Монтажник 7001','Тестовый Монтажник 7001 '],
        ['case-equivalent task','fm2_process_tasks','id=9001','task_type','assignment_order_original_upload','ASSIGNMENT_ORDER_ORIGINAL_UPLOAD'],
    ];
    foreach($collationDrifts as[$label,$table,$where,$column,$exact,$drift]){$exactOctets=$fieldOctets($fixture,$table,$where,$column);$setFixtureValue($fixture,$table,$where,$column,$drift);$driftOctets=$fieldOctets($fixture,$table,$where,$column);assertSameValue(false,hash_equals($exactOctets,$driftOctets),"{$label} is byte-distinct under binary oracle.");$before=$stateSnapshot($fixture);try{AssignmentOrderOriginalVerificationDatabaseFixture::seedExampleA($fixture,$prefix);throw new TestFailure("INTENDED_RED: {$label} was collation-accepted by seed.");}catch(AssignmentOrderOriginalVerificationFixtureConflict){}assertSameValue($before,$stateSnapshot($fixture),"{$label} seed conflict is zero DML.");try{AssignmentOrderOriginalVerificationDatabaseFixture::cleanupExampleA($fixture,$prefix);throw new TestFailure("INTENDED_RED: {$label} was collation-deleted by cleanup.");}catch(AssignmentOrderOriginalVerificationFixtureConflict){}assertSameValue($before,$stateSnapshot($fixture),"{$label} cleanup conflict is zero DML.");$setFixtureValue($fixture,$table,$where,$column,$exact);assertSameValue($exactOctets,$fieldOctets($fixture,$table,$where,$column),"{$label} restores exact literal octets.");}
    $ownedRows=[
        ...
        ['fm2_installation_cases','id=4512',['id']],['fm2_installation_cases','id=9999',['id']],['fm2_assignment_orders','id=81',['id']],
        ['fm2_order_installers','assignment_order_id=81 AND installer_tab_id=7001',['assignment_order_id','installer_tab_id']],['fm2_order_installers','assignment_order_id=81 AND installer_tab_id=7002',['assignment_order_id','installer_tab_id']],['fm2_process_tasks','id=9001',['id']],
    ];
    $fieldMutation=static function(mysqli$db,string$table,string$column,mixed$value)use($prefix):mixed{$statement=$db->prepare('SELECT LOWER(COLUMN_TYPE) t FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME=? AND COLUMN_NAME=?');$name=$prefix.$table;$statement->bind_param('ss',$name,$column);$statement->execute();$type=(string)$statement->get_result()->fetch_assoc()['t'];if($column==='installation_case_id')return'9999';if($column==='previous_assignment_order_id')return'81';if($value===null){if(str_contains($type,'date'))return'2026-09-03';if(str_contains($type,'int'))return'1';return'Drift-'.$column;}if(str_starts_with($type,'enum'))return$value==='active'?'blocked':'active';if(str_contains($type,'int'))return(string)(((int)$value)+1);if(str_contains($type,'date'))return str_starts_with((string)$value,'2026-09-03')?'2026-09-04':'2026-09-03';return(string)$value.'-drift';};
    foreach($ownedRows as[$table,$where,$identityColumns]){$row=$fixture->query("SELECT * FROM `{$prefix}{$table}` WHERE {$where}")->fetch_assoc();assertSameValue(true,is_array($row),"Full field matrix finds {$table} {$where}.");foreach($row as$column=>$exact){if(in_array($column,$identityColumns,true))continue;$exactOctets=$fieldOctets($fixture,$table,$where,$column);$drift=$fieldMutation($fixture,$table,$column,$exact);$setFixtureValue($fixture,$table,$where,$column,$drift);assertSameValue(false,hash_equals($exactOctets,$fieldOctets($fixture,$table,$where,$column)),"{$table}.{$column} mutation is octet-distinct.");$before=$stateSnapshot($fixture);try{AssignmentOrderOriginalVerificationDatabaseFixture::seedExampleA($fixture,$prefix);throw new TestFailure("INTENDED_RED: {$table}.{$column} drift was accepted by seed.");}catch(AssignmentOrderOriginalVerificationFixtureConflict){}assertSameValue($before,$stateSnapshot($fixture),"{$table}.{$column} seed conflict is zero DML.");$setFixtureValue($fixture,$table,$where,$column,$exact);$setFixtureValue($fixture,$table,$where,$column,$drift);$before=$stateSnapshot($fixture);try{AssignmentOrderOriginalVerificationDatabaseFixture::cleanupExampleA($fixture,$prefix);throw new TestFailure("INTENDED_RED: {$table}.{$column} drift was deleted by cleanup.");}catch(AssignmentOrderOriginalVerificationFixtureConflict){}assertSameValue($before,$stateSnapshot($fixture),"{$table}.{$column} cleanup conflict is zero DML.");$setFixtureValue($fixture,$table,$where,$column,$exact);assertSameValue($exactOctets,$fieldOctets($fixture,$table,$where,$column),"{$table}.{$column} restores literal octets.");}}
