...
'complete pages request and repeat'=>static function(F $f):void{
    ...
    $f->scenario('missing_optional_uf_xing');[$optionalResult,$optionalRows]=$f->fetch();bwdOutcome($optionalResult,'complete',null,2,2,51);assertSameValue(null,$optionalRows[0]['UF_XING'],'omitted optional fallback normalizes to explicit null');assertSameValue(array_keys(bwdPerson(1)),array_keys($optionalRows[0]),'selected output retains exact field order');
