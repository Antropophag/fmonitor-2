...
A::case('named-lock-contention',function($f){
 ...
 try{assertSameValue(1,(int)$other->query("SELECT GET_LOCK('{$lock}',0) n")->fetch_assoc()['n'],'independent connection owns lock');$start=hrtime(true);try{A::migrate($f);throw new TestFailure('Contended migration must fail');}catch(I\DatabaseUnavailable $e){assertSameValue('Original attempt audit schema unavailable.',$e->getMessage(),'fixed contention failure');}assertSameValue(true,(hrtime(true)-$start)<8_000_000_000,'configured lock wait stays within the fixture statement limit');assertSameValue($catalog,$f->catalog(),'contended migration no DDL');}finally{$other->query("SELECT RELEASE_LOCK('{$lock}')");$other->close();}
});
...
A::case('canonical-original-family13-repeat',function($f){
 [$exit,$result]=A::runCanonical($f);assertSameValue(0,$exit,'canonical applies original family');assertSameValue([true,13],[$result['ok']??null,$result['schemaVersion']??null],'canonical version13');assertSameValue(true,in_array(13,$result['appliedVersions']??[],true),'original v3 actually applied');
 $facts=$f->facts();assertSameValue([0,['ok'=>true,'schemaVersion'=>13,'appliedVersions'=>[]]],A::runCanonical($f),'entire canonical runner repeats through v5 capability successor');assertSameValue($facts,$f->facts(),'repeat preserves original facts');
});
A::case('canonical-clean-prefix-isolation',function($f){
 $old=$f->prefix;$before=$f->facts();$f->prefix='fresh_';
 assertSameValue([0,['ok'=>true,'schemaVersion'=>13,'appliedVersions'=>range(1,13)]],A::runCanonical($f),'canonical clean installs every version including original');
 assertSameValue([0,['ok'=>true,'schemaVersion'=>13,'appliedVersions'=>[]]],A::runCanonical($f),'canonical clean repeat');
