...
A::case('canonical-original-family13-repeat',function($f){
 [$exit,$result]=A::runCanonical($f);assertSameValue(0,$exit,'canonical applies original family');assertSameValue([true,27],[$result['ok']??null,$result['schemaVersion']??null],'canonical final version27, original remains step13');assertSameValue(true,in_array(13,$result['appliedVersions']??[],true),'original v3 actually applied');
 $facts=$f->facts();assertSameValue([0,['ok'=>true,'schemaVersion'=>27,'appliedVersions'=>[]]],A::runCanonical($f),'entire canonical runner repeats through current successors');assertSameValue($facts,$f->facts(),'repeat preserves original facts');
});
A::case('canonical-clean-prefix-isolation',function($f){
 $old=$f->prefix;$before=$f->facts();$f->prefix='fresh_';
 assertSameValue([0,['ok'=>true,'schemaVersion'=>27,'appliedVersions'=>range(1,27)]],A::runCanonical($f),'canonical clean installs every version including original');
 assertSameValue([0,['ok'=>true,'schemaVersion'=>27,'appliedVersions'=>[]]],A::runCanonical($f),'canonical clean repeat');
