...
foreach(['escaped_name'=>Corpus::escapedActiveName(),'indirect_action'=>Corpus::indirectActiveAction()]as$name=>$bytes)assertSameValue(AssignmentOrderOriginalPdfStatus::UNSAFE_PDF,$inspector->inspect($bytes)->status,"Forbidden {$name} remains unsafe after lexical name decoding and reference traversal.");
