...
$aggregateAtLimit=Corpus::aggregateFlateContentStreams(33_554_432);assertSameValue(true,strlen($aggregateAtLimit)<200_000,'Two-stream aggregate-bound control remains a compact received fixture.');assertSameValue(AssignmentOrderOriginalPdfStatus::PASSIVE_PDF,$inspector->inspect($aggregateAtLimit)->status,'Two individually bounded Flate streams at the exact aggregate decoded-byte limit are accepted.');unset($aggregateAtLimit);
$aggregateOverLimit=Corpus::aggregateFlateContentStreams(33_554_433);assertSameValue(true,strlen($aggregateOverLimit)<200_000,'Two-stream aggregate-overflow fixture remains compact.');
