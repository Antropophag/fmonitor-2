...
try{
    ...
    $second=$session->writeCommit($id1,'payload-two-latest');assertSameValue(['OK',$id1],[$second->status()->name,$second->currentSessionId()],'Repeated anonymous commit retains the caller current ID1.');
    $committed=glob($root.'/sessions/anonymous_repeat/s-*.session')?:[];assertSameValue(1,count($committed),'Repeated anonymous commit leaves exactly one committed file.');assertSameValue($root.'/sessions/anonymous_repeat/s-'.$id1.'.session',$committed[0],'Only ID1 remains addressable.');assertSameValue('payload-two-latest',file_get_contents($committed[0]),'Repeated commit publishes latest bytes under ID1.');$session->close();

    $foreignId=str_repeat('aa',32);$collisionDir=$root.'/sessions/external_collision';mkdir($collisionDir,0700,true);$foreign=$collisionDir.'/s-'.$foreignId.'.session';file_put_contents($foreign,'externally-owned-bytes');chmod($foreign,0600);$foreignHash=hash_file('sha256',$foreign);
    $collision=$owner('external_collision',[32=>[str_repeat("\xaa",32),str_repeat("\xbb",32)],16=>[str_repeat("\xcc",16)],12=>array_fill(0,4,str_repeat("\xdd",12))]);$collisionStart=$collision->start(null);assertSameValue(str_repeat('bb',32),$collisionStart->currentSessionId(),'Anonymous ID generation skips a truly preexisting committed collision.');assertSameValue($foreignHash,hash_file('sha256',$foreign),'Preexisting externally-owned collision bytes remain immutable.');$collision->close();
