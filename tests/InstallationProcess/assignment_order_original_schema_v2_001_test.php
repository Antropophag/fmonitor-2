...
$alterCount=static fn(mysqli$db):int=>(int)$db->query("SHOW SESSION STATUS LIKE 'Com_alter_table'")->fetch_assoc()['Value'];
...
try{
    ...
    $clean=$newDb('clean');$prepareV4($clean);$altersBefore=$alterCount($clean);$cleanResult=AssignmentOrderOriginalSchemaMigration::apply($clean,$prefix);
    assertSameValue([AssignmentOrderOriginalSchemaMigrationStatus::APPLIED,2,[...Contract::TABLES,'fm2_process_user_capabilities']],[$cleanResult->status(),$cleanResult->schemaVersion(),$cleanResult->affectedTables()],'Clean V4 applies schema v2 in manifest order with capability last.');assertSameValue(1,$alterCount($clean)-$altersBefore,'Clean v2 names idx_aoou_revision_content directly in CREATE; capability publication is the only ALTER.');$assertV2($clean,'Clean');$before=$snapshot($clean);$repeat=AssignmentOrderOriginalSchemaMigration::apply($clean,$prefix);assertSameValue([AssignmentOrderOriginalSchemaMigrationStatus::UNCHANGED,2,[]],[$repeat->status(),$repeat->schemaVersion(),$repeat->affectedTables()],'Exact populated-capable v2 repeat is unchanged.');assertSameValue($before,$snapshot($clean),'V2 repeat performs zero DDL/DML.');$clean->close();
