...
try {
    ...
    assertSameValue(32, $runnerResult['schemaVersion'], 'G2-01 canonical runner must own the current terminal v32 after proven v1-v7.');
    assertSameValue(range(1,32), $runnerResult['appliedVersions'], 'G2-01 runner ordering is exact.');
    ...
    $before = iesState($db, '');
    $repeat=iesRunRunner($database);assertSameValue(0,$repeat['exitCode'],'G2-02 repeat runner exits zero.');assertSameValue(['ok'=>true,'schemaVersion' => 32,'appliedVersions'=>[]],json_decode($repeat['stdout'],true,flags:JSON_THROW_ON_ERROR),'G2-02 repeat runner omits terminal migrations.');
