...
{
    $ddl=['fm2_installation_cases'=>'id BIGINT PRIMARY KEY,legacy_installation_object_id BIGINT NOT NULL,process_state VARCHAR(80) NOT NULL','fm2_assignment_orders'=>'id BIGINT PRIMARY KEY,installation_case_id BIGINT NOT NULL,version_no INT NOT NULL,status VARCHAR(40) NOT NULL','fm2_order_installers'=>'assignment_order_id BIGINT NOT NULL,installer_tab_id BIGINT NOT NULL,fio_snapshot VARCHAR(300) NOT NULL,position_snapshot VARCHAR(300) NOT NULL,employment_status_snapshot VARCHAR(40) NOT NULL,workforce_source_updated_at_snapshot VARCHAR(40) NOT NULL','fm2_workforce_catalog'=>'installer_tab_id BIGINT PRIMARY KEY,fio VARCHAR(300) NOT NULL,position VARCHAR(300) NOT NULL,employment_status VARCHAR(40) NOT NULL,dismissal_effective_at VARCHAR(40) NULL,workforce_source_updated_at VARCHAR(40) NOT NULL','fm2_checklist_template_snapshots'=>'id BIGINT PRIMARY KEY,snapshot_version VARCHAR(80) NOT NULL,valid_from DATETIME NOT NULL,content_sha256 CHAR(64) NOT NULL','fm2_checklist_template_associations'=>'subject_kind VARCHAR(40) NOT NULL,subject_id VARCHAR(160) NOT NULL,effective_at DATETIME NOT NULL,template_snapshot_id BIGINT NOT NULL,template_snapshot_version VARCHAR(80) NOT NULL,template_content_sha256 CHAR(64) NOT NULL'];foreach($ddl as$name=>$definition)$db->query('CREATE TABLE '.iesQuote($prefix.$name)."({$definition}) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");iesApply($db,$prefix);$hash=str_repeat('a',64);$db->query("INSERT INTO ".iesQuote($prefix.'fm2_installation_cases')." VALUES(71,1701,'working')");$db->query("INSERT INTO ".iesQuote($prefix.'fm2_assignment_orders')." VALUES(81,71,1,'registered')");$db->query("INSERT INTO ".iesQuote($prefix.'fm2_order_installers')." VALUES(81,42,'Иванов','Электромеханик','employed','2026-09-01')");$db->query("INSERT INTO ".iesQuote($prefix.'fm2_workforce_catalog')." VALUES(42,'Иванов','Электромеханик','employed',NULL,'2026-09-01')");$db->query("INSERT INTO ".iesQuote($prefix.'fm2_checklist_template_snapshots')." VALUES(91,'fixture-v1','2026-08-01 00:00:00','{$hash}')");$db->query("INSERT INTO ".iesQuote($prefix.'fm2_checklist_template_associations')." VALUES('operational_case','71','2026-08-02 00:00:00',91,'fixture-v1','{$hash}')");
}
...
try {
    ...
    assertSameValue(8, $runnerResult['schemaVersion'], 'G2-01 canonical runner must own literal inspection-evidence v8 after proven v1-v7.');
    assertSameValue([1,2,3,4,5,6,7,8], $runnerResult['appliedVersions'], 'G2-01 runner ordering is exact.');
    ...
    $before = iesState($db, '');
    $repeat=iesRunRunner($database);assertSameValue(0,$repeat['exitCode'],'G2-02 repeat runner exits zero.');assertSameValue(['ok'=>true,'schemaVersion'=>8,'appliedVersions'=>[]],json_decode($repeat['stdout'],true,flags:JSON_THROW_ON_ERROR),'G2-02 repeat runner omits v8.');
