...
try{
    ...
    $first=CanonicalMigrationApplication::run($db,$prefix,$catalogue);assertSameValue([0,true,30,range(1,30)],[$first['exitCode'],$first['result']['ok']??null,$first['result']['schemaVersion']??null,$first['result']['appliedVersions']??null],'clean canonical run reaches v26');
    ...
    $repeat=CanonicalMigrationApplication::run($db,$prefix,$catalogue);assertSameValue([0,['ok'=>true,'schemaVersion'=>30,'appliedVersions'=>[]]],[$repeat['exitCode'],$repeat['result']],'compatible populated repeat is no-op');
