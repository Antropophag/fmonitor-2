...
try {
    ...
    assertSameValue(false,str_contains($get['body'],'Legacy identity decoy'),'legacy user name decoy does not replace local representation identity');assertSameValue(false,str_contains($get['body'],'legacy.object-list@example.invalid'),'legacy/REMOTE_USER email decoy is not rendered or used as local identity');
    ...
    // Combined faults pin the successor order: route/method, trusted transport
    // identity, local RBAC admission, inherited CSS, then list read.
