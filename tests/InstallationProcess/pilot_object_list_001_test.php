...
function polStableStat(string $path):array{$stat=lstat($path);unset($stat[8],$stat['atime']);return$stat;}
...
try {
    ...
    assertSameValue(0,(int)$xpath->evaluate("count(//main[@id='main-content']//form | //main[@id='main-content']//input | //main[@id='main-content']//select)"),'native list has no rapid-pilot filter controls');assertSameValue(1,(int)$xpath->evaluate("count(//table[contains(concat(' ',normalize-space(@class),' '),' fm2-queue-table ')])"),'current queue rich table');assertSameValue(true,str_contains($get['body'],'Показано 3 из 3'),'current queue exact count copy');
    $items=$xpath->query("//table[contains(concat(' ',normalize-space(@class),' '),' fm2-queue-table ')]/tbody/tr");$actual=[];
    $expected=[['4513','/pilot/objects/4513',['77-000124','Москва, ул. Вторая, д. 7','1','01.10.2026','30.11.2026']],['4512','/pilot/objects/4512',['77-000123','Москва, ул. Примерная, д. 10','2','05.10.2026','18.12.2026']],['4515','/pilot/objects/4515',['77-000126','Москва, ул. Третья, д. 3','1','05.10.2026','20.12.2026']]];
    assertSameValue(3,$items->length,'exact current object row count');
    foreach($items as $item){$itemXpath=new DOMXPath($document);$links=$itemXpath->query(".//a[starts-with(@href,'/pilot/objects/') and contains(concat(' ',normalize-space(@class),' '),' fm2-object-link ')]",$item);assertSameValue(1,$links->length,'one public card link in each row');$link=$links->item(0);$actual[]=[preg_replace('/\s+/u',' ',trim($link->textContent)),$link->getAttribute('href'),preg_replace('/\s+/u',' ',trim($item->textContent))];}
    foreach($expected as $index=>$row){assertSameValue($row[1],$actual[$index][1],'canonical table row order '.$row[0]);assertSameValue(1,$xpath->query(".//a[@href='{$row[1]}']/strong[normalize-space(.)='№ {$row[0]}']",$items->item($index))->length,'exact linked object id '.$row[0]);assertSameValue(1,$xpath->query(".//a[@href='{$row[1]}']/span[normalize-space(.)='{$row[2][0]}']",$items->item($index))->length,'exact linked registration '.$row[0]);foreach($row[2] as $literal)assertSameValue(true,str_contains($actual[$index][2],$literal),'approved value belongs to row '.$row[0].' '.$literal);}
    ...
    foreach(['textarea','style'] as $tag)assertSameValue(0,$xpath->query('//'.$tag)->length,'forbidden element '.$tag);
    ...
    $db->query('DELETE FROM fm2_installation_cases'); $emptyBefore=polSnapshot($db); $server=polStart($environment); $empty=polParity($server['port'],'/pilot/objects'); assertSameValue(200,$empty['status'],'empty collection succeeds'); assertSameValue(true,str_contains($empty['body'],'Сейчас нет объектов монтажа')&&str_contains($empty['body'],'Объекты появятся после загрузки исходных данных.'),'exact current empty text'); assertSameValue(0,preg_match_all('~href=["\']/pilot/objects/\d+["\']~',$empty['body']),'empty has no card links'); assertSameValue($emptyBefore,polSnapshot($db),'empty read-only'); polStop($server);$server=null;
    ...
    try{assertSameValue(array_fill_keys(['server PID/pipes','list-fault DB resource','DB resource','list-fault DB user','denial DB user','reader DB user','task database','mutable root'],1),$cleanupAttempts,'cleanup inventory attempted exactly once');assertSameValue([], $admin->query("SELECT SCHEMA_NAME FROM information_schema.SCHEMATA WHERE SCHEMA_NAME='{$database}'")->fetch_all(MYSQLI_ASSOC),'exact task DB removed');assertSameValue([], $admin->query("SELECT User FROM mysql.user WHERE User IN ('{$reader}','{$denialReader}','{$listReadFaultReader}')")->fetch_all(MYSQLI_ASSOC),'exact task users removed');assertSameValue([['SCHEMA_NAME'=>$foreignDatabase]],$admin->query("SELECT SCHEMA_NAME FROM information_schema.SCHEMATA WHERE SCHEMA_NAME='{$foreignDatabase}'")->fetch_all(MYSQLI_ASSOC),'foreign DB preserved');assertSameValue([['id'=>'1','payload'=>'foreign-db-bytes']],$admin->query("SELECT * FROM `{$foreignDatabase}`.sentinel")->fetch_all(MYSQLI_ASSOC),'foreign DB bytes preserved');assertSameValue($foreignBefore,[polStableStat($foreignPath),polStableStat($foreignPath.'/keep'),hash_file('sha256',$foreignPath.'/keep')],'foreign file bytes and stable metadata preserved');}catch(Throwable $failure){$cleanupErrors[]=$failure;}
