...
{
    $tokens=['origin','provenance','classification','migration','demo'];$violations=[];
    foreach($xpath->query("//main[@id='main-content']//*[@id or @class or @role or @*[starts-with(name(),'data-')]]")as$element){
        foreach($element->attributes as$attribute){$name=strtolower($attribute->name);$value=mb_strtolower($attribute->value,'UTF-8');$inspectValue=in_array($name,['id','class','role'],true);$inspectName=str_starts_with($name,'data-');if(!$inspectValue&&!$inspectName)continue;foreach($tokens as$token)if(($inspectValue&&str_contains($value,$token))||($inspectName&&str_contains($name,$token)))$violations[]=$element->nodeName.'@'.$name.'='.$attribute->value;}
    }
...
try {
    ...
    $classificationProbe=polDocument('<!doctype html><html><head><meta charset="utf-8"></head><body><main id="main-content"><span class="fm2-provenance-badge">Источник</span><i id="migration-state"></i><b data-demo-kind="x"></b></main></body></html>');assertSameValue(3,count(polObjectListClassificationHooks(new DOMXPath($classificationProbe))),'classification hook oracle catches class, id and data-name mutations');
