...
try {
    ...
    $server=polStart($environment);$firstPage=polReadOnly($db,fn()=>polParity($server['port'],'/pilot/objects'),'501-case page one');assertSameValue(200,$firstPage['status'],'501 imported cases paginate');assertSameValue(50,preg_match_all('~href=["\']/pilot/objects/\d+["\']~',$firstPage['body']),'fixed page size');assertSameValue(true,str_contains($firstPage['body'],'Показано 50 из 501')&&str_contains($firstPage['body'],'href="/pilot/objects?page=2"'),'total and next page rendered');$lastPage=polReadOnly($db,fn()=>polParity($server['port'],'/pilot/objects?page=11'),'501-case last page');assertSameValue(200,$lastPage['status'],'last page succeeds');assertSameValue(1,preg_match_all('~href=["\']/pilot/objects/\d+["\']~',$lastPage['body']),'last page remainder');assertSameValue(true,str_contains($lastPage['body'],'Страница 11 из 11'),'page position rendered');
