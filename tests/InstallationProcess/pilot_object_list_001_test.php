...
try {
    ...
    foreach(['4999','77-009999','SECRET-','process_state','Следующее действие','Происхождение данных','Только миграция','Нативный импорт','Активен с cutover','Активны с cutover','Демо-данные','Локальная проверка','Локальные проверки'] as $forbidden)assertSameValue(false,str_contains($get['body'],$forbidden),'excluded value/control '.$forbidden);
    ...
    $server=polStart($environment);$overflow=polReadOnly($db,fn()=>polParity($server['port'],'/pilot/objects'),'501-case overflow');assertSameValue(503,$overflow['status'],'501 imported cases fail closed without pagination');assertSameValue("Service unavailable.\n",$overflow['body'],'501-case overflow exact redacted body');assertSameValue('60',polHeader($overflow,'retry-after'),'501-case overflow Retry-After');assertSameValue(0,preg_match_all('~href=["\']/pilot/objects/\d+["\']~',$overflow['body']),'501-case overflow never returns a partial page');polStop($server);$server=null;$db->query('DELETE FROM fm2_installation_cases WHERE id=501');$server=polStart($environment);$ceiling=polReadOnly($db,fn()=>polParity($server['port'],'/pilot/objects'),'500-case ceiling');assertSameValue(200,$ceiling['status'],'exactly 500 imported cases succeed');assertSameValue(500,preg_match_all('~href=["\']/pilot/objects/\d+["\']~',$ceiling['body']),'exactly 500 returns the complete unpaginated set');polStop($server);$server=null;
