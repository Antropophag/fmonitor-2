...
{
    $forbidden=static function(string$value):bool{$normalized=mb_strtolower(preg_replace('/\s+/u',' ',trim($value))??'','UTF-8');return preg_match('~(?:^|[^a-z0-9])(?:origin|provenance|classification|migration|demo|source)(?:$|[^a-z0-9])|источник|происхожд|классификац|миграц|демо~u',$normalized)===1;};$violations=[];
    foreach($xpath->query("//main[@id='main-content'] | //main[@id='main-content']//*")as$element){
        foreach($element->attributes as$attribute){$name=strtolower($attribute->name);$isGenericData=str_starts_with($name,'data-');if((in_array($name,['id','class','role'],true)||$isGenericData)&&$forbidden($attribute->value))$violations[]=$element->nodeName.'@'.$name.'='.$attribute->value;if($isGenericData&&$forbidden($name))$violations[]=$element->nodeName.'@'.$name;}
    }
    $visible=[];foreach($xpath->query("//main[@id='main-content']//text()[not(ancestor::script or ancestor::style or ancestor::*[@hidden or @aria-hidden='true'])]")as$text)$visible[]=$text->nodeValue;if($forbidden(implode(' ',$visible)))$violations[]='visible-copy';
...
try {
    ...
    $classificationSafeProbe=polDocument('<!doctype html><html><head><meta charset="utf-8"></head><body><aside class="fm2-provenance-panel">Источник данных</aside><main id="main-content"><span>77-000124 · Москва, ул. Вторая, д. 7 · 2026-10-01</span></main></body></html>');assertSameValue([],polObjectListClassificationHooks(new DOMXPath($classificationSafeProbe)),'classification oracle accepts approved object facts and ignores shared-shell provenance consumers');
