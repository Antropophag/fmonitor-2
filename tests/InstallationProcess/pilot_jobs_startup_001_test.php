...
try{
    $composeEnvironment=array_replace(getenv(),['FMONITOR_BOOTSTRAP_SUPERADMIN_PASSWORD'=>'fixture-only','FMONITOR_ERP_HOST'=>'erp.example.invalid','FMONITOR_ERP_SERVER_CERTIFICATE_FILE_HOST'=>dirname(__DIR__).'/fixtures/erp-server-certificate.pem','FMONITOR_ERP_DATABASE'=>'legacy-stage','FMONITOR_ERP_USER'=>'reader','FMONITOR_ERP_PASSWORD'=>'synthetic-password','FMONITOR_ERP_EQUIPMENT_FACTS_HMAC_KEY'=>str_repeat('h',40),'FMONITOR_ERP_EQUIPMENT_FACTS_MAX_ROWS'=>'500','FMONITOR_ERP_EQUIPMENT_FACTS_TIMEOUT_SECONDS'=>'5','FMONITOR_ERP_EQUIPMENT_FACTS_CHUNK_SIZE'=>'100']);
    ...
    foreach($expected as$serviceName=>$mode){
        ...
        assertSameValue(['CMD','php','bin/yii','jobs/process-health','--interactive=0'],$service['healthcheck']['test']??null,$serviceName.' process health uses native Jobs through Yii console');
    ...
    $base=array_replace(getenv(),['PATH'=>(string)getenv('PATH'),'TMPDIR'=>$tmp.'/temp','FMONITOR_SESSION_STATE_ROOT'=>$tmp.'/state','FMONITOR_BITRIX_CONFIG'=>$config,'FMONITOR_DB_HOST'=>'127.0.0.1','FMONITOR_DB_PORT'=>'1','FMONITOR_DB_NAME'=>'none','FMONITOR_DB_USER'=>'none','FMONITOR_DB_PASSWORD'=>'none','FMONITOR_SESSION_INSTANCE'=>'pilot','FMONITOR_ERP_HOST'=>'erp.example.invalid','FMONITOR_ERP_SERVER_CERTIFICATE_FILE'=>dirname(__DIR__).'/fixtures/erp-server-certificate.pem','FMONITOR_ERP_DATABASE'=>'legacy-stage','FMONITOR_ERP_USER'=>'reader','FMONITOR_ERP_PASSWORD'=>'synthetic-password','FMONITOR_ERP_EQUIPMENT_FACTS_HMAC_KEY'=>str_repeat('h',40),'FMONITOR_ERP_EQUIPMENT_FACTS_MAX_ROWS'=>'500','FMONITOR_ERP_EQUIPMENT_FACTS_TIMEOUT_SECONDS'=>'5','FMONITOR_ERP_EQUIPMENT_FACTS_CHUNK_SIZE'=>'100']);
