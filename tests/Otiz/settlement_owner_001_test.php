...
try {
    ...
    $db->query("INSERT INTO {$prefix}fm2_pilot_otiz_snapshots(id,report_date,status,rules_version,calculated_at,calculated_by_user_id,accepted_at,accepted_by_user_id,total_pool_cents,total_closed_cents,total_available_cents,content_hash)VALUES(103,'2026-10-31','accepted','premium-calculation-v1','2026-11-01T09:00:00+03:00',501,'2026-11-01T10:00:00+03:00',501,50000,100000,50000,'{$hash}')");
    $db->query("INSERT INTO {$prefix}fm2_pilot_otiz_snapshot_objects {$object}(103,7003,'AFTER','Built after closure',8500,10000,'2026-10-31',150000,10000,10000,150000,150000,100000,50000,50000,50000,0,'ready','{}')");
