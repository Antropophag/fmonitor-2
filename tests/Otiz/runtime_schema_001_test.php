...
try{
    ...
    $canonical=CanonicalMigrationApplication::run($db,$clean,$catalogue);assertSameValue([0,true,21,range(1,21)],[$canonical['exitCode'],$canonical['result']['ok']??null,$canonical['result']['schemaVersion']??null,$canonical['result']['appliedVersions']??null],'clean canonical migration reaches the complete v21 frontier');
    $repeat=CanonicalMigrationApplication::run($db,$clean,$catalogue);assertSameValue([0,true,21,[]],[$repeat['exitCode'],$repeat['result']['ok']??null,$repeat['result']['schemaVersion']??null,$repeat['result']['appliedVersions']??null],'complete canonical v21 repeat is a no-op');
    ...
    $store=orsRuntimeSource('rapid-pilot/legacy-migration/MigratedEvidenceProjectionStore.php');assertSameValue(true,str_contains($store,'rebuildDecisionState'), 'rebuildDecisionState remains covered by runtime DDL inventory');assertSameValue(true,str_contains($store,'backfill'), 'projection backfill remains covered by runtime DDL inventory');
