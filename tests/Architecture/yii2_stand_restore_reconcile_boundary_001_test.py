...
class T(unittest.TestCase):
 ...
 def test_production_attestation_uses_canonical_compose_database_service(self):
  code=(R/'app/RuntimeRestore/StandRestoreReconciliation.php').read_text()
  if "'ps','-q','db'" not in code:print('INTENDED_RED: reconciliation observes nonexistent compose service database')
  self.assertIn("'ps','-q','db'",code);self.assertNotIn("'ps','-q','database'",code)
