...
try {
    ...
    $correctionBreadcrumb=$fragment($correction['body'],'#<nav class="fm2-breadcrumb".*?</nav>#s','correction breadcrumb');$historyBreadcrumb=$fragment($history['body'],'#<nav class="fm2-breadcrumb".*?</nav>#s','history breadcrumb');$correctionPrimary=$fragment($correction['body'],'#<div class="fm2-order-object".*?</div>#s','correction primary identity');foreach($expectedEscaped as$value){assertSameValue(true,str_contains($correctionBreadcrumb,$value),'correction breadcrumb scoped '.$value);assertSameValue(true,str_contains($historyBreadcrumb,$value),'history breadcrumb scoped '.$value);assertSameValue(true,str_contains($correctionPrimary,$value),'correction primary scoped '.$value);}
    ...
    foreach(['form'=>$path.'/submit','history'=>$path.'/history']as$surface=>$url){$response=$f->request('GET',$url,[],$cookies);$missingExpected=['Регистрационный номер не указан','Адрес без номера','Подъезд 7','Заводской номер не указан'];$assertIdentity($response,$missingExpected,['Объект № 4512','Объект монтажа № 4512'],$surface.' missing identifiers');$missingBreadcrumb=$fragment($response['body'],'#<nav class="fm2-breadcrumb".*?</nav>#s',$surface.' missing breadcrumb');foreach($missingExpected as$value)assertSameValue(true,str_contains($missingBreadcrumb,$value),$surface.' missing breadcrumb scoped '.$value);}
    ...
    $denied=[];$f->login($denied,95);$detailsTable=$p.'fm2_object_detail_edits';$unavailableDetails=$detailsTable.'_unavailable';$f->db->query("RENAME TABLE `$detailsTable` TO `$unavailableDetails`");
    try {
        foreach([$path.'/submit',$path.'/history']as$url){$response=$f->request('GET',$url,[],$denied);assertSameValue(403,$response['status'],'no-read actor denied before unavailable effective owner '.$url);foreach(['Адрес без номера','Регистрационный номер не указан','TEST-4512']as$secret)assertSameValue(false,str_contains($response['body'],$secret),'denied response hides '.$secret);}
        foreach(['/pilot/objects/9999/assignment-orders/81/originals/submit','/pilot/objects/4512/assignment-orders/9999/originals/submit','/pilot/objects/9999/assignment-orders/81/originals/history','/pilot/objects/4512/assignment-orders/9999/originals/history']as$url){$response=$f->request('GET',$url,[],$cookies);assertSameValue(true,in_array($response['status'],[403,404],true),'unavailable object/order rejected before effective owner '.$url);}
        foreach([$path.'/submit',$path.'/history']as$url)assertSameValue(503,$f->request('GET',$url,[],$cookies)['status'],'authorized control reaches unavailable effective owner '.$url);
    } finally {$f->db->query("RENAME TABLE `$unavailableDetails` TO `$detailsTable`");}
