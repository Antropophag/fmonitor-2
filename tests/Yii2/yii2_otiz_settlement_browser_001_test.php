...
try {
    ...
    $source=['label'=>'Synthetic settlement browser','locator'=>'fixture://settlement-browser/7301','contentSha256'=>hash('sha256','settlement-browser-7301')];
    $operands=[];
    foreach(['premiumCents'=>100000,'shaftBp'=>10000,'progressBp'=>10000,'deadlineDate'=>'2026-09-30','reportDate'=>'2026-09-30','completionDate'=>null] as $key=>$value)$operands[$key]=['value'=>$value,'effectiveDate'=>'2026-09-30','source'=>$source];
    $calculation=FMonitor2\Otiz\PremiumCalculationV2::calculate($operands,['closures'=>[],'actualPayouts'=>[]]);
    $trace=json_encode(['premiumCalculation'=>$calculation,'sourceEvidence'=>['originalDeadline'=>$operands['deadlineDate'],'certificate'=>null,'completion'=>['root'=>null]]],JSON_THROW_ON_ERROR);
