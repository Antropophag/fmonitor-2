...
function preopeningConcurrent(array $requests,?callable $afterSend=null,?callable $beforeReceive=null):array
{
    $sockets=[];$results=[];
    try {
        foreach($requests as$index=>$r){
            ...
            if($afterSend!==null)$afterSend($index);
        }
        if($beforeReceive!==null)$beforeReceive();
