...
try {
  ...
  const suffix=config.mode==='initial'?'/submit':'/history';const url=config.origin+'/pilot/objects/4512/assignment-orders/81/originals'+suffix;let response=await page.goto(url);check(response.status()===200,'original identity surface status');
  ...
  fs.writeFileSync(config.result,JSON.stringify({mode:config.mode,desktop,narrow,hostileElement,screenshots:[1440,320]}));
