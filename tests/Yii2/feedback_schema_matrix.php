...
$catalogue=FMonitor2\InstallationProcess\ProductionPilotMigrationCatalogue::migrations();assertSameValue(range(1,26),array_keys($catalogue),'v26 canonical frontier');
$before=feedbackFacts($f);$repeat=FMonitor2\InstallationProcess\CanonicalMigrationApplication::run($f->db,$f->p,$catalogue);assertSameValue([0,26,[]],[$repeat['exitCode'],$repeat['result']['schemaVersion'],$repeat['result']['appliedVersions']],'populated schema replay');assertSameValue($before,feedbackFacts($f),'schema replay preserves feedback');
$actual=array_column($f->db->query('SHOW TABLES')->fetch_all(MYSQLI_NUM),0);sort($actual);
assertSameValue($actual,FMonitor2\RuntimeRestore\RuntimeRecoverySchemaV26::tables($f->p),'backup includes exact new inventory');
$auto=array_column($f->db->query('SELECT TABLE_NAME FROM information_schema.TABLES WHERE TABLE_SCHEMA=DATABASE() AND AUTO_INCREMENT IS NOT NULL ORDER BY BINARY TABLE_NAME')->fetch_all(MYSQLI_ASSOC),'TABLE_NAME');assertSameValue($auto,FMonitor2\RuntimeRestore\RuntimeRecoverySchemaV26::autoIncrement($f->p),'backup AI inventory');
