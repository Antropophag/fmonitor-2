...
$catalogue=FMonitor2\InstallationProcess\ProductionPilotMigrationCatalogue::migrations();assertSameValue(range(1,35),array_keys($catalogue),'canonical migration frontier');
...
assertSameValue($actual,FMonitor2\RuntimeRestore\RuntimeRecoverySchemaV35::tables($f->p),'backup includes exact current inventory');
$auto=array_column($f->db->query('SELECT TABLE_NAME FROM information_schema.TABLES WHERE TABLE_SCHEMA=DATABASE() AND AUTO_INCREMENT IS NOT NULL ORDER BY BINARY TABLE_NAME')->fetch_all(MYSQLI_ASSOC),'TABLE_NAME');assertSameValue($auto,FMonitor2\RuntimeRestore\RuntimeRecoverySchemaV35::autoIncrement($f->p),'backup AI inventory');
