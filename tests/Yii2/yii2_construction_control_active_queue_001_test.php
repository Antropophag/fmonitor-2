...
try {
    ...
    assertSameValue(true,str_contains($first['body'].$second['body'],'data-has-document="true"')&&str_contains($first['body'].$second['body'],'data-shlz-icon="folder-file-open"'),'INTENDED_RED queue exposes document action with inline shlz icon');
    assertSameValue(true,str_contains($first['body'].$second['body'],'class="shlz-button shlz-button--primary shlz-button--icon fm2-control-document-action"')&&str_contains($first['body'].$second['body'],'href="https://bitrix24public.com/control-4512"'),'INTENDED_RED queue action exposes exact safe URL');
    ...
    try{$ambiguousQueue=$fixture->page('/pilot/construction-control?ownership=all&completed=1&page=2');$ambiguousChecklist=$fixture->page();assertSameValue(true,str_contains($ambiguousQueue['body'],'data-object-id="4512"')&&str_contains($ambiguousQueue['body'],'Зав. № CONTROL-4512'),'ambiguity queue retains target identity');foreach([$ambiguousQueue['body'],$ambiguousChecklist['body']]as$body){assertSameValue(false,str_contains($body,'https://bitrix24public.com/control-4512')||str_contains($body,'https://bitrix24public.com/control-ambiguous'),'ambiguous order publishes no arbitrary URL');}assertSameValue(true,str_contains($ambiguousQueue['body'],'fm2-control-document-action')&&str_contains($ambiguousQueue['body'],'disabled'),'ambiguous queue exposes disabled document action');}
    ...
    try{$unavailableQueue=$fixture->page('/pilot/construction-control?ownership=all&completed=1');$unavailableChecklist=$fixture->page();assertSameValue([200,200,true,false,false],[$unavailableQueue['status'],$unavailableChecklist['status'],str_contains($unavailableQueue['body'],'fm2-control-document-action')&&str_contains($unavailableQueue['body'],'disabled'),str_contains($unavailableQueue['body'],'bitrix24public.com/control-4512'),str_contains($unavailableChecklist['body'],'fm2-control-document-button')],'document outage exposes disabled queue action and is fail-soft');}
