...
try {
 ...
 $queue2=$f->page('/pilot/construction-control?page=2');assertSameValue(200,$queue2['status'],'queue second page');assertSameValue(1,preg_match_all('/data-control-row\b/',$queue2['body']),'one remaining queue row');assertSameValue(true,str_contains($queue2['body'],'data-object-id="4513"'),'queue orders by device activity, not server receipt');assertSameValue(true,str_contains($queue2['body'],'page=1'),'queue previous page');
