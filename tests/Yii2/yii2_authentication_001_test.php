...
try{
    ...
    $cookies=[];$guestRoles=yaRequest($server,'GET','/pilot/admin/roles',[],$cookies);assertSameValue(303,$guestRoles['status'],'protected GET stores return target');$anonymousId=$cookies['fm2yii_'.$server['port']]??null;$login=yaLogin($server,$fixture,$cookies,$fixture->password);
    assertSameValue([303,'/pilot/admin/roles'],[$login['status'],$login['headers']['location'][0]??null],'successful two-step login');
    assertSameValue(true,isset($cookies['fm2yii_'.$server['port']]),'new isolated Yii cookie namespace');
    assertSameValue(false,$anonymousId===$cookies['fm2yii_'.$server['port']],'successful login rotates session ID');
