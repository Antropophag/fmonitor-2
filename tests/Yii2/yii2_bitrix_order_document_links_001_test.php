...
try{
 ...
 $cookies=[];assertSameValue(303,$f->login($cookies,95)['status'],'objects.read actor login');$card=$f->request('GET','/pilot/objects/4512',[],$cookies);assertSameValue(200,$card['status'],'authorized card');foreach(['Техническая документация','https://tenant.bitrix24.test/docs/501','&lt;Тех &amp; 0012.03&gt;']as$text)assertSameValue(true,str_contains($card['body'],$text),'INTENDED_RED card content '.$text);assertSameValue(false,str_contains($card['body'],'<Тех & 0012.03>'),'source title escaped');assertSameValue(true,preg_match('#href="https://tenant\.bitrix24\.test/docs/501"[^>]*rel="noopener noreferrer"#',$card['body'])===1,'safe external relation');
 ...
 $f->db->query("UPDATE {$p}fm_maintable SET zavnumber='NO-DOC' WHERE id=4512");$empty=$f->request('GET','/pilot/objects/4512',[],$cookies);assertSameValue([200,true],[$empty['status'],str_contains($empty['body'],'Техническая документация не найдена')],'empty section only');
