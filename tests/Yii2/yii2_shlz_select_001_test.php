...
foreach(['shlz-select-root','shlz-select__trigger','role="combobox"','role="listbox"','role="option"','aria-expanded','aria-selected','type="hidden"']as$needle)assertSameValue(true,str_contains($support,$needle),'INTENDED_RED shared SHLZ Select contract '.$needle);
foreach(['objects.php','installers.php','users.php','otiz.php','object-card.php']as$file){$source=(string)file_get_contents($root.'/app/YiiRuntime/Views/'.$file);assertSameValue(false,str_contains($source,'Html::dropDownList'),'INTENDED_RED no native-only select '.$file);assertSameValue(true,str_contains($source,'ViewSupport::select'),'INTENDED_RED shared select renderer '.$file);}
