...
try {
 ...
 $f->checklist(6105,8,42,'completion_retracted');$row=$owner->read(9101,'451205','',1)['objects'][0];assertSameValue('Монтажные работы',$row['status'],'retracted weight reduces displayed progress');assertSameValue([451206],array_map('intval',array_column($owner->read(9101,'','document_closeout',1)['objects'],'id')),'current-state closeout filter excludes retracted item');assertSameValue([451201,451205,451208],array_map('intval',array_column($owner->read(9101,'','installation',1)['objects'],'id')),'current-state installation filter includes retracted item');
