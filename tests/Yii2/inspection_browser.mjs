...
try {
 ...
 const queueRows=page.locator('[data-control-row]');check(JSON.stringify(await queueRows.evaluateAll(rows=>rows.map(r=>Number(r.dataset.objectId))))===JSON.stringify([4513,4512,4514]),'construction-control queue sorts active before completed');
 check(await page.locator('[data-control-row][data-object-id="4513"]').getAttribute('data-engineer-id')==='0','missing assignment has no legacy engineer fallback');
 check(await page.locator('[data-control-row][data-object-id="4514"]').getAttribute('data-completed')==='true','completed documentary row is available to client filter');
 check(await page.locator('[data-control-row][data-object-id="4512"]').getAttribute('data-engineer-id')==='73','standalone current engineer is authoritative');
 check(await page.locator('[data-control-row]:visible').count()===1,'mine filter excludes colleague and hides completed by default');await page.locator('[name=ownership][value=all]').check();check(await page.locator('[data-control-row]:visible').count()===2,'all includes active colleague');await page.locator('[data-show-completed]').check();check(await page.locator('[data-control-row]:visible').count()===3,'completed toggle reveals documentary completion');await page.locator('[data-control-search]').fill('QUEUE-4514');check(await page.locator('[data-control-row]:visible').count()===1,'completed documentary row is searchable when enabled');await page.locator('[data-control-search]').fill('QUEUE-4513');check(await page.locator('[data-control-row]:visible').count()===1,'literal search among active rows');await page.locator('[data-control-search]').fill('NO-SUCH-OBJECT');check(await page.locator('[data-control-empty]').isVisible(),'empty filter state');await page.locator('[data-clear-filters]').click();
