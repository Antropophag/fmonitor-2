...
try {
 ...
 await page.waitForFunction(async()=>await new Promise(resolve=>{const r=indexedDB.open('fmonitor2-fast-pilot',2);r.onsuccess=()=>{const db=r.result,q=db.transaction('operations').objectStore('operations').getAll();q.onsuccess=()=>{resolve(q.result.some(x=>x.itemId===29));db.close();};};}));
 ...
 for(let i=0;i<3;i++){await page.waitForFunction(async({id,status})=>await new Promise(resolve=>{const r=indexedDB.open('fmonitor2-fast-pilot',2);r.onsuccess=()=>{const db=r.result,q=db.transaction('operations').objectStore('operations').getAll();q.onsuccess=()=>{resolve(q.result.some(x=>x.clientOperationId===id&&x.status===status));db.close();};};}),{id:offlineId,status:['conflict','rejected','retryable_error'][i]});check((await localRows()).rows.some(r=>r.clientOperationId===offlineId),'failed intention retained');await page.locator('[data-sync-now]').click();}
 await page.waitForFunction(async id=>await new Promise(resolve=>{const r=indexedDB.open('fmonitor2-fast-pilot',2);r.onsuccess=()=>{const db=r.result,q=db.transaction('operations').objectStore('operations').getAll();q.onsuccess=()=>{resolve(q.result.some(x=>x.clientOperationId===id&&x.status==='accepted'));db.close();};};}),offlineId);
 ...
 await page.waitForFunction(async()=>await new Promise(resolve=>{const r=indexedDB.open('fmonitor2-fast-pilot',2);r.onsuccess=()=>{const db=r.result,q=db.transaction('photoBlobs').objectStore('photoBlobs').count();q.onsuccess=()=>{resolve(q.result>0);db.close();};};}));
 await context.setOffline(false);await page.waitForFunction(async()=>await new Promise(resolve=>{const r=indexedDB.open('fmonitor2-fast-pilot',2);r.onsuccess=()=>{const db=r.result,q=db.transaction('operations').objectStore('operations').getAll();q.onsuccess=()=>{resolve(q.result.some(x=>x.type==='section_completed'&&x.status==='accepted'));db.close();};};}),{},{timeout:30000});
