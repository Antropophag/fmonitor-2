...
try {
 ...
 const queueRows=page.locator('[data-control-row]');check(JSON.stringify(await queueRows.evaluateAll(rows=>rows.map(r=>Number(r.dataset.objectId))))===JSON.stringify([4513,4512]),'active construction-control queue sorted no activity first');
 ...
 check(await page.locator('[data-control-row][data-object-id="4514"]').count()===0,'documentary closeout is absent from server queue');
 ...
 check(await page.locator('[data-control-row]:visible').count()===1,'mine filter excludes other engineer');await page.locator('[name=ownership][value=all]').check();check(await page.locator('[data-control-row]:visible').count()===2,'all includes colleague');await page.locator('[data-show-completed]').check();check(await page.locator('[data-control-row]:visible').count()===2,'completed toggle cannot resurrect documentary closeout');await page.locator('[data-control-search]').fill('QUEUE-4514');check(await page.locator('[data-control-row]:visible').count()===0,'excluded documentary row is not searchable');check(await page.locator('[data-control-empty]').isVisible(),'excluded search has empty state');await page.locator('[data-control-search]').fill('QUEUE-4513');check(await page.locator('[data-control-row]:visible').count()===1,'literal search among active rows');await page.locator('[data-control-search]').fill('NO-SUCH-OBJECT');check(await page.locator('[data-control-empty]').isVisible(),'empty filter state');await page.locator('[data-clear-filters]').click();
