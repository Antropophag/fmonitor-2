...
try{
    ...
    foreach(['jobs/worker','jobs/scheduler','jobs/health']as$route){[$exit,$out,$err]=yjcRun(['php','bin/yii',$route,'--interactive=0'],$env,$root);assertSameValue(70,$exit,$route.' ignores missing demo manifest and reaches Jobs owner');}
    ...
    foreach([['preparing','pilot_'],['ready',''],['ready','bad-prefix!']]as[$state,$prefix]){file_put_contents($tmp.'/state/pilot-demo/one/active.json',json_encode(['state'=>$state,'processPrefix'=>$prefix],JSON_THROW_ON_ERROR));[$exit,$out,$err]=yjcRun(['php','bin/yii','jobs/health','--interactive=0'],$env,$root);assertSameValue(70,$exit,'legacy manifest state/prefix cannot override canonical environment');}
    file_put_contents($tmp.'/state/pilot-demo/one/active.json',json_encode(['state'=>'ready','processPrefix'=>'pilot_'],JSON_THROW_ON_ERROR));mkdir($tmp.'/state/pilot-demo/two',0700);file_put_contents($tmp.'/state/pilot-demo/two/active.json',json_encode(['state'=>'ready','processPrefix'=>'other_'],JSON_THROW_ON_ERROR));[$exit,$out,$err]=yjcRun(['php','bin/yii','jobs/scheduler','--interactive=0'],$env,$root);assertSameValue(70,$exit,'ambiguous legacy manifests are irrelevant');@unlink($tmp.'/state/pilot-demo/two/active.json');@rmdir($tmp.'/state/pilot-demo/two');
