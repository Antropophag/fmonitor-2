...
try {
 for(const [name,width]of[['desktop',1280],['mobile',360]]){
  ...
  const source=name==='desktop'?'/pilot/calendar':'/pilot/otiz/payments';
  await page.goto(input.url+'/pilot/feedback?from='+encodeURIComponent(source));
  ...
  assert(await page.locator('input[name="pagePath"]').inputValue()===source,'current source context '+source);
  ...
  const back=page.locator(`a[href="${source}"]`);assert(await back.count()>0,'confirmation return '+source);
  await c.close();
  ...
  const report=admin.locator('section.fm2-feedback-surface').filter({hasText:'Браузер '+name});assert(await report.getByText('Исходный экран',{exact:false}).count()>0,'operator sees source label');assert(await report.getByText(source,{exact:false}).count()>0,'operator sees persisted source');assert(await report.getByText('Сборка',{exact:false}).count()>0,'operator sees build label');assert(await report.getByText(input.build,{exact:false}).count()>0,'operator sees persisted full build');
