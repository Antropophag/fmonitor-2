...
try {
 for(const [name,width]of[['desktop',1280],['mobile',360]]){
  ...
  await admin.goto(input.url+'/pilot/admin/feedback');await admin.getByText('Проверено '+name,{exact:true}).waitFor();await ac.close();
 }
