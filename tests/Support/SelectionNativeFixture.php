...
final class SelectionNativeFixture
{
    ...
    public function __construct(public readonly string $prefix = '', ?\Closure $beforeRegistry = null)
    {
        ...
        try {
            ...
            \assertSameValue(['applied'=>true,'schemaVersion'=>27,'tablesCreated'=>[$prefix.'fm2_control_engineer_assignments']],I\ControlEngineerAssignmentDefinitionSchemaMigration::apply($this->db,$prefix),'approved current engineer schema');
            ...
            foreach ([[18,'Сотрудник теста'],[73,'Инженер теста']] as [$id,$name]) {
                $this->schema->insert($prefix.'fm2_pilot_users',['user_id'=>$id,'full_name'=>$name,'email'=>"test$id@example.invalid",'status'=>1,'activation_state'=>'active','source_updated_at'=>'2026-09-01T06:00:00Z']);
            }
            foreach ([[1,'fkr_operator'],[2,'construction_control_engineer']] as [$id,$code]) {
                $this->schema->insert($prefix.'fm2_pilot_roles',['role_id'=>$id,'code'=>$code,'name'=>$code,'description'=>'synthetic','status'=>1,'source_updated_at'=>'2026-09-01T06:00:00Z']);
            }
            ...
            \assertSameValue(true,I\AssignmentOrderSelectionSchemaMigration::isReady($this->db,$prefix),'ready before target action');
    ...
    public function app(): C\AssignmentOrderCompositionApplication
    {
        ...
        $p=C\AssignmentOrderCompositionNativeVerificationFactory::dependencies($this->db,fn()=>$this->schema->source->connect($this->schema->source->name),$this->prefix);
    ...
    {
        $rows=[];
        foreach ($this->schema->state() as $table=>$state) $rows[str_starts_with($table,$this->prefix)?substr($table,strlen($this->prefix)):$table]=$state[1];
        return $rows;
    }
