...
function execute(array $argv, array $environment, bool $allowFailure=false): array
{
    $pipes=[];$process=proc_open($argv,[0=>['file','/dev/null','r'],1=>['pipe','w'],2=>['pipe','w']],$pipes,null,$environment);
...
function realRun(string $packagePath,string $evidence,array $opts): never
{
    ...
    execute([...$compose,'up','--detach','--wait','db'],$env);execute([...$compose,'--profile','deployment','run','--rm','prepare'],$env);execute([...$compose,'--profile','deployment','run','--rm','migrate'],$env);
