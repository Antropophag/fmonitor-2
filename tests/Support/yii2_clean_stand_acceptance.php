...
function deriveFacts(array $driver, array $manifest, array $auth): array
{
    ...
    if (($db['schemaBefore']??null)!==[] || ($db['schemaVersion']??null)!==25 || count($db['migrationLedger']??[])!==25) emit('PROVISIONING_INVALID');
...
function validateHttpContext(array $context): array
{
    ...
    foreach($requests as $request){
        ...
        if(isset($request['factAssertions'])&&!is_array($request['factAssertions']))emit('HTTP_CONTEXT_INVALID');foreach(($request['factAssertions']??[])as$assertion){$assertionKeys=is_array($assertion)?array_keys($assertion):[];sort($assertionKeys);if($assertionKeys!==['expectation','sql']||!in_array($assertion['expectation']??null,['append-one','stable'],true)||!is_string($assertion['sql']??null)||preg_match('/^\s*SELECT\b[^;]*;?\s*$/isD',$assertion['sql'])!==1)emit('HTTP_CONTEXT_INVALID');}
    ...
    foreach(($context['validationQueries']??[])as$query){$keys=is_array($query)?array_keys($query):[];sort($keys);if($keys!==['eventually','expectedSha256','sql']||!is_string($query['sql']??null)||preg_match('/^\s*SELECT\b[^;]*;?\s*$/isD',$query['sql'])!==1||!preg_match('/^[0-9a-f]{64}$/',(string)($query['expectedSha256']??'')))emit('HTTP_CONTEXT_INVALID');$eventually=$query['eventually']??null;$eventuallyKeys=is_array($eventually)?array_keys($eventually):[];sort($eventuallyKeys);if($eventuallyKeys!==['intervalMilliseconds','timeoutSeconds']||!is_int($eventually['timeoutSeconds'])||$eventually['timeoutSeconds']<1||$eventually['timeoutSeconds']>60||!is_int($eventually['intervalMilliseconds'])||$eventually['intervalMilliseconds']<50||$eventually['intervalMilliseconds']>1000)emit('HTTP_CONTEXT_INVALID');}
    return $summary;
}
...
function realRun(string $packagePath,string $evidence,array $opts): never
{
    ...
    foreach(($context['httpRequests']??[]) as $request){
        $factBefore=[];foreach(($request['factAssertions']??[])as$assertion){[, $rows]=execute([...$compose,'exec','-T','-e','MYSQL_PWD','db','mariadb','-N','-B','-u',$package['runtime']['acceptanceDatabaseUser'],$package['names']['database']],array_merge($env,['MYSQL_PWD'=>$acceptancePassword]),false,$assertion['sql']);$factBefore[]=$rows;}
        ...
        foreach(($request['factAssertions']??[])as$index=>$assertion){[, $after]=execute([...$compose,'exec','-T','-e','MYSQL_PWD','db','mariadb','-N','-B','-u',$package['runtime']['acceptanceDatabaseUser'],$package['names']['database']],array_merge($env,['MYSQL_PWD'=>$acceptancePassword]),false,$assertion['sql']);if($assertion['expectation']==='stable'){if(!hash_equals(hash('sha256',$factBefore[$index]),hash('sha256',$after)))emit('GOLDEN_FACT_INVALID',[],70);}else{$before=trim($factBefore[$index]);$current=trim($after);if(!ctype_digit($before)||!ctype_digit($current)||(int)$current!==(int)$before+1)emit('GOLDEN_FACT_INVALID',[],70);}}
    }
    foreach(($context['validationQueries']??[]) as $query){$deadline=hrtime(true)+$query['eventually']['timeoutSeconds']*1_000_000_000;do{[, $rows]=execute([...$compose,'exec','-T','-e','MYSQL_PWD','db','mariadb','-N','-B','-u',$package['runtime']['acceptanceDatabaseUser'],$package['names']['database']],array_merge($env,['MYSQL_PWD'=>$acceptancePassword]),false,$query['sql']);if(hash_equals($query['expectedSha256'],hash('sha256',$rows)))break;usleep($query['eventually']['intervalMilliseconds']*1000);}while(hrtime(true)<$deadline);if(!hash_equals($query['expectedSha256'],hash('sha256',$rows)))emit('ACCEPTANCE_FACT_INVALID',[],70);[, $stable]=execute([...$compose,'exec','-T','-e','MYSQL_PWD','db','mariadb','-N','-B','-u',$package['runtime']['acceptanceDatabaseUser'],$package['names']['database']],array_merge($env,['MYSQL_PWD'=>$acceptancePassword]),false,$query['sql']);if(!hash_equals(hash('sha256',$rows),hash('sha256',$stable)))emit('ACCEPTANCE_FACT_INVALID',[],70);}
    execute([...$compose,'exec','-T','jobs-worker','php','bin/yii','jobs/health','--interactive=0'],$env);
