...
final class BitrixDeliveryFixture
{
    ...
    public function client(array $overrides=[]):void
    {
        ...
        $path=$this->root.'/client-'.$this->sequence.'.json';file_put_contents($path,json_encode(['config'=>$this->config,'output'=>$this->root.'/records-'.$this->sequence],JSON_THROW_ON_ERROR));chmod($path,0600);
    ...
    {
        $connections=$this->connections();$this->step++;if(fwrite($this->worker['pipes'][0],"fetch\n")!==6)throw new \TestFailure('Worker command');$this->wait('done'.$this->step,$this->config['deadlineSeconds']+3);
