...
final class NativePilotSessionFilesystem implements PilotSessionFilesystemPrimitives {
    ...
    public function flock(PilotSessionFileHandle $h,int $op):PilotSessionPrimitiveResult{$f=$this->fault('flock',$this->handleArtifact($h));if($f)return$f;$blocked=0;if(@flock($this->resource($h),$op,$blocked))return PilotSessionPrimitiveResult::ok(null);return$blocked===1?PilotSessionPrimitiveResult::wouldBlock():PilotSessionPrimitiveResult::nativeFalse();}
