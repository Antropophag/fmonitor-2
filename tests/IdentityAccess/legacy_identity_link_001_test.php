...
try {
    ...
    $f->db->query("INSERT IGNORE INTO {$p}users(id,name,email,role_id,status) VALUES(7301,'Legacy Engineer','legacy.engineer@example.test',42,1),(7302,'Same Name','duplicate@example.test',42,1),(7303,'Inactive Legacy','inactive@example.test',42,0)");
    ...
    foreach(['password','hash','session','users_rights'] as $forbidden)assertSameValue(false,str_contains(strtolower(json_encode($rows,JSON_THROW_ON_ERROR)),$forbidden),'no credential inheritance '.$forbidden);
