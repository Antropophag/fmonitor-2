...
3. Продолжить основной сценарий объекта `444` с уже назначенным Ветровым Константином: `карточка → распоряжение только на монтажников → номер 1С ДО → открытие работ`.
