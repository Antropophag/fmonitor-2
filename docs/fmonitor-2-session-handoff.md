...
Текущий экран показывает 8 разделов и 42 работы, общий/секционный прогресс, фото раздела, offline/pending состояния и доступность только закреплённому инженеру стройконтроля. Для остальных пользователей экран read-only. Проверены desktop и mobile на объекте `444`.
