...
- First exact-source CI run `34967569646` failed during setup in `fast`, `unit`, `governance`, both integration shards and `e2e`: every primary failure reported the same missing `tests/Yii2/yii2_main_navigation_001_test.php` entry in `tools/verification/categories.json`; `verify` failed only as the aggregate. No `REGRESSION_FAILURE` was reported. The missing normal test registration was added, then the focused inventory and navigation regressions passed locally.
- Second exact-source CI run `34968274150` passed `unit`, `e2e` and integration shard 2. `fast` and `governance` exposed the remaining frozen-roster hash update required by the same test registration; integration shard 1 separately reported the pre-existing out-of-scope `tests/InstallationProcess/inspection_item_complete_001_mariadb_test.php`. `verify` failed only as the aggregate. The roster hash was updated without changing verification behavior or policy; the unrelated integration test was not modified.
