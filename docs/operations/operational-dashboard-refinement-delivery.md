...
- CI setup correction: run `35614232072` and its one allowed same-source retry both failed before tests because `uv python install` received repeated external `504 Gateway Timeout`; shared setup now prefers the runner's already-installed exact `PYTHON_VERSION` and downloads through `uv` only when the exact version is absent.
