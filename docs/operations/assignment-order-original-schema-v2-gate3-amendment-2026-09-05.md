...
- The next diagnostic run exposed a second fixture-only defect: `LIKE '%capability%in%'` also matched the independent engineer-position CHECK because `position_snapshot` contains the letters `in` after `capability`. The fixture now reads all CHECK clauses and selects the sole normalized clause that starts with `capabilityin`, matching the already-approved capability classifier semantics.
