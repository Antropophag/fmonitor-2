...
Gate 3 и Gate 5 APPROVED; focused checks GREEN. Фактические commit/PR/CI проверять через `python3 tools/delivery/harness.py state`. Один full exact-source CI требуется до merge-ready; stand/deployment и общий #76 этим срезом не закрываются. Параллельный OTIZ checkout и его WIP не затрагиваются.
