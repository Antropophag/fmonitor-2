...
не сбрасывайте его ради подготовки второй машины или checkout. Актуальный
локальный Yii2 runtime использует `deploy/runtime/compose.yaml`; для каждого
одновременного контура задайте отдельные Compose project name и порт.
...
```bash
cp .env.example .env
# Отредактируйте все значения replace_me в .env.
make up
```

Откройте <http://127.0.0.1:8093/>. `make up` собирает актуальный image, поднимает
MariaDB, создаёт exact DML runtime account, выполняет prepare и migrations,
создаёт initial owner и запускает web только после runtime checks. Повторный
запуск с тем же `.env` не удаляет database, sessions или artifacts. Пароли и
ключи не коммитьте и не публикуйте в логах.
...
`make reset` удаляет volumes только явно выбранного `fm2-local-*` project.
`make down` останавливает сервисы, но сохраняет данные. Production runbook
остаётся отдельным низкоуровневым operational path:
[`production-runtime-runbook.md`](operations/production-runtime-runbook.md).
Выполняйте reset только для одноразового контура после резервной копии. Правила сохранения и
...
Exact-source GitHub CI выполняет проверки, выбранные planner/policy. Когда
выбран полный matrix, его каноническая команда — `make test`.

В delivery-задачах эту полную команду не запускают локально без нового прямого
поручения владельца: локально выполняют focused/fast checks. Полная команда
поднимает одноразовую MariaDB из
`compose.test.yaml`, по умолчанию
