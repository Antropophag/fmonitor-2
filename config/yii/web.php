<?php
declare(strict_types=1);

use FMonitor2\YiiRuntime\{ReliableSession, SafeErrorHandler, WebResponse};
use FMonitor2\IdentityAccess\{MariaDbYiiLocalIdentityStore, YiiCanonicalAccessChecker, YiiLocalIdentity, YiiUserAccess};
use yii\helpers\ArrayHelper;

return ArrayHelper::merge(require __DIR__ . '/common.php', [
    'controllerNamespace' => 'FMonitor2\\YiiRuntime\\Controllers',
    'on beforeRequest' => static function (): void {
        $request = Yii::$app->request;
        if (defined('FMONITOR_PRODUCTION_WEB_CUTOVER')
            && !$request->getIsGet() && !$request->getIsHead()
            && Yii::$app->user->isGuest && !$request->validateCsrfToken()) {
            throw new yii\web\BadRequestHttpException();
        }
    },
    'components' => [
        'request' => ['cookieValidationKey' => getenv('FMONITOR_YII_COOKIE_VALIDATION_KEY') ?: '',
            'scriptUrl' => '/runtime.php', 'baseUrl' => '', 'enableCsrfCookie' => false],
        'localIdentity' => [
            'class' => MariaDbYiiLocalIdentityStore::class,
            'tablePrefix' => getenv('FMONITOR_PROCESS_TABLE_PREFIX') ?: '',
            'identityKey' => getenv('FMONITOR_YII_IDENTITY_KEY') ?: '',
        ],
        'canonicalAccess' => ['class' => YiiCanonicalAccessChecker::class],
        'userAccess' => ['class'=>YiiUserAccess::class,'tablePrefix'=>getenv('FMONITOR_PROCESS_TABLE_PREFIX')?:''],
        'session' => [
            'class' => ReliableSession::class,
            'name' => (static function(): string {$name=getenv('FMONITOR_YII_SESSION_COOKIE');return is_string($name)&&preg_match('/^[A-Za-z][A-Za-z0-9_]{1,63}$/D',$name)===1?$name:'fm2yii';})(),
            'savePath' => getenv('FMONITOR_YII_SESSION_PATH') ?: (getenv('FMONITOR_SESSION_STATE_ROOT') ?: '/home/fmonitor/.local/state/fmonitor2') . '/yii-sessions',
            'timeout' => 604800,
            'useStrictMode' => true,
            'cookieParams' => [
                'lifetime' => 604800, 'path' => '/pilot', 'httponly' => true,
                'secure' => getenv('FMONITOR_TRUSTED_REQUEST_SCHEME') === 'https',
                'sameSite' => yii\web\Cookie::SAME_SITE_STRICT,
            ],
        ],
        'user' => [
            'identityClass' => YiiLocalIdentity::class,
            'enableAutoLogin' => false,
            'enableSession' => true,
            'loginUrl' => ['/auth/login'],
            'accessChecker' => 'canonicalAccess',
        ],
        'response' => [
            'on beforeSend' => static function ($event): void {
                WebResponse::closeAndSecure($event);
                if ($event->sender->statusCode === 503) {
                    $event->sender->headers->set('Retry-After', '60');
                }
            },
            'on afterPrepare' => WebResponse::head(...),
        ],
        'errorHandler' => ['class' => SafeErrorHandler::class],
        'urlManager' => [
            'cache' => false,
            'enablePrettyUrl' => true,
            'showScriptName' => false,
            'enableStrictParsing' => true,
            'rules' => [
                '' => 'site/index',
                'health/live' => 'health/live',
                'health/ready' => 'health/ready',
                'GET,HEAD pilot/login' => 'auth/login',
                'POST pilot/login' => 'auth/login',
                'GET,HEAD pilot/otiz/login' => 'auth/login',
                'POST pilot/otiz/login' => 'auth/login',
                'POST pilot/logout' => 'auth/logout',
                'GET pilot/logout' => 'auth/logout',
                'GET,HEAD pilot/objects' => 'object-queue/index',
                'GET,HEAD pilot/completion-register' => 'completion-register/index',
                'pilot/completion-register' => 'completion-register/index',
                'GET,HEAD pilot/dashboard' => 'dashboard/index',
                'GET,HEAD pilot/dashboard/installers/observations/<date:\\d{4}-\\d{2}-\\d{2}>/<series:[a-z_]+>' => 'dashboard/observation',
                'GET,HEAD pilot/dashboard/installers/forecast/<weekStart:\\d{4}-\\d{2}-\\d{2}>/<bucket:[a-z]+>' => 'dashboard/forecast',
                'GET,HEAD pilot/calendar<slash:\/?>' => 'calendar/index',
                ...(require __DIR__ . '/feedback-routes.php'),
                'GET,HEAD pilot/installers/<tabId:[1-9][0-9]*>' => 'installer-utilization/view',
                'pilot/installers/<tabId:[1-9][0-9]*>' => 'installer-utilization/view',
                'GET,HEAD pilot/installers' => 'installer-directory/index',
                'pilot/installers' => 'installer-directory/index',
                'GET,HEAD pilot/construction-control' => 'checklist/queue',
                'POST pilot/construction-control/objects/<id:[1-9]\\d*>/inspection-plan' => 'checklist/inspection-plan',
                'pilot/construction-control/objects/<id:[1-9]\\d*>/inspection-plan' => 'checklist/inspection-plan',
                'GET,HEAD pilot/objects/<id:[1-9]\\d*>/checklist' => 'checklist/view',
                'GET,HEAD pilot/construction-control/objects/<id:[1-9]\\d*>/checklist' => 'checklist/control',
                'GET,HEAD pilot/construction-control/objects/<id:[1-9]\\d*>/sync-context' => 'checklist/context',
                'POST pilot/objects/<id:[1-9]\\d*>/checklist/operations' => 'checklist/operation',
                'POST pilot/construction-control/objects/<id:[1-9]\\d*>/checklist/operations' => 'checklist/operation',
                'POST pilot/objects/<id:[1-9]\\d*>/checklist/photos' => 'checklist/photo',
                'POST pilot/construction-control/objects/<id:[1-9]\\d*>/checklist/photos' => 'checklist/photo',
                'GET,HEAD pilot/objects/<id:[1-9]\\d*>/checklist/photos/<photoId:[1-9]\\d*>' => 'checklist/photo-read',
                'pilot/objects/<id:[1-9]\\d*>/checklist' => 'object-card/method',
                'pilot/construction-control/objects/<id:[1-9]\\d*>/checklist' => 'object-card/method',
                'pilot/construction-control/objects/<id:[1-9]\\d*>/sync-context' => 'object-card/method',
                'pilot/objects/<id:[1-9]\\d*>/checklist/operations' => 'checklist/operation',
                'pilot/construction-control/objects/<id:[1-9]\\d*>/checklist/operations' => 'checklist/operation',
                'pilot/objects/<id:[1-9]\\d*>/checklist/photos' => 'checklist/photo',
                'pilot/construction-control/objects/<id:[1-9]\\d*>/checklist/photos' => 'checklist/photo',
                'pilot/objects/<id:[1-9]\\d*>/checklist/photos/<photoId:[1-9]\\d*>' => 'checklist/photo-read',
                'GET,HEAD pilot/objects/<id:[1-9]\\d*>' => 'object-card/view',
                'POST pilot/objects/<id:[1-9]\\d*>/details' => 'object-card/details',
                'POST pilot/objects/<id:[1-9]\\d*>/control-engineer-assignment' => 'object-card/assignment',
                ...(require __DIR__ . '/deadline-certificate-routes.php'),
                'POST pilot/objects/<id:[1-9]\\d*>/completion' => 'completion/index',
                'pilot/objects/<id:[1-9]\\d*>/completion' => 'completion/method',
                'GET,HEAD pilot/objects/<id:[1-9]\\d*>/assignment-order/prepare' => 'object-card/prepare',
                'POST pilot/objects/<id:[1-9]\\d*>/assignment-order/prepare' => 'object-card/gone',
                'GET,HEAD,POST pilot/objects/<id:[1-9]\\d*>/assignment-order/selection' => 'selection/index',
                'GET,HEAD pilot/objects/<id:[1-9]\\d*>/assignment-order/installers' => 'selection/installers',
                'POST pilot/objects/<id:[1-9]\\d*>/assignment-orders/<orderId:[1-9]\\d*>/template' => 'assignment-order-template/generate',
                'GET,HEAD pilot/objects/<id:[1-9]\\d*>/assignment-orders/<orderId:[1-9]\\d*>/originals/submit' => 'original/form',
                'POST pilot/objects/<id:[1-9]\\d*>/assignment-orders/<orderId:[1-9]\\d*>/originals' => 'original/upload',
                'GET,HEAD pilot/objects/<id:[1-9]\\d*>/assignment-orders/<orderId:[1-9]\\d*>/originals/history' => 'original-history/index',
                'GET,HEAD pilot/objects/<id:[1-9]\\d*>/assignment-orders/<orderId:[1-9]\\d*>/originals/<revisionId:[A-Za-z0-9][A-Za-z0-9._:-]{0,79}>/download' => 'original-history/download',
                'GET,HEAD,POST pilot/objects/<id:[1-9]\\d*>/execution' => 'execution/index',
                'GET,HEAD,POST pilot/objects/<id:[1-9]\\d*>/assignment-orders/<orderId:[1-9]\\d*>/artifacts/<artifact:(?:order|appendix|signed_original)>' => 'object-card/gone',
                'GET,HEAD,POST pilot/objects/<id:[1-9]\\d*>/assignment-orders/<orderId:[1-9]\\d*>/registration' => 'object-card/gone',
                'GET,HEAD,POST pilot/objects/<id:[1-9]\\d*>/control-engineer' => 'object-card/gone',
                'GET,HEAD,POST pilot/objects/<id:[1-9]\\d*>/open' => 'object-card/gone',
                'pilot/objects/<id:[1-9]\\d*>' => 'object-card/method',
                'pilot/objects/<id:[1-9]\\d*>/details' => 'object-card/details-method',
                'pilot/objects/<id:[1-9]\\d*>/control-engineer-assignment' => 'object-card/method',
                'pilot/objects/<id:[1-9]\\d*>/assignment-order/selection' => 'selection/index-method',
                'pilot/objects/<id:[1-9]\\d*>/assignment-order/installers' => 'selection/installers-method',
                'pilot/objects/<id:[1-9]\\d*>/assignment-orders/<orderId:[1-9]\\d*>/template' => 'assignment-order-template/method',
                'pilot/objects/<id:[1-9]\\d*>/assignment-orders/<orderId:[1-9]\\d*>/originals/submit' => 'original/form-method',
                'pilot/objects/<id:[1-9]\\d*>/assignment-orders/<orderId:[1-9]\\d*>/originals' => 'original/upload-method',
                'pilot/objects/<id:[1-9]\\d*>/assignment-orders/<orderId:[1-9]\\d*>/originals/history' => 'original-history/index-method',
                'pilot/objects/<id:[1-9]\\d*>/assignment-orders/<orderId:[1-9]\\d*>/originals/<revisionId:[A-Za-z0-9][A-Za-z0-9._:-]{0,79}>/download' => 'original-history/download-method',
                'pilot/objects/<id:[1-9]\\d*>/execution' => 'execution/method',
                'GET,HEAD,POST pilot/objects/<id:[1-9]\\d*>/inspection-schedule' => 'object-queue/schedule',
                'GET pilot/admin/roles' => 'roles/index',
                'GET,HEAD pilot/admin/integrations' => 'integration-status/index',
                'pilot/admin/integrations' => 'site/not-found',
                'GET,HEAD pilot/admin/users' => 'user-access/index',
                'GET,HEAD pilot/users' => 'user-access/index',
                'GET,HEAD,POST pilot/admin/users/invite' => 'user-access/invite',
                'GET,HEAD,POST pilot/admin/users/<id:[1-9]\\d*>/invitation' => 'user-access/reissue',
                'GET,HEAD,POST pilot/admin/users/<id:[1-9]\\d*>/roles' => 'user-access/role',
                'GET,HEAD,POST pilot/admin/users/<id:[1-9]\\d*>/roles/<roleId:[1-9]\\d*>' => 'user-access/role',
                'GET,HEAD,POST pilot/admin/users/<id:[1-9]\\d*>/status' => 'user-access/status',
                'pilot/admin/users/<id:[1-9]\\d*>/legacy-link' => 'user-access/legacy-link',
                'GET,POST pilot/activate' => 'user-access/activate',
                'GET pilot/otiz' => 'otiz-settlement/index',
                'GET pilot/otiz/objects' => 'otiz-settlement/objects',
                'GET pilot/otiz/payments' => 'otiz-settlement/payments',
                'GET pilot/otiz/history' => 'otiz-settlement/history',
                'GET pilot/otiz/calculations/<id:\\d+>' => 'otiz-settlement-v2/calculation',
                'POST pilot/otiz/calculations' => 'otiz-settlement-v2/create',
                'POST pilot/otiz/admission' => 'otiz-admission/record',
                'POST pilot/otiz/calculations/<id:\\d+>/delete' => 'otiz-settlement-v2/delete',
                'POST pilot/otiz/calculations/<id:\\d+>/refresh' => 'otiz-settlement-v2/refresh',
                'POST pilot/otiz/calculations/<id:\\d+>/cancel' => 'otiz-settlement-v2/cancel',
                'POST pilot/otiz/calculations/<id:\\d+>/replace' => 'otiz-settlement-v2/replace',
                'POST pilot/otiz/calculations/<id:\\d+>/decisions' => 'otiz-settlement-v2/decision',
                'POST pilot/otiz/calculations/<id:\\d+>/deductions' => 'otiz-settlement-v2/deduction',
                'POST pilot/otiz/calculations/<id:\\d+>/accept' => 'otiz-settlement-v2/accept',
                'POST pilot/otiz/calculations/<id:\\d+>/payments' => 'otiz-settlement-v2/payment',
                'POST pilot/otiz/payments/<id:\\d+>/reverse' => 'otiz-settlement-v2/reverse',
                'GET pilot/otiz/calculations/<id:\\d+>/export.xlsx' => 'otiz-settlement-v2/export',
                'GET pilot/otiz/reconciliation' => 'otiz-evidence/reconciliation',
                'GET pilot/otiz/reconciliation/quarantine' => 'otiz-evidence/quarantine',
                'GET pilot/otiz/active-baselines' => 'otiz-evidence/active-baselines',
                'GET pilot/otiz/historical-replay' => 'otiz-evidence/historical-replay',
                'POST pilot/otiz/calculate' => 'otiz-settlement-v2/create',
                'POST pilot/otiz/snapshots/<id:\\d+>/accept' => 'otiz-settlement-v2/legacy-payment',
                'POST pilot/otiz/reconciliation/decisions' => 'otiz-evidence/reconciliation-decision',
                'POST pilot/otiz/reconciliation/quarantine/decisions' => 'otiz-evidence/quarantine-decision',
                'GET pilot/otiz/snapshots/<id:\\d+>' => 'otiz-settlement/snapshot',
                'GET pilot/otiz/snapshots/<id:\\d+>/export.xlsx' => 'otiz-settlement/export',
                'POST pilot/otiz/snapshots/<id:\\d+>/closures' => 'otiz-settlement-v2/legacy-payment',
                'POST pilot/otiz/snapshots/<id:\\d+>/payments/complete' => 'otiz-settlement-v2/legacy-payment',
                'POST pilot/otiz/closures/<id:[1-9]\\d*>/reverse' => 'otiz-settlement-v2/legacy-payment',
                ...(require __DIR__ . '/assets.php'),
            ],
        ],
    ],
]);
