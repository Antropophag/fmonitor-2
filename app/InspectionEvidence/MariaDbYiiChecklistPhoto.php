...
trait MariaDbYiiChecklistPhoto
{
    ...
    if(!is_string($b)||!in_array($mime,['image/jpeg','image/png','image/webp'],true)||!is_array($image)||($image['mime']??null)!==$mime||!is_int($size)||$size<1||$size>5242880||strlen($b)!==$size||!is_string($sha)||!preg_match('/^[a-f0-9]{64}$/D',$sha)||!hash_equals($sha,hash('sha256',$b))||!is_string($name)||$name===''||mb_strlen($name)>255||preg_match('/[\x00-\x1f\x7f]/u',$name)===1)return['status'=>'rejected'];
    ...
    if(!is_dir($dir)&&!@mkdir($dir,0700,true)&&!is_dir($dir))throw new ChecklistInfrastructureUnavailable();
    $storage=$sha.'.bin';
    $path=$dir.'/'.$storage;
    if(!is_file($path)&&file_put_contents($path,$b,LOCK_EX)!==$size)throw new ChecklistInfrastructureUnavailable();
    @chmod($path,0600);
