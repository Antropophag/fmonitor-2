...
trait MariaDbYiiChecklistRead
{
    ...
    $eventDate="COALESCE(NULLIF(JSON_UNQUOTE(JSON_EXTRACT(latest_event.payload_json,'$.newDate')),'null'),NULLIF(JSON_UNQUOTE(JSON_EXTRACT(latest_event.payload_json,'$.inspectionDate')),'null'),schedule.inspection_date)";
    $currentPlans="SELECT schedule.installation_case_id,MIN(schedule.id) schedule_id,MIN($eventDate) inspection_date,MIN(latest_event.event_version) inspection_version,COUNT(*) inspection_count FROM {$this->t('fm2_pilot_inspection_schedules')} schedule JOIN {$this->t('fm2_pilot_inspection_schedule_events')} latest_event ON latest_event.schedule_id=schedule.id AND latest_event.event_version=(SELECT MAX(newer.event_version) FROM {$this->t('fm2_pilot_inspection_schedule_events')} newer WHERE newer.schedule_id=schedule.id) WHERE latest_event.event_type<>'inspection_cancelled' AND $eventDate>=? GROUP BY schedule.installation_case_id";
    ...
    if((int)($this->one("SELECT MAX(COALESCE(inspection_plan.inspection_count,0)) n".$from." WHERE $predicate",$params)['n']??0)>1)throw new \RuntimeException('Multiple current inspection plans.');
