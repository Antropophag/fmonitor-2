...
trait MariaDbYiiChecklistRead
{
    ...
    $where=[$active];$params=[];$globalScope="NOT EXISTS(SELECT 1 FROM {$this->t('fm2_pilot_user_roles')} engineer_ur JOIN {$this->t('fm2_pilot_roles')} engineer_role ON engineer_role.role_id=engineer_ur.role_id AND engineer_role.status=1 WHERE engineer_ur.user_id=? AND BINARY engineer_role.code='control_engineer')";$assignedScope="EXISTS(SELECT 1 FROM {$this->t('fm2_control_engineer_assignments')} scope_assignment WHERE scope_assignment.installation_case_id=c.id AND scope_assignment.assignment_sequence=(SELECT MAX(scope_latest.assignment_sequence) FROM {$this->t('fm2_control_engineer_assignments')} scope_latest WHERE scope_latest.installation_case_id=c.id) AND scope_assignment.engineer_user_id=?)";$where[]="($globalScope OR $assignedScope)";$params[]=$actorId;$params[]=$actorId;
