...
final class JobHandlerRuntime
{
    ...
    {
        if($job['jobType']==='installer-utilization.capture'){
            ...
            try{if($documentSync!==null)$result=$documentSync();else{if($config===null)throw new \InvalidArgumentException();$db=MariaDbJobsConnection::open($config);$yii=new \yii\db\Connection(['dsn'=>'mysql:host='.$config->value('FMONITOR_DB_HOST').';port='.$config->port().';dbname='.$config->value('FMONITOR_DB_NAME'),'username'=>$config->value('FMONITOR_DB_USER'),'password'=>$config->value('FMONITOR_DB_PASSWORD'),'charset'=>'utf8mb4']);$yii->open();try{$date=(string)$job['payload']['captureDate'];$result=(new \FMonitor2\Workforce\MariaDbInstallerUtilizationObservations($db,$config->prefix(),new \FMonitor2\Workforce\MariaDbInstallerUtilization($yii,$config->prefix(),$config->value('FMONITOR_LEGACY_TABLE_PREFIX'))))->capture($date,$date.'T03:17:00+03:00');}finally{$yii->close();$db->close();}}return['status'=>'completed','result'=>$result];}catch(\InvalidArgumentException$e){throw$e;}catch(\Throwable){return['status'=>'retryable','failureCode'=>'INSTALLER_UTILIZATION_CAPTURE_FAILED'];}
        }
