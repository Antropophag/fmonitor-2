...
final class JobHandlerClaim
{
    public static function decode(string $input): array
    {
        ...
        if($job['jobType']==='installer-utilization.capture'&&$job['payloadVersion']===1){$payload=$job['payload'];if(!is_array($payload))throw new \InvalidArgumentException();JobValues::keys($payload,['captureDate','dueAtUtc']);JobValues::text($payload['captureDate'],'/^\d{4}-\d{2}-\d{2}$/D');JobValues::date($payload['dueAtUtc']);return$job;}
        ...
        if($job['jobType']==='erp.equipment-facts.sync'&&$job['payloadVersion']===1){$payload=$job['payload'];if(!is_array($payload))throw new \InvalidArgumentException();JobValues::keys($payload,['scheduleSlot']);JobValues::text($payload['scheduleSlot'],'/^\d{4}-\d{2}-\d{2}T\d{2}$/D');return$job;}
