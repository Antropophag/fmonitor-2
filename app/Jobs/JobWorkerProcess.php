...
final class JobWorkerProcess
{
    ...
    {
        $factory=$this->registry[$job['jobType']][$job['payloadVersion']]??null;if(!is_callable($factory))return['status'=>'permanent','failureCode'=>$job['jobType']==='erp.equipment-facts.sync'?'CONFIGURATION_INVALID':'JOB_HANDLER_UNAVAILABLE'];$command=$factory($job);if(!is_array($command)||$command===[])return['status'=>'permanent','failureCode'=>'JOB_HANDLER_UNAVAILABLE'];
        ...
        $out.=stream_get_contents($pipes[1]);$err.=stream_get_contents($pipes[2]);$exit=$this->reap($process,$pipes);if($exit===64&&$err===''&&$this->configurationInvalid($out))return['status'=>'permanent','failureCode'=>'CONFIGURATION_INVALID'];if($exit!==0||$err!==''||strlen($out)>65535)return['status'=>'retryable','failureCode'=>'JOB_HANDLER_FAILED'];try{$result=json_decode(trim($out),false,32,JSON_THROW_ON_ERROR);return$this->result($result);}catch(\Throwable){return['status'=>'retryable','failureCode'=>'JOB_HANDLER_FAILED'];}
    }
    private function configurationInvalid(string$output):bool
    {
        try{$value=json_decode(trim($output),true,4,JSON_THROW_ON_ERROR);return$value===['ok'=>false,'error'=>'CONFIGURATION_INVALID'];}catch(\Throwable){return false;}
    }
    ...
    private function result(mixed$result):array{if(!$result instanceof \stdClass)throw new \InvalidArgumentException();$values=get_object_vars($result);$status=$values['status']??null;if($status==='completed'){JobValues::keys($values,['status','result']);if(!$values['result'] instanceof \stdClass)throw new \InvalidArgumentException();$payload=json_decode(json_encode($values['result'],JSON_THROW_ON_ERROR),true,32,JSON_THROW_ON_ERROR);JobValues::json($payload);return['status'=>'completed','result'=>$payload];}if(in_array($status,['retryable','permanent'],true)){$keys=array_keys($values);sort($keys);if($keys!==['failureCode','status']&&$keys!==['failureCode','result','status'])throw new \InvalidArgumentException();JobValues::text($values['failureCode'],'/^[A-Z][A-Z0-9_]{2,79}$/D');if(isset($values['result'])){if(!$values['result'] instanceof \stdClass)throw new \InvalidArgumentException();JobValues::json(json_decode(json_encode($values['result'],JSON_THROW_ON_ERROR),true,32,JSON_THROW_ON_ERROR));}return['status'=>$status,'failureCode'=>$values['failureCode']];}throw new \InvalidArgumentException();}
