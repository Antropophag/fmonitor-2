...
final class JobsRuntimeCommand
{
    public static function execute(array $arguments): array
    {
        ...
        try{
            ...
            $queue=new MariaDbJobQueue($db,$prefix,null,null,['workforce.sync'=>[1],'outbox.dispatch'=>[1],'bitrix.order-document-links.sync'=>[1],'weekly-fkr-report.generate'=>[1],'erp.equipment-facts.sync'=>[1]]);
            ...
            switch($request->mode){
                ...
                case 'scheduler':
                    $result=(new JobsSchedulerProcess(new MariaDbWorkforceScheduler($db,$prefix),
                        ...
                        new MariaDbWeeklyFkrScheduler($db,$prefix),new MariaDbEquipmentFactsScheduler($db,$prefix)))->run();
                ...
                case 'worker':
                    ...
                    $registry=['workforce.sync'=>[1=>$command],'outbox.dispatch'=>[1=>$command],'bitrix.order-document-links.sync'=>[1=>$command],'weekly-fkr-report.generate'=>[1=>$command],'erp.equipment-facts.sync'=>[1=>$command]];
