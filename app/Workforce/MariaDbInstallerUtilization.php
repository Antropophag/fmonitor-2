...
final class MariaDbInstallerUtilization
{
    ...
    public function forecast(string $cutoff, int $actorId=0): array
    {
        if (!$this->date($cutoff)) throw new \InvalidArgumentException('Forecast cutoff unavailable.');
        $zone=new \DateTimeZone('Europe/Moscow');$day=new \DateTimeImmutable($cutoff,$zone);$monday=$day->modify('monday this week');
        $weeks=[];for($i=0;$i<6;$i++){$start=$monday->modify("+$i week");$weeks[]=['start'=>$start->format('Y-m-d'),'end'=>$start->modify('+6 days')->format('Y-m-d'),'denominator'=>0,'busy'=>0,'free'=>0,'unknown'=>0,'releasing'=>0,'conflict'=>0,'members'=>[]];}
        $p=$this->processPrefix;$l=$this->legacyPrefix;$plannedStart=MariaDbYiiObjectQueue::plannedStartDateExpression('m.workdatestart');$effectiveReg=MariaDbEffectiveObjectDetails::sqlValue('regnumber','m.regnumber','e');$effectiveAddress=MariaDbEffectiveObjectDetails::sqlValue('address','m.ordadr_address','e');
        $gap=(int)$this->db->createCommand("SELECT COUNT(*) FROM `{$p}fm2_installation_cases` c WHERE c.actual_start_date IS NOT NULL AND NOT EXISTS(SELECT 1 FROM `{$p}fm2_pilot_completion_facts` pf WHERE pf.installation_case_id=c.id AND pf.fact_type='pto_act') AND NOT EXISTS(SELECT 1 FROM `{$p}fm2_assignment_order_applications` a WHERE a.installation_case_id=c.id) AND NOT EXISTS(SELECT 1 FROM `{$p}fm2_assignment_orders` o JOIN `{$p}fm2_order_installers` oi ON oi.assignment_order_id=o.id AND oi.change_action<>'release' WHERE o.installation_case_id=c.id AND o.status='registered')")->queryScalar();if($gap>0)throw new \RuntimeException('UTILIZATION_SOURCE_UNAVAILABLE');
        $workforce=$this->db->createCommand("SELECT installer_tab_id,fio,employment_status,reconciliation_state,workforce_source_updated_at FROM `{$p}fm2_workforce_catalog` WHERE employment_status='employed' ORDER BY fio,installer_tab_id")->queryAll();
        if($workforce===[])throw new \RuntimeException('UTILIZATION_SOURCE_UNAVAILABLE');
        $native=$this->db->createCommand("SELECT a.*,c.actual_start_date,($plannedStart) planned_start_date_snapshot,COALESCE(dc.new_deadline,NULLIF(LEFT(m.workdateendadjusted,10),''),LEFT(m.plan_finish_date,10)) planned_finish_date_snapshot,($effectiveReg) registration_number,($effectiveAddress) address,dc.new_deadline,(SELECT MIN(pf.fact_date) FROM `{$p}fm2_pilot_completion_facts` pf WHERE pf.installation_case_id=c.id AND pf.fact_type='pto_act') pto_date FROM `{$p}fm2_assignment_order_applications` a JOIN (SELECT installation_case_id,MAX(application_sequence) seq FROM `{$p}fm2_assignment_order_applications` GROUP BY installation_case_id) x ON x.installation_case_id=a.installation_case_id AND x.seq=a.application_sequence JOIN `{$p}fm2_installation_cases` c ON c.id=a.installation_case_id JOIN `{$l}fm_maintable` m ON m.id=c.legacy_installation_object_id LEFT JOIN `{$p}fm2_object_detail_edits` e ON e.object_id=m.id LEFT JOIN `{$p}fm2_deadline_certificate_roots` dr ON dr.installation_case_id=c.id LEFT JOIN `{$p}fm2_deadline_certificate_revisions` dc ON dc.id=dr.current_revision_id")->queryAll();
        $legacy=$this->db->createCommand("SELECT oi.installer_tab_id,oi.valid_from,oi.valid_to,c.actual_start_date,($plannedStart) planned_start_date_snapshot,COALESCE(dc.new_deadline,NULLIF(LEFT(m.workdateendadjusted,10),''),LEFT(m.plan_finish_date,10)) planned_finish_date_snapshot,c.legacy_installation_object_id object_id,($effectiveReg) registration_number,($effectiveAddress) address,(SELECT MIN(pf.fact_date) FROM `{$p}fm2_pilot_completion_facts` pf WHERE pf.installation_case_id=c.id AND pf.fact_type='pto_act') pto_date FROM `{$p}fm2_order_installers` oi JOIN `{$p}fm2_assignment_orders` o ON o.id=oi.assignment_order_id JOIN (SELECT installation_case_id,MAX(version_no) version_no FROM `{$p}fm2_assignment_orders` WHERE status='registered' GROUP BY installation_case_id) x ON x.installation_case_id=o.installation_case_id AND x.version_no=o.version_no JOIN `{$p}fm2_installation_cases` c ON c.id=o.installation_case_id JOIN `{$l}fm_maintable` m ON m.id=c.legacy_installation_object_id LEFT JOIN `{$p}fm2_object_detail_edits` e ON e.object_id=m.id LEFT JOIN `{$p}fm2_deadline_certificate_roots` dr ON dr.installation_case_id=c.id LEFT JOIN `{$p}fm2_deadline_certificate_revisions` dc ON dc.id=dr.current_revision_id WHERE oi.change_action<>'release' AND NOT EXISTS(SELECT 1 FROM `{$p}fm2_assignment_order_applications` a WHERE a.installation_case_id=c.id)")->queryAll();
        $intervals=[];
        foreach($native as$row){try{$snapshot=$this->nativeSnapshot($row);}catch(\Throwable){throw new \RuntimeException('UTILIZATION_SOURCE_UNAVAILABLE');}foreach($snapshot['selectedInstallers']as$member){$tab=(int)($member['tabId']??0);if($tab<1)throw new \RuntimeException('UTILIZATION_SOURCE_UNAVAILABLE');$intervals[$tab][]=$this->forecastInterval($row,$row['actual_start_date']?:$row['planned_start_date_snapshot']);}}
        foreach($legacy as$row){$tab=(int)$row['installer_tab_id'];if($tab<1)throw new \RuntimeException('UTILIZATION_SOURCE_UNAVAILABLE');$intervals[$tab][]=$this->forecastInterval($row,$row['actual_start_date']?:$row['planned_start_date_snapshot'],$row['valid_to']??null);}
        foreach($workforce as$row){$tab=(int)($row['installer_tab_id']??0);$fio=trim((string)($row['fio']??''));$known=$tab>0&&$fio!==''&&($row['reconciliation_state']??null)==='delivered';foreach($weeks as&$week){$week['denominator']++;$hits=[];if($known)foreach($intervals[$tab]??[]as$interval)if($interval['start']<=$week['end']&&($interval['end']===null||$interval['end']>=$week['start']))$hits[]=$interval;$bucket=!$known?'unknown':($hits===[]?'free':'busy');$week[$bucket]++;$releasing=false;foreach($hits as$ended){if($ended['end']===null||$ended['end']<$week['start']||$ended['end']>$week['end'])continue;$coveredLater=false;if($ended['end']<$week['end'])foreach($hits as$other)if($other!==$ended&&$other['start']<=$week['end']&&($other['end']===null||$other['end']>$ended['end'])){$coveredLater=true;break;}if(!$coveredLater){$releasing=true;break;}}$conflict=false;for($a=0;$a<count($hits)&&!$conflict;$a++)for($b=$a+1;$b<count($hits);$b++){if(max($hits[$a]['start'],$hits[$b]['start'],$week['start'])<=min($hits[$a]['end']??$week['end'],$hits[$b]['end']??$week['end'],$week['end'])){$conflict=true;break;}}if($releasing)$week['releasing']++;if($conflict)$week['conflict']++;$week['members'][]=['installerTabId'=>$tab,'fio'=>$fio,'bucket'=>$bucket,'releasing'=>$releasing,'conflict'=>$conflict,'reasons'=>$hits];}unset($week);}
        foreach($weeks as&$week)usort($week['members'],static fn(array$a,array$b):int=>[$a['fio'],$a['installerTabId']]<=>[$b['fio'],$b['installerTabId']]);unset($week);
        return['cutoff'=>$cutoff,'weeks'=>$weeks];
    }

    public function forecastDetail(string $weekStart,string $bucket,int $actorId=0):array
    {
        if(!$this->date($weekStart)||!in_array($bucket,['busy','free','unknown','releasing','conflict'],true))throw new \OutOfBoundsException('Forecast detail unavailable.');
        $forecast=$this->forecast($weekStart,$actorId);$week=$forecast['weeks'][0];$members=array_values(array_filter($week['members'],static fn(array$m):bool=>$bucket==='releasing'?$m['releasing']:($bucket==='conflict'?$m['conflict']:$m['bucket']===$bucket)));
        return['weekStart'=>$week['start'],'weekEnd'=>$week['end'],'bucket'=>$bucket,'count'=>count($members),'denominator'=>$week['denominator'],'members'=>$members];
    }

    private function forecastInterval(array$row,mixed$start,mixed$explicitEnd=null):array
    {
        $start=$this->dateOrNull($start);if($start===null)throw new \RuntimeException('UTILIZATION_SOURCE_UNAVAILABLE');$ends=[];foreach([$explicitEnd,$row['pto_date']??null,$row['planned_finish_date_snapshot']??null]as$value){$date=$this->dateOrNull($value);if($date!==null)$ends[]=$date;}$end=$ends===[]?null:min($ends);
        return['objectId'=>(int)($row['object_id']??$row['legacy_installation_object_id']??0),'registrationNumber'=>(string)($row['registration_number']??$row['regnumber']??''),'address'=>(string)($row['address']??$row['ordadr_address']??''),'startDate'=>$start,'endDate'=>$end,'start'=>$start,'end'=>$end];
    }
    ...
    private function projection(int $actorId=0,?array$onlyTabs=null): array
    {
        ...
        $nativeBase=(new Query())
            ->select([
                'a.application_id','a.installation_case_id','a.application_sequence','a.object_id','a.assignment_order_id','a.order_version','a.composition_identity','a.composition_sha256','a.control_engineer_user_id','a.selected_snapshot_json','a.document_date',
                'planned_finish'=>new Expression('NULL'),
                'registration_number'=>new Expression($effectiveReg),
                'address'=>new Expression($effectiveAddress),
                'is_active'=>new Expression("NOT($released)"),
                'is_current'=>new Expression("c.actual_start_date IS NOT NULL AND NOT($released)"),
            ])
            ->from(['a'=>$applications])
            ->innerJoin(['la'=>$latestApplications],'la.installation_case_id=a.installation_case_id AND la.application_sequence=a.application_sequence')
            ->innerJoin(['c'=>$cases],'c.id=a.installation_case_id')
            ->innerJoin(['m'=>$objects],'m.id=a.object_id')
            ->leftJoin(['e'=>$edits],'e.object_id=a.object_id');
        $nativeRows=$nativeBase->all($this->db);
        $nativeApplied=array_values(array_filter($nativeRows,static fn(array $row):bool => (bool)$row['is_active']));
        $native=array_values(array_filter($nativeRows,static fn(array $row):bool => (bool)$row['is_current']));
        ...
        $legacyBase=(new Query())->select([
            'oi.installer_tab_id','object_id'=>'c.legacy_installation_object_id',
            'registration_number'=>new Expression("CONCAT([[ao.registration_number]], ' · ', $effectiveReg)"),
            'address'=>new Expression($effectiveAddress),'start'=>'oi.valid_from','end'=>'oi.valid_to',
            'planned_finish'=>'ao.planned_finish_date_snapshot',
            'is_active'=>new Expression("NOT($released)"),
            'is_current'=>new Expression("c.actual_start_date IS NOT NULL AND NOT($released)"),
        ])->from(['oi'=>$members])->innerJoin(['ao'=>$orders],'ao.id=oi.assignment_order_id')->innerJoin(['lo'=>$latestOrders],'lo.installation_case_id=ao.installation_case_id AND lo.version_no=ao.version_no')->innerJoin(['c'=>$cases],'c.id=ao.installation_case_id')->innerJoin(['m'=>$objects],'m.id=c.legacy_installation_object_id')->leftJoin(['e'=>$edits],'e.object_id=c.legacy_installation_object_id')->where(['ao.status'=>'registered'])->andWhere(['<>','oi.change_action','release'])->andWhere(['<=','oi.valid_from',$today])->andWhere(['or',['oi.valid_to'=>null],['>=','oi.valid_to',$today]])->andWhere(['not exists',(new Query())->from(['x'=>$applications])->where(new Expression('[[x.installation_case_id]]=[[ao.installation_case_id]]'))]);
        $legacyRows=$legacyBase->all($this->db);
        $legacyApplied=array_values(array_filter($legacyRows,static fn(array $row):bool => (bool)$row['is_active']));
        $legacy=array_values(array_filter($legacyRows,static fn(array $row):bool => (bool)$row['is_current']));
