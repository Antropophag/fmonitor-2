...
final class MariaDbInstallerUtilization
{
    ...
    private function projection(int $actorId=0,?array$onlyTabs=null): array
    {
        ...
        $edits=$this->processPrefix.'fm2_object_detail_edits';$effectiveReg=MariaDbEffectiveObjectDetails::sqlValue('regnumber','m.regnumber','e');$effectiveAddress=MariaDbEffectiveObjectDetails::sqlValue('address','m.ordadr_address','e');
    ...
    private function historyPage(int $tabId,int $page,int$actorId=0):array
    {
        ...
        $end="(SELECT n.document_date FROM `$applications` n WHERE n.installation_case_id=a.installation_case_id AND n.application_sequence>a.application_sequence AND JSON_CONTAINS(n.selected_snapshot_json,$candidate,'$.selectedInstallers')=0 ORDER BY n.application_sequence ASC LIMIT 1)";
        ...
        $legacy="SELECT 0 application_sequence,c.id installation_case_id,c.legacy_installation_object_id object_id,oi.valid_from start,NULL end,CONVERT(CONCAT(ao.registration_number,' · ',($historyReg)) USING utf8mb4) COLLATE utf8mb4_unicode_ci registration_number,CONVERT(($historyAddress) USING utf8mb4) COLLATE utf8mb4_unicode_ci address,ao.planned_finish_date_snapshot planned_finish FROM `$members` oi JOIN `$orders` ao ON ao.id=oi.assignment_order_id JOIN `$cases` c ON c.id=ao.installation_case_id JOIN `$objects` m ON m.id=c.legacy_installation_object_id LEFT JOIN `$edits` e ON e.object_id=c.legacy_installation_object_id WHERE ($role OR $ownedLegacy) AND oi.installer_tab_id=".(int)$tabId." AND ao.status='registered' AND oi.change_action<>'release' AND oi.valid_from<=$today AND (oi.valid_to IS NULL OR oi.valid_to>=$today) AND ao.version_no=(SELECT MAX(lo.version_no) FROM `$orders` lo WHERE lo.installation_case_id=ao.installation_case_id) AND NOT EXISTS(SELECT 1 FROM `$applications` x WHERE x.installation_case_id=ao.installation_case_id) AND NOT EXISTS(SELECT 1 FROM `{$p}fm2_pilot_completion_facts` release_fact WHERE release_fact.installation_case_id=c.id AND release_fact.fact_type='pto_act')";
