...
final class WorkerConfiguration
{
    public static function fromFile(string $path): array
    {
        ...
        if (!is_array($document) || array_diff(array_keys($document), ['baseUrl', 'departments', 'documentBaseUrl']) !== []) self::fail();
        ...
        $documentBaseUrl = $document['documentBaseUrl'] ?? null;
        if ($documentBaseUrl !== null && !is_string($documentBaseUrl)) self::fail();
        if (is_string($documentBaseUrl)) {
            $documentCredentials = self::webhookCredentials($documentBaseUrl);
            if ($origin === $documentCredentials['origin'] && (int) $match[1] === $documentCredentials['webhookUserId']
                && hash_equals($match[2], $documentCredentials['token'])) self::fail();
        }
        return ['origin' => $origin, 'webhookUserId' => (int) $match[1], 'token' => $match[2], 'departmentIds' => $ids, 'documentBaseUrl' => $documentBaseUrl];
    }

    public static function fromDocumentFile(string $path): array
    {
        $workforce = self::fromFile($path);
        $documentBaseUrl = $workforce['documentBaseUrl'];
        if (!is_string($documentBaseUrl) || $documentBaseUrl === '') self::fail();
        return self::webhookCredentials($documentBaseUrl);
    }
    ...
    private static function webhookCredentials(string $baseUrl): array
    {
        $parts = parse_url($baseUrl);
        if ($parts === false || ($parts['scheme'] ?? null) !== 'https' || isset($parts['user']) || isset($parts['pass'])
            || isset($parts['query']) || isset($parts['fragment']) || !is_string($parts['host'] ?? null)
            || preg_match('#^/rest/([1-9][0-9]*)/([A-Za-z0-9_-]{1,256})/?$#D', (string) ($parts['path'] ?? ''), $match) !== 1) self::fail();
        return [
            'origin' => 'https://' . $parts['host'] . (isset($parts['port']) ? ':' . $parts['port'] : ''),
            'webhookUserId' => (int) $match[1],
            'token' => $match[2],
        ];
    }
