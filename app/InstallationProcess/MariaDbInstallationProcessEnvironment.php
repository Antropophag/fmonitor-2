...
final class MariaDbInstallationProcessEnvironment
{
    ...
    public function getInstallationObjectProcess(int $id): array
    {
        ...
        $nativeOpened=$case['actual_start_date']!==null;
        $migratedActive=null;
        if(!$nativeOpened&&(string)$case['process_state']==='working')$migratedActive=$this->migratedActiveProjection($caseId,$id);
        if(!$nativeOpened&&(string)$case['process_state']==='working'&&$migratedActive===null)throw new \RuntimeException('Working installation has no valid opening or cutover provenance.');
        if($migratedActive!==null)$projection+=['actualStartDate'=>null,'openedAt'=>null,'openedByUserId'=>null,'actualStartDateUnknownAtCutover'=>true,'operationallyActive'=>true,'activeCaseProvenance'=>$migratedActive];
        $active=$nativeOpened||$migratedActive!==null;
        return $projection+['assignmentOrders'=>$orders,'assignments'=>$assignments,'openTasks'=>[],'installationOpened'=>$active,'checklistAvailable'=>$active,'events'=>$events];
    }

    private function migratedActiveProjection(int $caseId,int $legacyObjectId):?array
    {
        $required=['fm2_active_case_provenance','fm2_legacy_active_baselines','fm2_checklist_template_snapshots','fm2_checklist_template_associations'];$names=array_map(fn(string $name):string=>$this->tablePrefix.$name,$required);$quoted=implode(',',array_map(fn(string $name):string=>"'".$this->connection->real_escape_string($name)."'",$names));$tables=$this->connection->query("SELECT TABLE_NAME FROM information_schema.TABLES WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME IN ({$quoted})")->fetch_all(MYSQLI_ASSOC);if(count($tables)!==count($required))return null;
        $sql="SELECT p.legacy_object_id,p.baseline_id,p.baseline_sha256,p.cutover_at,p.template_snapshot_id,p.template_snapshot_version,p.template_content_sha256,p.connector_version,b.legacy_object_id baseline_legacy_object_id,b.content_sha256 stored_baseline_sha256,b.payload_json,t.snapshot_version,t.valid_from,t.validity_scope,t.content_sha256 stored_template_sha256,a.effective_at,a.template_snapshot_id associated_template_id,a.template_snapshot_version associated_template_version,a.template_content_sha256 associated_template_sha256 FROM {$this->tablePrefix}fm2_active_case_provenance p JOIN {$this->tablePrefix}fm2_legacy_active_baselines b ON b.id=p.baseline_id JOIN {$this->tablePrefix}fm2_checklist_template_snapshots t ON t.id=p.template_snapshot_id JOIN {$this->tablePrefix}fm2_checklist_template_associations a ON a.subject_kind='operational_case' AND a.subject_id=CAST(p.installation_case_id AS CHAR) WHERE p.installation_case_id=? LIMIT 2";$statement=$this->connection->prepare($sql);$statement->bind_param('i',$caseId);$statement->execute();$rows=$statement->get_result()->fetch_all(MYSQLI_ASSOC);if(count($rows)!==1)return null;$row=$rows[0];
        try{$payload=$this->decode((string)$row['payload_json']);}catch(\Throwable){return null;}$quarantine=$payload['classification']['quarantineCodes']??null;$valid=is_array($quarantine)&&$quarantine===[]&&($payload['classification']['category']??null)==='legacy_active'&&(int)$row['legacy_object_id']===$legacyObjectId&&(int)$row['baseline_legacy_object_id']===$legacyObjectId&&$row['connector_version']==='active-baseline-operational-case-v1'&&hash_equals((string)$row['baseline_sha256'],(string)$row['stored_baseline_sha256'])&&hash_equals((string)$row['baseline_sha256'],hash('sha256',(string)$row['payload_json']))&&(string)$row['cutover_at']===(string)$row['effective_at']&&(int)$row['template_snapshot_id']===(int)$row['associated_template_id']&&(string)$row['template_snapshot_version']===(string)$row['snapshot_version']&&(string)$row['template_snapshot_version']===(string)$row['associated_template_version']&&hash_equals((string)$row['template_content_sha256'],(string)$row['stored_template_sha256'])&&hash_equals((string)$row['template_content_sha256'],(string)$row['associated_template_sha256'])&&(string)$row['valid_from']<=(string)$row['cutover_at']&&(string)$row['validity_scope']==='active_baseline_and_future_native_only';if(!$valid)return null;
        return['baselineId'=>(int)$row['baseline_id'],'baselineHash'=>(string)$row['baseline_sha256'],'cutoverAt'=>(string)$row['cutover_at'],'templateSnapshotId'=>(int)$row['template_snapshot_id'],'templateVersion'=>(string)$row['template_snapshot_version'],'templateHash'=>(string)$row['template_content_sha256'],'connectorVersion'=>(string)$row['connector_version']];
    }
