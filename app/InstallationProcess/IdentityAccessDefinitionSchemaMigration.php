...
final class IdentityAccessDefinitionSchemaMigration
{
    ...
    public static function definitions(string $prefix, string $collation): array
    {
        ...
        $ddls = [
            'fm2_pilot_users' => "CREATE TABLE `{$prefix}fm2_pilot_users`(user_id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,full_name VARCHAR(300) NOT NULL,email VARCHAR(254) NOT NULL,phone VARCHAR(100) NOT NULL DEFAULT '',status TINYINT(1) NOT NULL DEFAULT 1,activation_state ENUM('pending_invitation','invited','active','blocked') NOT NULL,session_version INT UNSIGNED NOT NULL DEFAULT 1,source_updated_at VARCHAR(40) NOT NULL,PRIMARY KEY(user_id),UNIQUE KEY uq_ia_users_email(email),KEY ix_ia_users_status_name(status,full_name)){$tail}",
