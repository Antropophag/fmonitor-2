...
final class MariaDbErpEquipmentFactsSource
{
    ...
    public static function shipmentsQuery():string{return "SELECT z.номер AS sourceOrderNumber, MIN(e.датаотгрузки) AS shipmentDate FROM BI_DзаказклиентаID z JOIN BI_Dэтаппроизводства2_2ID e ON e.заказклиентаID=z.ID WHERE e.пометкаудаления=0 AND e.наименование='лифтовоеоборудование' AND e.датаотгрузки IS NOT NULL AND e.датаотгрузки<>'0001-01-01' GROUP BY z.номер";}
}
