...
final class AssignmentOrderOriginalDatabaseSetupV1
{
    ...
    public static function sqlChecks(): array
    {
        $id=static fn(string$v,int$n=80):string=>"CHAR_LENGTH({$v}) BETWEEN 1 AND {$n} AND {$v} NOT REGEXP '[[:cntrl:]/\\\\\\\\]'";$uuid=static fn(string$v):string=>"{$v} REGEXP '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'";$ev='root_original_id IS NOT NULL AND current_revision_id IS NOT NULL AND revision_number IS NOT NULL AND document_date IS NOT NULL AND sha256 IS NOT NULL AND byte_size IS NOT NULL AND uploaded_at_utc IS NOT NULL';$empty='root_original_id IS NULL AND current_revision_id IS NULL AND revision_number IS NULL AND document_date IS NULL AND sha256 IS NULL AND byte_size IS NULL AND uploaded_at_utc IS NULL';$rej="reason_code IN ('authorization_denied','invalid_command','order_not_found','composition_not_confirmed','invalid_composition','file_too_large','not_pdf','invalid_pdf','unsafe_pdf','future_document_date','no_changes')";$con="reason_code IN ('semantic_collision','stale_revision','target_not_found','target_not_current','initial_already_exists')";
        return[self::TABLES[0]=>["composition_sha256 REGEXP '^[0-9a-f]{64}$'",$id('root_original_id'),$id('current_revision_id')],self::TABLES[1]=>['revision_number >= 1',"pdf_sha256 REGEXP '^[0-9a-f]{64}$'","operation_fingerprint REGEXP '^[0-9a-f]{64}$'",'byte_size BETWEEN 1 AND 20971520',"event_type IN ('assignment_order_original_accepted','assignment_order_original_corrected')","((revision_number=1 AND previous_revision_id IS NULL AND correction_reason IS NULL AND event_type='assignment_order_original_accepted') OR (revision_number>1 AND previous_revision_id IS NOT NULL AND CHAR_LENGTH(TRIM(correction_reason)) BETWEEN 1 AND 500 AND event_type='assignment_order_original_corrected'))",$id('revision_id'),$id('root_original_id'),$id('previous_revision_id'),$id('private_content_identity',160)],self::TABLES[2]=>[$uuid('request_id'),"mode IN ('initial','correction')","status IN ('accepted','replayed','rejected','conflict')",'retryable=0',"(status NOT IN ('accepted','replayed') OR (reason_code IS NULL AND {$ev}))","(status <> 'rejected' OR ({$rej} AND {$empty}))","(status <> 'conflict' OR ({$con} AND {$empty}))",$id('root_original_id'),$id('current_revision_id')],self::TABLES[3]=>["event_type IN ('assignment_order_original_accepted','assignment_order_original_corrected')",$id('root_original_id'),$id('revision_id')],self::TABLES[4]=>[$uuid('request_id'),"mode IN ('initial','correction')","status IN ('accepted','rejected','conflict')","((status='accepted' AND reason_code IS NULL) OR (status='rejected' AND {$rej}) OR (status='conflict' AND {$con}))"],self::TABLES[5]=>[$uuid('request_id'),"status IN ('completed','replayed','rejected','partial')","((status IN ('completed','replayed') AND reason_code IS NULL AND retryable=0) OR (status='rejected' AND reason_code IN ('invalid_command','authorization_denied') AND retryable=0) OR (status='partial' AND reason_code IN ('locked','storage_failure') AND retryable=1))",'scanned=deleted+retained+failed'],self::TABLES[6]=>[$uuid('request_id'),"status IN ('completed','rejected','partial')","((status='completed' AND reason_code IS NULL AND retryable=0) OR (status='rejected' AND reason_code IN ('invalid_command','authorization_denied') AND retryable=0) OR (status='partial' AND reason_code IN ('locked','storage_failure') AND retryable=1))",'scanned=deleted+retained+failed']];
    }
}
