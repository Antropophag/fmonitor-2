...
final readonly class MariaDbYiiObjectQueue
{
    ...
    public function readCalendar(string $first, string $last): array
    {
        ...
        $rows=$this->db->createCommand("SELECT s.id schedule_id,s.legacy_object_id object_id,s.inspection_date event_date,'inspection' event_type,".$effective('regnumber','regnumber')." regnumber,".$effective('address','ordadr_address')." ordadr_address,".$effective('entrance','entrance')." entrance FROM `{$p}fm2_pilot_inspection_schedules` s LEFT JOIN `{$l}fm_maintable` m ON m.id=s.legacy_object_id LEFT JOIN `{$p}fm2_object_detail_edits` e ON e.object_id=s.legacy_object_id WHERE s.inspection_date BETWEEN :first AND :last ORDER BY s.inspection_date,s.legacy_object_id,s.id LIMIT ".(self::CALENDAR_ROW_LIMIT+1),[':first'=>$first,':last'=>$last])->queryAll();
