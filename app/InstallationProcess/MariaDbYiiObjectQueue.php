...
final readonly class MariaDbYiiObjectQueue
{
    ...
    public function readCalendar(int|string $actor,string $today,string $first='', string $last=''): array
    {
        ...
        $current=MariaDbYiiInspectionPlanning::currentProjectionSql($p,':today');$global="NOT EXISTS(SELECT 1 FROM `{$p}fm2_pilot_user_roles` engineer_ur JOIN `{$p}fm2_pilot_roles` engineer_role ON engineer_role.role_id=engineer_ur.role_id AND engineer_role.status=1 WHERE engineer_ur.user_id=:actor AND BINARY engineer_role.code='control_engineer')";$assigned="EXISTS(SELECT 1 FROM `{$p}fm2_control_engineer_assignments` scope_assignment WHERE scope_assignment.installation_case_id=current.installation_case_id AND scope_assignment.assignment_sequence=(SELECT MAX(scope_latest.assignment_sequence) FROM `{$p}fm2_control_engineer_assignments` scope_latest WHERE scope_latest.installation_case_id=current.installation_case_id) AND scope_assignment.engineer_user_id=:actor)";$scope=$actor===0?'1=1':"($global OR $assigned)";
        ...
        $plannedScope=$actor===0?'1=1':"($global OR EXISTS(SELECT 1 FROM `{$p}fm2_installation_cases` scope_case JOIN `{$p}fm2_control_engineer_assignments` scope_assignment ON scope_assignment.installation_case_id=scope_case.id AND scope_assignment.assignment_sequence=(SELECT MAX(scope_latest.assignment_sequence) FROM `{$p}fm2_control_engineer_assignments` scope_latest WHERE scope_latest.installation_case_id=scope_case.id) WHERE scope_case.legacy_installation_object_id=m.id AND scope_assignment.engineer_user_id=:actor))";$plannedParams=($actor===0?[]:[':actor'=>$actor])+[':first'=>$first,':last'=>$last];$planned=$this->db->createCommand("SELECT m.id object_id,m.workdatestart,{$finishColumn} workdatefinish,m.plan_finish_date,".$effective('regnumber','regnumber')." regnumber,".$effective('address','ordadr_address')." ordadr_address,".$effective('entrance','entrance')." entrance FROM `{$legacyTable}` m LEFT JOIN `{$p}fm2_object_detail_edits` e ON e.object_id=m.id WHERE $plannedScope AND ((LEFT(m.workdatestart,10) BETWEEN :first AND :last) OR (LEFT({$finishExpression},10) BETWEEN :first AND :last)) LIMIT ".(self::CALENDAR_ROW_LIMIT+1),$plannedParams)->queryAll();
