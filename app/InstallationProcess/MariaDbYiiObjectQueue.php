...
final readonly class MariaDbYiiObjectQueue
{
    ...
    public function readCalendar(string $first, string $last): array
    {
        ...
        $eventDate="COALESCE(NULLIF(JSON_UNQUOTE(JSON_EXTRACT(pe.payload_json,'$.newDate')),'null'),NULLIF(JSON_UNQUOTE(JSON_EXTRACT(pe.payload_json,'$.inspectionDate')),'null'),s.inspection_date)";
        $rows=$this->db->createCommand("SELECT s.id schedule_id,s.legacy_object_id object_id,{$eventDate} event_date,'inspection' event_type,".$effective('regnumber','regnumber')." regnumber,".$effective('address','ordadr_address')." ordadr_address,".$effective('entrance','entrance')." entrance FROM `{$p}fm2_pilot_inspection_schedules` s LEFT JOIN `{$p}fm2_pilot_inspection_schedule_events` pe ON pe.schedule_id=s.id AND pe.id=(SELECT MAX(px.id) FROM `{$p}fm2_pilot_inspection_schedule_events` px WHERE px.schedule_id=s.id) LEFT JOIN `{$l}fm_maintable` m ON m.id=s.legacy_object_id LEFT JOIN `{$p}fm2_object_detail_edits` e ON e.object_id=s.legacy_object_id WHERE (pe.id IS NULL OR pe.event_type<>'inspection_cancelled') AND {$eventDate} BETWEEN :first AND :last ORDER BY event_date,s.legacy_object_id,s.id LIMIT ".(self::CALENDAR_ROW_LIMIT+1),[':first'=>$first,':last'=>$last])->queryAll();
