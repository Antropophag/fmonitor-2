...
final readonly class MariaDbYiiObjectQueue
{
    ...
    public function readCalendar(string $first, string $last): array
    {
        ...
        $eventDate="COALESCE(NULLIF(JSON_UNQUOTE(JSON_EXTRACT(pe.payload_json,'$.newDate')),'null'),NULLIF(JSON_UNQUOTE(JSON_EXTRACT(pe.payload_json,'$.inspectionDate')),'null'),s.inspection_date)";$today=(new \DateTimeImmutable('now',new \DateTimeZone('Europe/Moscow')))->format('Y-m-d');
        $current="SELECT s.installation_case_id,MIN(s.id) schedule_id,MIN(s.legacy_object_id) object_id,MIN($eventDate) inspection_date,COUNT(*) current_count FROM `{$p}fm2_pilot_inspection_schedules` s LEFT JOIN `{$p}fm2_pilot_inspection_schedule_events` pe ON pe.schedule_id=s.id AND pe.event_version=(SELECT MAX(px.event_version) FROM `{$p}fm2_pilot_inspection_schedule_events` px WHERE px.schedule_id=s.id) WHERE (pe.id IS NULL OR pe.event_type<>'inspection_cancelled') AND $eventDate>=:today GROUP BY s.installation_case_id";
        if((int)$this->db->createCommand("SELECT MAX(current_count) FROM ($current) current",[':today'=>$today])->queryScalar()>1)throw new \RuntimeException('Multiple current inspection plans.');
        $rows=$this->db->createCommand("SELECT current.schedule_id,current.object_id,current.inspection_date event_date,'inspection' event_type,current.current_count,".$effective('regnumber','regnumber')." regnumber,".$effective('address','ordadr_address')." ordadr_address,".$effective('entrance','entrance')." entrance FROM ($current) current LEFT JOIN `{$l}fm_maintable` m ON m.id=current.object_id LEFT JOIN `{$p}fm2_object_detail_edits` e ON e.object_id=current.object_id WHERE current.inspection_date BETWEEN :first AND :last ORDER BY current.inspection_date,current.object_id,current.schedule_id LIMIT ".(self::CALENDAR_ROW_LIMIT+1),[':today'=>$today,':first'=>$first,':last'=>$last])->queryAll();
        foreach($rows as$row)if((int)$row['current_count']!==1)throw new \RuntimeException('Multiple current inspection plans.');
