...
final class MariaDbEquipmentFacts
{
    ...
    private function complete(array$c,string$hash,string$key):array
    {
        ...
        $hasEdits=(int)$this->db->query("SELECT COUNT(*) FROM information_schema.TABLES WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='".$this->db->real_escape_string($this->p.'fm2_object_detail_edits')."'")->fetch_column()===1;
        foreach($c['records']as$r){$h=hash_hmac('sha256',$r['sourceOrderNumber'],$key);$sql=$hasEdits?"SELECT m.id FROM `{$this->p}fm_maintable` m LEFT JOIN `{$this->p}fm2_object_detail_edits` e ON e.object_id=m.id WHERE BINARY (CASE WHEN JSON_CONTAINS_PATH(e.values_json,'one','$.zavnumber') THEN JSON_UNQUOTE(JSON_EXTRACT(e.values_json,'$.zavnumber')) ELSE m.zavnumber END)=BINARY ? ORDER BY m.id FOR UPDATE":"SELECT id FROM `{$this->p}fm_maintable` WHERE BINARY zavnumber=BINARY ? ORDER BY id FOR UPDATE";$s=$this->db->prepare($sql);$s->bind_param('s',$r['sourceOrderNumber']);$s->execute();$ids=array_map('intval',array_column($s->get_result()->fetch_all(MYSQLI_ASSOC),'id'));
