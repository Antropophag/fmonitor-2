...
final readonly class MariaDbOriginalOpening
{
    ...
    public function openInstallation(int $object, string $date, int $expectedApplication, int $actor): array
    {
        ...
        try {
            ...
            if (!$this->authorized($s,$actor,(int)$application['engineerUserId'],false)) return $failure('authorization_denied');
    ...
    public function openPreparedWithinTransaction(int $object,string $date,int $expectedApplication,int $actor,array $application,O\AssignmentOrderOriginalApplicationReferenceReader $referenceReader,O\AssignmentOrderOriginalApplicationReference $reference,SelectionInstant $instant,?array $requestIdentity=null):array
    {
        ...
        if(!$this->authorized($s,$actor,(int)$application['engineerUserId'],true))throw new \RuntimeException('authorization_denied');
    ...
    private function authorized(MariaDbSelectionSql $s,int $actor,int $engineer,bool $lock): bool
    {
        return $s->rows('SELECT u.user_id FROM '.$s->table('fm2_pilot_users').' u JOIN '.$s->table('fm2_pilot_user_roles').' ur ON ur.user_id=u.user_id JOIN '.$s->table('fm2_pilot_roles').' r ON r.role_id=ur.role_id JOIN '.$s->table('fm2_pilot_role_permissions')." p ON p.role_id=r.role_id WHERE u.user_id=? AND u.status=1 AND u.activation_state='active' AND r.status=1 AND (r.code IN ('fkr_operator','manager') OR (r.code='construction_control_engineer' AND u.user_id=?)) AND p.permission='installation.open'".($lock?' FOR UPDATE':''),[$actor,$engineer])!==[];
    }
}
