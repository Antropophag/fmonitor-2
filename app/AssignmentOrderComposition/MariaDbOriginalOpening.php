...
final readonly class MariaDbOriginalOpening
{
    ...
    public function openInstallation(int $object, string $date, int $expectedApplication, int $actor): array
    {
        ...
        try {
            ...
            if ($reader->confirmCurrent($object,$expectedApplication)!=='matched') throw new \RuntimeException('application_changed');
            if ($original->confirmCurrent($reference->reference)!==O\AssignmentOrderOriginalApplicationGuardStatus::MATCHED) throw new \RuntimeException('original_requires_reapplication');
            if ($case[0]['actual_start_date']!==null || $case[0]['opened_at']!==null) throw new \RuntimeException('already_open');
            $id=(int)$case[0]['id'];
            $facts=$s->rows('SELECT fact_type FROM '.$s->table('fm2_pilot_completion_facts').' WHERE installation_case_id=?',[$id]);
            if ($facts!==[]) throw new \RuntimeException('completion_document_exists');
            $legacy=$s->rows('SELECT workdatefinish,ptoactdate FROM '.$s->table('fm_maintable').' WHERE id=?',[$object]);
            foreach (['workdatefinish','ptoactdate'] as $key) if (!empty($legacy[0][$key]) && !str_starts_with($legacy[0][$key],'0000-00-00')) throw new \RuntimeException('completion_document_exists');
            $eligibility=AssignmentOrderCurrentEligibility::confirm($this->db,$this->prefix,$application['installerTabIds'],$application['engineerUserId'],$application['documentDate']);
            if($eligibility!=='eligible')throw new \RuntimeException($eligibility);
            $template=(new MariaDbOpeningTemplate($s))->bind($id,$date,$at->utcRfc3339Seconds);
            $statement=$this->db->prepare('UPDATE '.$s->table('fm2_installation_cases')." SET process_state='working',actual_start_date=?,opened_at=?,opened_by_user_id=?,updated_at=?,lock_version=lock_version+1 WHERE id=? AND actual_start_date IS NULL");
            $statement->execute([$date,$at->utcRfc3339Seconds,$actor,$at->utcRfc3339Seconds,$id]);
            if ($statement->affected_rows!==1) throw new \RuntimeException('application_changed');$statement->close();
            $s->insert('fm2_process_events',['installation_case_id'=>$id,'event_type'=>'installation_opened_from_original','occurred_at'=>$at->utcRfc3339Seconds,
                'actor_user_id'=>$actor,'payload_json'=>SelectionScalar::json(['application'=>$application,'actualStartDate'=>$date,'template'=>$template])]);
