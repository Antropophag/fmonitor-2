...
final class MariaDbAssignmentOrderCurrentEligibility
{
    public static function confirm(\mysqli $db,string $prefix,array $installerTabIds,int $engineerUserId,string $documentDate):string
    {
        try{$s=new MariaDbAssignmentOrderApplicationSql($db,$prefix);if($s->idle()||$installerTabIds===[]||$engineerUserId<1)return'dependency_unavailable';
            ...
            if($local)$engineer=$s->rows('SELECT u.user_id FROM '.$s->table('fm2_pilot_users').' u JOIN '.$s->table('fm2_pilot_user_roles').' ur ON ur.user_id=u.user_id JOIN '.$s->table('fm2_pilot_roles')." r ON r.role_id=ur.role_id WHERE u.user_id=? AND u.status=1 AND BINARY u.activation_state IN ('active','pending_invitation','invited') AND r.status=1 AND BINARY r.code='construction_control_engineer' FOR UPDATE",[$engineerUserId]);
