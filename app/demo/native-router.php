...
$entrypoint = require dirname(__DIR__) . '/PilotHttp/production-entrypoint.php';
...
function emitDemoNativeResponse(int $status, array $headers, string $body): never
{
...
function demoNativeSecurityHeaders(string $body): array
{
    return [
...
if(is_string($requestPath)&&preg_match('#^/pilot/assets/fonts/(golos-text-(?:cyrillic|latin)-(?:400|500|600)-normal\.woff2)$#D',$requestPath,$fontMatch)===1){
    $fontPath=dirname(__DIR__,2).'/rapid-pilot/fonts/'.$fontMatch[1];
    $fontBytes=is_file($fontPath)&&!is_link($fontPath)?file_get_contents($fontPath):false;
    if(!is_string($fontBytes))emitDemoNativeResponse(404,demoNativeSecurityHeaders("Not found.\n"),"Not found.\n");
    emitDemoNativeResponse(200,[
...
if (PHP_SAPI === 'cli-server' && $localServerAddress === '127.0.0.1') {
    ...
    if (getenv('FMONITOR_DEMO_LOOPBACK') === '1' && is_string($demoNonce)
        && preg_match('/^[0-9a-f]{32}$/D', $demoNonce) === 1 && is_string($demoHost)
        && preg_match('/^127\.0\.0\.1:([1-9][0-9]{3,4})$/D', $demoHost, $parts) === 1
...
emitDemoNativeResponse($response->status,$headers,$response->body);
