...
use FMonitor2\RuntimeRestore\StandBackupBundle;
...
final class StandRestoreController extends Controller
{
    ...
    public $reconciliationId=null;
    public $fixtureDriver=null;
    public $authorization=null;
    public function options($actionID):array{return array_merge(parent::options($actionID),['manifest','bundleDigest','operationId','reconciliationId','fixtureDriver','authorization']);}
    ...
    public function actionReconcileUnknown():int{$p=$this->reconciliationArguments(array_slice($GLOBALS['FMONITOR2_RAW_ARGV']??[],1));if($p===null)return$this->finish(['ok'=>false,'reason'=>'CONFIGURATION_INVALID'],64);$o=(new StandRestoreApplication())->reconcileUnknown($p['manifest'],$p['operation-id'],$p['reconciliation-id'],$p['authorization'],$p['fixture-driver']??null);return$this->finish($o['result'],$o['exitCode']);}
    ...
    private function reconciliationArguments(array$args):?array{if(($args[0]??null)!=='stand-restore/reconcile-unknown'||end($args)!=='--interactive=0')return null;$out=[];foreach(array_slice($args,1,-1)as$item){if(preg_match('/^--([a-z-]+)=(.*)$/Ds',$item,$m)!==1||!in_array($m[1],['manifest','operation-id','reconciliation-id','authorization','fixture-driver'],true)||isset($out[$m[1]])||$m[2]==='')return null;$out[$m[1]]=$m[2];}foreach(['manifest','operation-id','reconciliation-id','authorization']as$key)if(!isset($out[$key]))return null;if(!StandBackupBundle::uuid($out['operation-id'])||!StandBackupBundle::uuid($out['reconciliation-id']))return null;return$out;}
