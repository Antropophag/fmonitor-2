...
<?php if ((int)$summary['total']===0):?><p class="shlz-table__empty">Каталог монтажников пока не загружен</p><?php elseif($rows===[]):?><p class="shlz-table__empty">Ничего не найдено</p><?php else:?><div class="shlz-table-wrap fm2-installer-table-scroll"><table class="shlz-table fm2-installer-table"><thead class="shlz-table__head"><tr class="shlz-table__row"><th class="shlz-table__cell">Монтажник</th><th class="shlz-table__cell">Кадровый статус</th><th class="shlz-table__cell">Загрузка</th><th class="shlz-table__cell">Объекты</th></tr></thead><tbody><?php foreach($rows as$row):$count=(int)$row['currentWorkCount'];?><tr class="shlz-table__row" data-current-work-count="<?=$count?>"><td class="shlz-table__cell"><a class="shlz-link" href="/pilot/installers/<?=(int)$row['installer_tab_id']?>?return=<?=rawurlencode(http_build_query(['q'=>$filters['q'],'current'=>$filters['current'],'upcoming'=>$filters['upcoming'],'page'=>$filters['page']]))?>"><strong><?=Html::encode(trim($row['fio']))?></strong></a><small>таб. <?=str_pad((string)$row['installer_tab_id'],6,'0',STR_PAD_LEFT)?> · <?=Html::encode($row['position'])?> · <?=Html::encode($row['workforce_source'])?></small></td><td class="shlz-table__cell"><?=$row['employment_status']==='employed'?'Работает':'Уволен'?></td><td class="shlz-table__cell"><?=$count===0?'Нет текущих работ · Нет действующих закреплений':$count.' '.($count===1?'текущая работа':'текущие работы')?><?php if(($row['upcomingAssignments']??[])!==[]):?><small>Предстоящее назначение: <?=Html::encode($row['upcomingAssignments'][0]['registration_number'])?></small><?php endif?></td><td class="shlz-table__cell"><?php $shown=[];foreach($row['assignments']as$a):$shown[(int)$a['object_id']]=true;?><a class="shlz-link fm2-assignment-link" href="/pilot/objects/<?=(int)$a['object_id']?>"><strong><?=Html::encode($a['registration_number'])?></strong><span><?=Html::encode($a['address'])?></span></a><?php endforeach?><?php foreach($row['upcomingAssignments']as$a):if(isset($shown[(int)$a['object_id']]))continue;?><a class="shlz-link fm2-assignment-link" href="/pilot/objects/<?=(int)$a['object_id']?>"><strong><?=Html::encode($a['registration_number'])?></strong><span><?=Html::encode($a['address'])?></span></a><?php endforeach?></td></tr><?php endforeach?></tbody></table></div><?php endif?>
