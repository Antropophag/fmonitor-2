...
$technicalDocuments=$access['technicalDocuments']??['status'=>'unavailable','links'=>[]];$technicalDocument=$technicalDocuments['status']==='available'?($technicalDocuments['links'][0]??null):null;$orderNumber=$access['orderNumber']??null;
...
<?php if($orderNumber!==null&&$orderNumber!==''):?>
<?php if(is_array($technicalDocument)):?><a class="fm2-check-order-link" href="<?=Html::encode($technicalDocument['url'])?>" target="_blank" rel="noopener noreferrer" aria-label="Открыть техническую документацию заказа <?=Html::encode($orderNumber)?>" title="Открыть техническую документацию"><?=Html::encode($orderNumber)?></a><?php else:?><span class="fm2-check-order-number"><?=Html::encode($orderNumber)?></span><?php endif?>
<?php endif?>
<strong>
