...
<section class="fm2-order-surface">
    ...
    <?php if ($latest): ?><div class="fm2-order-team"><div class="fm2-order-section-head"><div><h2>Выбранный состав · версия <?= (int) $latest['version'] ?></h2><p><?= $latest['hasAcceptedOriginal'] ? 'Подписанный оригинал принят' : 'Ожидается подписанный оригинал' ?></p></div></div><?php foreach ($latest['installers'] as $installer): ?><div class="fm2-object-installer"><strong><?= Html::encode($installer['fullName']) ?></strong><span>Табельный № <?= Html::encode((string) ($installer['tabId'] ?? 'Не указан')) ?></span></div><?php endforeach ?><p class="fm2-order-engineer-line"><strong>Инженер строительного контроля</strong><span><?= Html::encode($latest['engineer']['fullName']) ?></span></p></div><?php else: ?><div class="fm2-order-team"><h2>Состав ещё не выбран</h2><p>Сначала выберите монтажников и инженера строительного контроля.</p></div><?php endif ?>
