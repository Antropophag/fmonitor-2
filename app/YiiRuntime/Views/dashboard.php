...
<section class="shlz-chart-widget fm2-utilization-history" data-installer-utilization-history aria-labelledby="installer-utilization-title"><header class="shlz-chart-widget__header"><div><h2 id="installer-utilization-title">Загрузка монтажников и динамика</h2><p data-utilization-legend><span><i class="fm2-chart-key fm2-chart-key--start"></i>Без текущих работ</span> · <span><i class="fm2-chart-key fm2-chart-key--finish"></i>Из них без следующего назначения</span></p></div></header><div class="shlz-chart-widget__controls fm2-utilization-period"><a class="shlz-button shlz-button--secondary shlz-button--sm" href="/pilot/dashboard?utilizationTo=<?=$previousTo?>">Предыдущие 6 недель</a><strong aria-label="Период загрузки"><?=Html::encode($periodLabel)?></strong><?php if($atCurrent):?><span class="shlz-button shlz-button--secondary shlz-button--sm" aria-disabled="true">Следующие 6 недель</span><?php else:?><a class="shlz-button shlz-button--secondary shlz-button--sm" href="/pilot/dashboard?utilizationTo=<?=$nextTo?>">Следующие 6 недель</a><?php endif?></div>
<?php if($uh===[]):?><div class="shlz-chart-widget__empty"><span>За период <?=Html::encode($periodLabel)?> наблюдений нет.</span></div><?php else:?><div class="shlz-chart-widget__plot" aria-label="Динамика загрузки за <?=Html::encode($periodLabel)?>"><div class="fm2-utilization-weeks"><?php foreach($utilizationWeeks as$week):$point=$week['point'];$weekLabel=$week['from']->format('d.m').'–'.$week['to']->format('d.m');?><div class="fm2-utilization-week" data-utilization-week><span class="fm2-chart-week__label" aria-label="<?=Html::encode($weekLabel)?>"><span><?=$week['from']->format('d.m')?></span><i aria-hidden="true"></i><span><?=$week['to']->format('d.m')?></span></span><?php if($point===null):?><div class="fm2-utilization-week__empty" aria-label="Нет наблюдений за <?=Html::encode($weekLabel)?>">—</div><?php else:$label=(new DateTimeImmutable($point['date']))->format('d.m');?><div class="fm2-utilization-week__bars" data-observation-date="<?=$point['date']?>"><a class="fm2-chart-bar fm2-chart-bar--utilization-current" data-series="without_current" data-count="<?=$point['without_current']?>" href="/pilot/dashboard/installers/observations/<?=$point['date']?>/without_current" aria-label="Без текущих работ, <?=$label?>: <?=$point['without_current']?>"><?=$utilizationMark($point['without_current'],$umax)?><span class="fm2-chart-bar__value"><?=$point['without_current']?></span></a><a class="fm2-chart-bar fm2-chart-bar--utilization-next" data-series="without_next" data-count="<?=$point['without_next']?>" href="/pilot/dashboard/installers/observations/<?=$point['date']?>/without_next" aria-label="Из них без следующего назначения, <?=$label?>: <?=$point['without_next']?>"><?=$utilizationMark($point['without_next'],$umax)?><span class="fm2-chart-bar__value"><?=$point['without_next']?></span></a></div><?php endif?></div><?php endforeach?></div></div><?php endif?></section>
<?php endif;$stageTitle='Объекты по этапам процесса';$weekTitle='Плановая нагрузка на 6 недель';$riskTitle='Риск срыва ближайших стартов';
