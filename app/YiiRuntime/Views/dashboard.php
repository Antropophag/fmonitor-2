...
<?php if($installerForecastUnavailable):?><section class="shlz-chart-widget fm2-installer-forecast" data-dashboard-chart="installer-forecast" role="status"><h2>Прогноз загрузки на 6 недель</h2><p>Прогноз временно недоступен. Нулевые значения не публикуются.</p></section><?php else:$forecastNames=['busy'=>'Заняты','free'=>'Свободны','releasing'=>'Освобождаются','conflict'=>'Конфликты','unknown'=>'Нет данных']; ?>
<section id="installer-forecast" class="shlz-chart-widget fm2-installer-forecast" data-dashboard-chart="installer-forecast" aria-labelledby="installer-forecast-title"><header class="shlz-chart-widget__header"><div><h2 id="installer-forecast-title">Прогноз загрузки на 6 недель</h2><p class="fm2-forecast-legend"><?php foreach($forecastNames as$key=>$name):?><span data-forecast-legend="<?=$key?>"><?=Html::encode($name)?></span><?php endforeach?></p></div></header><div class="fm2-forecast-scroll" tabindex="0"><div class="fm2-forecast-grid"><?php foreach($installerForecast['weeks']as$week):$label=(new DateTimeImmutable($week['start']))->format('d.m').'–'.(new DateTimeImmutable($week['end']))->format('d.m');?><section class="fm2-forecast-week"><h3><?=$label?></h3><p><?=$week['denominator']?> монтажников</p><div><?php foreach($forecastNames as$key=>$name):?><a class="fm2-forecast-value fm2-forecast-value--<?=$key?>" href="/pilot/dashboard/installers/forecast/<?=$week['start']?>/<?=$key?>" aria-label="<?=Html::encode($name.', '.$label.': '.$week[$key])?>"><strong><?=$week[$key]?></strong><span><?=Html::encode($name)?></span></a><?php endforeach?></div></section><?php endforeach?></div></div></section>
<?php endif?>
<?php if($installerUtilizationUnavailable):?><section class="shlz-chart-widget fm2-utilization-history" data-installer-utilization-history-unavailable role="status"><h2>Загрузка монтажников и динамика</h2><p>Сводка временно недоступна: кадровый источник или официальные назначения неполны. Нулевые значения не публикуются.</p></section><?php else:$util=$installerUtilization;$uh=$installerUtilizationHistory;$uf=$uh[0]??null;$ul=$uh===[]?null:$uh[count($uh)-1];$umax=max(1,...array_merge([1],array_column($uh,'without_current'))); ?>
