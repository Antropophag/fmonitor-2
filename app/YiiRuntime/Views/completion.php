...
$submittedFormVisible = match ($submittedAction) {
    ...
    'record_declaration' => $at85 && $pto !== null && $canRecordDeclaration && ($declaration === null || $errorMessage !== ''),
...
<section id="completion" class="fm2-panel fm2-completion">
    ...
    <?php if ($submittedAction === 'record_declaration' && $errorMessage !== '' && $at85 && $pto !== null && $declaration !== null && $canRecordDeclaration): ?>
        <form class="fm2-completion-form" method="post" action="/pilot/objects/<?= (int) $id ?>/completion" data-completion-form="record_declaration"><?= Html::hiddenInput('_csrf', $csrf) ?><?= Html::hiddenInput('action', 'record_declaration') ?><?= $generalError('record_declaration') ?><label class="shlz-field"><span class="shlz-field__label">Дата декларации</span><input class="shlz-input" type="date" name="declarationDate" value="<?= Html::encode($value('record_declaration', 'declarationDate', $today)) ?>" required max="<?= $today ?>"></label><label class="shlz-field fm2-completion-details"><span class="shlz-field__label">Реквизиты декларации</span><input class="shlz-input" name="declarationDetails" value="<?= Html::encode($value('record_declaration', 'declarationDetails', '')) ?>" required maxlength="500"></label><button class="shlz-button shlz-button--primary" type="submit">Завершить работы</button></form>
    <?php endif ?>
