...
<article class="fm2-object-data" data-work-surface>
    ...
    <dialog class="shlz-modal fm2-object-details-modal" data-object-details-dialog <?= ($detailEditor['open']??false)?'data-object-details-invalid':'' ?> aria-labelledby="object-details-title"><form class="shlz-modal__surface" method="post" action="/pilot/objects/<?= (int)$id ?>/details"><header class="shlz-modal__header"><div><h2 class="shlz-modal__title" id="object-details-title">Редактировать данные объекта</h2></div><button class="shlz-modal__close" type="button" data-object-details-close aria-label="Закрыть"><span aria-hidden="true">×</span></button></header><div class="shlz-modal__body">
        ...
        <fieldset><legend>Классификация оборудования</legend><?php foreach(['floors'=>'Этажность','weight'=>'Грузоподъёмность','speed'=>'Скорость']as$name=>$label): $error=$editorErrors[$name]??null; ?><label class="fm2-details-field"><span><?= $label ?></span><input class="shlz-input" inputmode="decimal" name="<?= $name ?>" value="<?= Html::encode((string)($editorValues[$name]??'')) ?>" <?= $error!==null?'aria-invalid="true" aria-describedby="details-error-'.$name.'"':'' ?>><?php if($error!==null): ?><span id="details-error-<?= $name ?>" role="alert"><?= Html::encode($error) ?></span><?php endif ?></label><?php endforeach ?><?php foreach(['pittype'=>'Тип шахты','pitmaterial'=>'Материал шахты','lift_type'=>'Тип лифта']as$name=>$label): $error=$editorErrors[$name]??null;$selected=(string)($editorValues[$name]??'');$options=ObjectDetailsReferenceCatalogue::options($name);if($selected!==''&&!array_key_exists($selected,$options))$options=[$selected=>'Недопустимое значение: '.$selected]+$options;$attributes=['class'=>'shlz-select','prompt'=>'Не изменять'];if($error!==null){$attributes['aria-invalid']='true';$attributes['aria-describedby']='details-error-'.$name;} ?><label class="fm2-details-field"><span><?= $label ?></span><?= Html::dropDownList($name,$selected,$options,$attributes) ?><?php if($error!==null): ?><span id="details-error-<?= $name ?>" role="alert"><?= Html::encode($error) ?></span><?php endif ?></label><?php endforeach ?><label class="fm2-details-field"><span>Спаренность лифта</span><?= Html::dropDownList('paired',$editorValues['paired']??null,['1'=>'Да','0'=>'Нет'],['class'=>'shlz-select','prompt'=>'Не изменять']) ?></label></fieldset>
