            ...
            <div class="fm2-compact-list">
                <section class="fm2-compact-row" aria-labelledby="engineer-heading">
                    ...
                    <div class="fm2-team-content">
                        ...
                        <?php if ($canAssignEngineer): ?><form class="fm2-engineer-form" method="post" action="/pilot/objects/<?= (int) $id ?>/control-engineer-assignment"><?= Html::hiddenInput('_csrf', $csrf) ?><?= Html::hiddenInput('requestId', ViewSupport::uuid()) ?><?= Html::hiddenInput('expectedRevision', (int) ($currentEngineerAssignment['revision'] ?? 0)) ?><label class="shlz-field"><span class="shlz-field__label">Выберите инженера</span><span class="shlz-field__control"><?= Html::dropDownList('engineerUserId', null, $engineerOptions, ['class' => 'shlz-input', 'required' => true]) ?></span></label><button class="shlz-button shlz-button--secondary" type="submit">Закрепить</button></form><?php endif ?>
                    </div>
                </section>
