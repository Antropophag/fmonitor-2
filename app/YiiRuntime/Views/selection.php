...
<form method="post" action="<?= $path ?>">
    ...
    <fieldset class="fm2-order-surface">
        ...
        <section class="fm2-order-team" aria-labelledby="installers-title"><div class="fm2-order-section-head"><div><h2 id="installers-title">Монтажники</h2><p>Выберите одного или нескольких сотрудников кадрового каталога.</p></div></div>
            <div class="fm2-order-selection"><div class="fm2-picker-selection" data-main-selection aria-live="polite"><?php if (!$selected): ?><span class="fm2-picker-selection-empty" data-selection-empty>Монтажники ещё не выбраны</span><?php endif ?><?php foreach ($selected as $installer): ?><span class="fm2-picker-chip" data-selected-installer data-tab-id="<?= (int) $installer['tabId'] ?>" data-full-name="<?= Html::encode($installer['fullName']) ?>"><span><?= Html::encode($installer['fullName']) ?></span><small>№ <?= str_pad((string) $installer['tabId'], 6, '0', STR_PAD_LEFT) ?></small><button type="button" data-remove-installer aria-label="Убрать <?= Html::encode($installer['fullName']) ?>"><img src="/pilot/assets/shlz-icons/close.svg" alt=""></button><?= Html::hiddenInput('installerTabIds[]', $installer['tabId']) ?></span><?php endforeach ?></div><button class="shlz-button shlz-button--primary" type="button" data-dialog-open>Выбрать монтажников</button></div>
        </section>
        <section class="fm2-order-engineer"><div><span>Инженер строительного контроля</span><small>Закрепляется отдельно в карточке объекта</small></div><div><?php if($assignment['status']==='found'): ?><strong><?= Html::encode($assignment['engineer']['fullName']) ?></strong><?php else: ?><p>Сначала закрепите инженера в карточке объекта</p><?php endif ?></div></section>
