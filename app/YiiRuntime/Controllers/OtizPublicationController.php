...
{
 public function behaviors():array{return['verbs'=>['class'=>VerbFilter::class,'actions'=>['calculate'=>['POST'],'accept'=>['POST']]],'access'=>['class'=>AccessControl::class,'rules'=>[['allow'=>true,'roles'=>['otiz.manage']]],'denyCallback'=>function():void{if(Yii::$app->user->isGuest){$path=Yii::$app->request->pathInfo;$return=$path==='pilot/otiz/calculate'?'/pilot/otiz/payments':(preg_match('#^pilot/otiz/snapshots/(\d+)/accept$#D',$path,$m)?'/pilot/otiz/snapshots/'.$m[1]:'/pilot/otiz');Yii::$app->user->setReturnUrl($return);Yii::$app->response->statusCode=303;Yii::$app->response->headers->set('Location','/pilot/otiz/login');return;}throw new ForbiddenHttpException();}]];}
 ...
 public function actionAccept(int$id):Response
 {
  ...
  catch(DomainException$e){if($e->getMessage()==='NOT_FOUND')throw new NotFoundHttpException();if($e->getMessage()==='FORBIDDEN')throw new ForbiddenHttpException();if($e->getMessage()==='STALE_CALCULATION'){Yii::$app->response->statusCode=409;Yii::$app->response->content='Требуется новый расчёт.';return Yii::$app->response;}$code=match($e->getMessage()){'IMMUTABLE'=>'immutable','BLOCKERS'=>'blockers','SNAPSHOT_INCOMPLETE'=>'incomplete',default=>throw$e};return$this->go('/pilot/otiz/snapshots/'.$id.'?error='.$code);}
 }
