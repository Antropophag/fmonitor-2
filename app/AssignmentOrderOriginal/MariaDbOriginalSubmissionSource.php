...
final readonly class MariaDbOriginalSubmissionSource
{
    ...
    public function read(int $object,int $order,bool $lock=false):?array
    {
        $s=$this->sql;$suffix=$lock?' FOR UPDATE':'';$cases=$s->rows('SELECT id FROM '.$s->table('fm2_installation_cases').' WHERE legacy_installation_object_id='.$object.$suffix);
        ...
        $composition=MariaDbRegisteredCompositionQuery::read($s,$case,$order,null,$lock);
        ...
        $members=$s->rows('SELECT * FROM '.$s->table('fm2_assignment_order_selection_members').' WHERE assignment_order_id='.$order.' ORDER BY installer_tab_id'.$suffix);
        $reader=new AssignmentOrderOriginalStoredReader($s,$lock);$line=$reader->assignment($case,$order);$current=null;
        if($line->status()===AssignmentOrderOriginalLookupStatus::FOUND){
            ...
            $rows=$s->rows('SELECT request_id FROM '.$s->table('fm2_assignment_order_original_revisions').' WHERE revision_id='.$s->quote($line->currentRevisionId()).$suffix);
