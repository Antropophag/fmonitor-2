...
final class FMonitorPassivePdfInspector implements AssignmentOrderOriginalPdfInspector{
 ...
 private function xrefStream(string$b,int$offset,int&$decodedBytes):?array{
  ...
  if(preg_match('/\/Filter\s*\/(\w+)/',$x[3],$filter)){if($filter[1]!=='FlateDecode')return null;$payload=@gzuncompress($payload,67108865-$decodedBytes);if(!is_string($payload)||!$this->accountDecoded($decodedBytes,strlen($payload)))return null;}
