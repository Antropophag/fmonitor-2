...
final readonly class MariaDbConstructionControlQueue
{
    ...
    public function read(int $page,int $size=50):array
    {
        ...
        $sql='SELECT c.legacy_installation_object_id object_id,m.ordadr_address address,m.entrance,m.regnumber registration_number,m.ptoactdate,'.$activitySelect.' last_activity_at,'
        ...
        foreach($selected as$row){$id=filter_var($row['object_id'],FILTER_VALIDATE_INT,['options'=>['min_range'=>1]]);if($id===false||isset($seen[$id])||trim((string)$row['address'])===''||trim((string)$row['entrance'])===''||trim((string)$row['registration_number'])==='')throw new PilotHttpInfrastructureUnavailable();$seen[$id]=true;$engineerId=filter_var($row['engineer_id'],FILTER_VALIDATE_INT,['options'=>['min_range'=>1]]);$engineerName=trim((string)($row['engineer_name']??''));$objects[]=['id'=>$id,'address'=>(string)$row['address'],'entrance'=>(string)$row['entrance'],'registrationNumber'=>(string)$row['registration_number'],'controlEngineer'=>$engineerId!==false&&$engineerName!==''?['userId'=>$engineerId,'fullName'=>$engineerName]:null,'completed'=>$row['ptoactdate']!==null,'lastChecklistActivityAt'=>$row['last_activity_at'],'_pagination'=>['page'=>$page,'pages'=>$pages,'total'=>$total,'pageSize'=>$size]];}
        return$objects;
    }
