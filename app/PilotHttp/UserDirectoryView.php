...
final class ProductionUserDirectoryRenderer
{
    ...
    {
        $e=[PilotView::class,'e'];$roles=$directory['roles'];$assigned=array_sum(array_column($roles,'userCount'));$latest='';foreach($roles as$role)$latest=max($latest,$role['updatedAt']);$latestLabel=$latest===''?'Нет данных':(new \DateTimeImmutable($latest))->setTimezone(new \DateTimeZone('Europe/Moscow'))->format('d.m.Y H:i');$body='<div class="fm2-page-header"><div><h1>Роли</h1><p>Справочник ролей и охват пользователей</p></div><span class="fm2-result-count">'.count($roles).' ролей</span></div><section class="fm2-directory-summary fm2-role-summary"><div><strong>'.count($roles).'</strong><span>Всего ролей</span></div><div><strong>'.count(array_filter($roles,fn($x)=>$x['active'])).'</strong><span>Активные</span></div><div><strong>'.$assigned.'</strong><span>Назначений</span></div><p><b>Снимок production:</b> '.$e($latestLabel).'<br><span>Справочник legacy FMonitor</span></p></section>';$rows='';foreach($roles as$role)$rows.='<tr><td><span class="shlz-tag fm2-role-chip">'.$e($role['name']).'</span><small>ID '.$role['id'].'</small></td><td><span class="fm2-employment fm2-employment--'.($role['active']?'active':'dismissed').'">'.($role['active']?'Активна':'Архивная').'</span></td><td><strong>'.$role['userCount'].'</strong></td></tr>';$body.='<section class="fm2-list-surface"><table class="shlz-table fm2-role-table"><thead><tr><th>Роль</th><th>Статус</th><th>Пользователей</th></tr></thead><tbody>'.$rows.'</tbody></table></section>';return PilotView::document($actor,'Роли','Роли','',$body);
    }
}
