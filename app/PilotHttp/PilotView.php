...
final class PilotView
{
    ...
    {
        $objects=$current==='Объекты монтажа'?' aria-current="page"':'';$installers=$current==='Монтажники'?' aria-current="page"':'';$users=$current==='Пользователи'?' aria-current="page"':'';
        ...
        $nav='<span class="fm2-nav-group">Работа</span>'.$item('work','Моя работа').'<a class="fm2-nav-item" href="/pilot/objects"'.$objects.'>'.self::icon('objects').'<span class="fm2-nav-text">Объекты монтажа</span></a>'.$item('orders','Распоряжения').$item('inspections','Инспекции')
            ...
            .'<span class="fm2-nav-group">Управление</span>'.$item('otiz','Расчёты ОТиЗ').$item('control','Контроль').'<a class="fm2-nav-item" href="/pilot/users"'.$users.'>'.self::icon('admin').'<span class="fm2-nav-text">Пользователи</span></a>';
