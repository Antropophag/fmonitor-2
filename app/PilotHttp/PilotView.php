...
final class PilotView
{
    ...
    {
        $objects=$current==='Объекты монтажа'?' aria-current="page"':'';$installers=$current==='Монтажники'?' aria-current="page"':'';$users=$current==='Пользователи'?' aria-current="page"':'';$roles=$current==='Роли'?' aria-current="page"':'';$adminOpen=$users!==''||$roles!=='';
        ...
        $nav='<span class="fm2-nav-group">Работа</span>'.$item('work','Моя работа').'<a class="fm2-nav-item" href="/pilot/objects"'.$objects.'>'.self::icon('objects').'<span class="fm2-nav-text">Объекты монтажа</span></a>'.$item('orders','Распоряжения').$item('inspections','Инспекции')
            ...
            .'<span class="fm2-nav-group">Управление</span>'.$item('otiz','Расчёты ОТиЗ').$item('control','Контроль').'<details class="fm2-admin-nav"'.($adminOpen?' open':'').'><summary class="fm2-nav-item"'.($adminOpen?' aria-current="page"':'').'>'.self::icon('admin').'<span class="fm2-nav-text">Администрирование</span><svg class="fm2-admin-chevron" viewBox="0 0 16 16" aria-hidden="true"><path d="m5 3 5 5-5 5"/></svg></summary><div class="fm2-admin-links"><a href="/pilot/admin/users"'.$users.'>Пользователи</a><a href="/pilot/admin/roles"'.$roles.'>Роли</a></div></details>';
