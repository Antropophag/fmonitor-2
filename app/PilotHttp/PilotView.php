...
final class PilotView
{
    ...
    public static function pagination(string $path,int $current,int $pages,string $label,array $query=[]):string
    {
        ...
        $items='<li>'.($current>1?'<a class="shlz-pagination__item fm2-pagination-item" rel="prev" href="'.$href($current-1).'" aria-label="Предыдущая страница">'.$icon('left').'</a>':'<span class="shlz-pagination__item shlz-pagination__item--disabled fm2-pagination-item" aria-disabled="true">'.$icon('left').'<span class="shlz-visually-hidden">Предыдущая страница недоступна</span></span>').'</li>';
        ...
        foreach($window as$page){if($previous!==0&&$page>$previous+1)$items.='<li class="fm2-pagination__context"><span class="shlz-pagination__item shlz-pagination__item--ellipsis fm2-pagination-item" aria-hidden="true">…</span></li>';$context=$page!==1&&$page!==$pages&&$page!==$current?' class="fm2-pagination__context"':'';$items.='<li'.$context.'><a class="shlz-pagination__item fm2-pagination-item" href="'.$href($page).'"'.($page===$current?' aria-current="page"':'').' aria-label="Страница '.$page.'">'.$page.'</a></li>';$previous=$page;}
        $items.='<li>'.($current<$pages?'<a class="shlz-pagination__item fm2-pagination-item" rel="next" href="'.$href($current+1).'" aria-label="Следующая страница">'.$icon('right').'</a>':'<span class="shlz-pagination__item shlz-pagination__item--disabled fm2-pagination-item" aria-disabled="true">'.$icon('right').'<span class="shlz-visually-hidden">Следующая страница недоступна</span></span>').'</li>';
        return'<nav class="shlz-pagination" aria-label="'.self::e($label).'"><ul class="shlz-pagination__list fm2-pagination-list">'.$items.'</ul></nav>';
    }
    ...
    public static function document(HttpUser $user,string $title,string $current,string $breadcrumb,string $content,bool $canonicalJourney=false):string
    {
        if(!$canonicalJourney){
            $work=$current==='Моя работа'?' aria-current="page"':'';$objects=$current==='Объекты монтажа'?' aria-current="page"':'';$constructionControl=$current==='Стройконтроль'?' aria-current="page"':'';$installers=$current==='Монтажники'?' aria-current="page"':'';$users=$current==='Пользователи'?' aria-current="page"':'';$roles=$current==='Роли'?' aria-current="page"':'';
            ...
            $nav='<span class="fm2-nav-group">Работа</span><a class="fm2-nav-item" href="/pilot/"'.$work.'>'.self::icon('work').'<span class="fm2-nav-text">Моя работа</span></a>';
            ...
            return '<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>'.self::e($title).' · FMonitor</title><link rel="stylesheet" href="/pilot/assets/shlz.css"><link rel="stylesheet" href="/pilot/assets/pilot.css"></head><body class="shlz-scope"><a class="fm2-skip shlz-link" href="#main-content">Перейти к содержанию</a><div class="fm2-shell"><aside class="fm2-sidebar"><div class="fm2-nav-body"><a class="fm2-logo" href="/pilot/objects" aria-label="FMonitor — объекты монтажа">'.self::logo().'</a><nav class="fm2-primary-nav" aria-label="Основная навигация">'.$nav.'</nav></div>'.$userMenu.'<details class="fm2-nav-state" open>'.$trigger.'</details></aside><div class="fm2-workspace"><main class="fm2-main" id="main-content" tabindex="-1">'.$breadcrumb.$content.'</main></div></div><script src="/pilot/assets/navigation.js"></script></body></html>';
        }
