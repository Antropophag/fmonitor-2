...
final class PilotCommandSession
{
    ...
    {
        if($this->id===null||$this->state===null)throw new PilotHttpInfrastructureUnavailable();$result=$this->storage->writeCommit($this->id,$this->encoded());if($result->status()!==PilotSessionOperationStatus::OK)throw new PilotHttpInfrastructureUnavailable();$this->id=$result->currentSessionId();
    }
    private function encoded():string{$bytes=(new PilotSessionPayloadCodec())->encode($this->state??[]);if($bytes===null)throw new PilotHttpInfrastructureUnavailable();return$bytes;}
