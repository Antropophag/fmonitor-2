...
final class ShlzCssAsset implements CssAsset
{
    ...
    public function __construct(private readonly string $path,private readonly CssDescriptorOpener $descriptors){}
...
final class MariaDbPrepareFormReader implements PrepareFormReader
{
    ...
    public function read(int $id,string $businessDate):array|null
    {
        try{
            if(self::date($businessDate)!==$businessDate)throw new PilotHttpInfrastructureUnavailable();$cases=$this->many('SELECT id,legacy_installation_object_id,process_state,actual_start_date,opened_at,opened_by_user_id FROM fm2_installation_cases WHERE legacy_installation_object_id=? LIMIT 2',$id);$legacy=$this->many("SELECT id,ordadr_address,entrance,regnumber,workdatestart,workdateendadjusted,plan_finish_date,workdatefinish,ptoactdate,responsstroicontrol FROM `{$this->prefix}fm_maintable` WHERE id=? LIMIT 2",$id);
    ...
    private function many(string $sql,int $id):array{$s=$this->connection->prepare($sql);$s->bind_param('i',$id);$s->execute();return $s->get_result()->fetch_all(MYSQLI_ASSOC);}
...
final class MariaDbObjectListReader implements ObjectListReader
{
    ...
    {
        try{$projectionColumns=$this->connection->query("SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='fm2_assignment_orders' AND COLUMN_NAME IN ('installation_case_id','status','version_no')")->fetch_all(MYSQLI_ASSOC);if(\count($projectionColumns)!==0&&\count($projectionColumns)!==3)throw new PilotHttpInfrastructureUnavailable();$projectionAvailable=\count($projectionColumns)===3;$projection=$projectionAvailable?",(SELECT COUNT(*) FROM fm2_assignment_orders ao WHERE ao.installation_case_id=c.id AND ao.status IN ('prepared','registered')) AS active_order_count,(SELECT ao.status FROM fm2_assignment_orders ao WHERE ao.installation_case_id=c.id ORDER BY ao.version_no DESC,ao.id DESC LIMIT 1) AS latest_order_status":",0 AS active_order_count,NULL AS latest_order_status";$rows=$this->connection->query("SELECT c.id AS case_id,c.legacy_installation_object_id,c.process_state,c.actual_start_date,c.opened_at,c.opened_by_user_id,l.id AS legacy_id,l.ordadr_address,l.entrance,l.regnumber,l.workdatestart,l.workdateendadjusted,l.plan_finish_date{$projection} FROM fm2_installation_cases c LEFT JOIN `{$this->prefix}fm_maintable` l ON l.id=c.legacy_installation_object_id LIMIT 501")->fetch_all(MYSQLI_ASSOC);}catch(PilotHttpInfrastructureUnavailable $e){throw $e;}catch(\Throwable $e){throw new PilotHttpInfrastructureUnavailable('',0,$e);}
        if(\count($rows)>500)throw new PilotHttpInfrastructureUnavailable();
        $objects=[];$caseIds=[];$objectIds=[];
        foreach($rows as $row){$caseId=self::positiveId($row['case_id']);$id=self::positiveId($row['legacy_installation_object_id']);$legacyId=self::positiveId($row['legacy_id']);$address=\trim((string)$row['ordadr_address']);$entrance=\trim((string)$row['entrance']);$registration=\trim((string)$row['regnumber']);$start=self::date($row['workdatestart']);$finish=self::date($row['workdateendadjusted'])??self::date($row['plan_finish_date']);$active=(int)$row['active_order_count'];$opened=$row['actual_start_date']!==null||$row['opened_at']!==null||$row['opened_by_user_id']!==null;$allOpened=$row['actual_start_date']!==null&&$row['opened_at']!==null&&self::positiveId($row['opened_by_user_id'])!==null;if($caseId===null||$id===null||$legacyId!==$id||isset($caseIds[$caseId])||isset($objectIds[$id])||$address===''||$entrance===''||$registration===''||$start===null||$finish===null||$opened!==$allOpened||$active<0||$active>1)throw new PilotHttpInfrastructureUnavailable();$status=match((string)$row['process_state']){'needs_assignment_order'=>(!$opened&&$active===0&&$row['latest_order_status']===null)?'Требуется распоряжение':throw new PilotHttpInfrastructureUnavailable(),'assignment_order_prepared'=>(!$opened&&$active===1&&$row['latest_order_status']==='prepared')?'Распоряжение подготовлено':((!$opened&&$active===1&&$row['latest_order_status']==='registered')?'Готов к открытию':throw new PilotHttpInfrastructureUnavailable()),'working'=>($allOpened&&$active===1&&$row['latest_order_status']==='registered')?'В работе':throw new PilotHttpInfrastructureUnavailable(),'needs_assignment_change'=>($allOpened&&$active===1&&$row['latest_order_status']==='registered')?'Требуется изменение':throw new PilotHttpInfrastructureUnavailable(),default=>throw new PilotHttpInfrastructureUnavailable()};$caseIds[$caseId]=true;$objectIds[$id]=true;$objects[]=['id'=>$id,'registrationNumber'=>$registration,'address'=>$address,'entrance'=>$entrance,'plannedStartDate'=>$start,'plannedFinishDate'=>$finish,'status'=>$status];}
...
final class ProductionPilotHttpDependencies implements PilotHttpDependencies,ObjectCardReaderProvider,ObjectListReaderProvider,PrepareFormReaderProvider
{
    ...
    public function objectCards():ObjectCardReader
    {
        ...
        return $this->objectCardReader=new MariaDbObjectCardReader($this->connection,$this->legacyTablePrefix);
    }
    public function objectList():ObjectListReader
    {
        if($this->objectListReader!==null)return $this->objectListReader;$this->users();if(!$this->connection instanceof \mysqli||$this->legacyTablePrefix===null)throw new PilotHttpInfrastructureUnavailable();return $this->objectListReader=new MariaDbObjectListReader($this->connection,$this->legacyTablePrefix);
    }
    public function prepareForms():PrepareFormReader
    {
        if($this->prepareFormReader!==null)return $this->prepareFormReader;$this->users();if(!$this->connection instanceof \mysqli||$this->legacyTablePrefix===null)throw new PilotHttpInfrastructureUnavailable();return $this->prepareFormReader=new MariaDbPrepareFormReader($this->connection,$this->legacyTablePrefix);
    }
