...
{
    private array $members=[];private array $assets=[];private array $directories=[];private int $trustedUid;
    public function __construct(string $entry,private readonly CssDescriptorOpener $opener){if($entry===''||$entry[0]!=='/'||\basename($entry)!=='shlz.css')throw new CssAssetUnavailable();$root=\dirname($entry);ManifestCssAsset::validateComponents($root,true);ManifestCssAsset::validateComponents($entry);$root=\realpath($root);$canonical=\realpath($entry);if($root===false||$canonical===false||\dirname($canonical)!==$root)throw new CssAssetUnavailable();$rootStat=ManifestCssAsset::identity($root);$euid=\function_exists('posix_geteuid')?\posix_geteuid():$rootStat['uid'];if($rootStat['uid']!==$euid)throw new CssAssetUnavailable();$this->trustedUid=$euid;$this->captureDirectory($root);$this->walk($root,'shlz.css',1,0);\ksort($this->members);foreach($this->directories as$path=>$identity)if(ManifestCssAsset::identity($path)!==$identity)throw new CssAssetUnavailable();foreach($this->members as$member){$asset=new ManifestCssAsset($member['path'],$member['identity'],$this->opener);try{$asset->readBytes();}finally{$asset->close();}}}
    ...
    private function walk(string $root,string $relative,int $depth,int $total):int{if(isset($this->members[$relative]))return$total;if($depth>32||\count($this->members)>=256)throw new CssAssetUnavailable();$path=$root.'/'.$relative;ManifestCssAsset::validateComponents($path);$canonical=\realpath($path);if($canonical===false||!\str_starts_with($canonical,$root.'/'))throw new CssAssetUnavailable();$directory=\dirname($path);while(\str_starts_with($directory,$root.'/')){$this->captureDirectory($directory);$directory=\dirname($directory);}$identity=ManifestCssAsset::identity($path);if($identity['uid']!==$this->trustedUid||($identity['mode']&0022)!==0||($identity['mode']&0111)!==0)throw new CssAssetUnavailable();$a=new ManifestCssAsset($path,$identity,$this->opener);try{$bytes=$a->readBytes();}finally{$a->close();}$identity['hash']=\hash('sha256',$bytes);$total+=\strlen($bytes);if($total>8388608||\preg_match('//u',$bytes)!==1)throw new CssAssetUnavailable();$this->members[$relative]=['path'=>$path,'identity'=>$identity];foreach($this->imports($bytes)as$target){$resolved=$this->resolve($relative,$target);if($resolved==='pilot.css')throw new CssAssetUnavailable();$total=$this->walk($root,$resolved,$depth+1,$total);}return$total;}
    private function captureDirectory(string $path):void{$identity=ManifestCssAsset::identity($path);if(($identity['mode']&0170000)!==0040000||$identity['uid']!==$this->trustedUid||($identity['mode']&0022)!==0)throw new CssAssetUnavailable();if(isset($this->directories[$path])&&$this->directories[$path]!==$identity)throw new CssAssetUnavailable();$this->directories[$path]=$identity;}
