...
final class ProductionObjectCardRenderer implements ObjectCardRenderer
{
    ...
    {
        $e=static fn(mixed $v):string=>\htmlspecialchars((string)$v,ENT_QUOTES|ENT_SUBSTITUTE|ENT_HTML5,'UTF-8');
        $pair=static fn(string $term,string $value):string=>'<dt class="shlz-text">'.\htmlspecialchars($term,ENT_QUOTES|ENT_SUBSTITUTE|ENT_HTML5,'UTF-8').'</dt><dd class="shlz-text">'.$value.'</dd>';
        $section=static fn(string $title,string $rows):string=>'<section><h2 class="shlz-text">'.$title.'</h2><dl>'.$rows.'</dl></section>';
        $order=$c['order'];
        $team='';
        if($order===null){$team.=$pair('Распоряжение','Распоряжение ещё не сформировано');$team.=$pair('Команда','Подтверждённая команда ещё не сформирована');if(($c['canPrepare']??false)&&!($c['hasPtoAct']??false)&&$c['status']==='Требуется распоряжение')$team.='<p><a class="shlz-link" href="/pilot/objects/'.$e($c['id']).'/assignment-order/prepare">Сформировать распоряжение</a></p>';}
        else{
            $document=$order['status']==='prepared'?'Ожидается номер 1С ДО':'Зарегистрировано в 1С ДО';
            $caption=$order['registrationNumber']===null?'Распоряжение от '.$e($order['orderDate']).' · версия '.$e($order['version']):'Распоряжение № '.$e($order['registrationNumber']).' от '.$e($order['orderDate']).' · версия '.$e($order['version']);
            if($order['status']==='registered'&&$c['status']!=='В работе'){$team.=$pair('Распоряжение',$caption);$team.=$pair('Статус распоряжения',$document);}
            else{$team.=$pair('Статус распоряжения',$document);$team.=$pair('Распоряжение',$caption);}
            $team.=$pair('Инженер',$e($order['engineer']['fullName']).' · '.$e($order['engineer']['position']).' · '.$e($order['engineer']['userId']));
            foreach($order['installers'] as $installer)$team.=$pair('Исполнитель',$e($installer['tabId']).' · '.$e($installer['fullName']).' · '.$e($installer['position']).' · '.$e($installer['status']));
            $team.=$pair('Форма организации труда',$order['organizationType']==='brigade'?'Бригадная':'Индивидуальная');
        }
        $work=$c['opened']
            ?$pair('Фактическое начало','Фактическое начало '.$e($c['actualStartDate'])).$pair('Аудит открытия','Открыто: '.$e($c['openedAt']).' · Открыл пользователь: '.$e($c['openedByUserId'])).$pair('Чек-лист','Чек-лист: Доступен')
            :$pair('Состояние работ','Работы ещё не открыты');
        $events='';
...
final class ProductionPrepareFormRenderer implements PrepareFormRenderer
{
    ...
    {
        $e=static fn(mixed $v):string=>\htmlspecialchars((string)$v,ENT_QUOTES|ENT_SUBSTITUTE|ENT_HTML5,'UTF-8');$installerControls='';
        if($f['installers']===[])$installerControls='<p class="shlz-text">Нет монтажников, допустимых для планового периода объекта.</p>';
...
final class MariaDbPrepareFormReader implements PrepareFormReader
{
    ...
    public function read(int $id,string $businessDate):array|null
    {
        try{
            ...
            $fields='installer_tab_id,fio,position,employment_status,employed_from,employed_to,workforce_source,workforce_source_updated_at'.($history?',reconciliation_state,last_successful_sync_run_id,last_successful_sync_at':'');$workforce=$this->connection->query("SELECT {$fields} FROM fm2_workforce_catalog LIMIT 502")->fetch_all(MYSQLI_ASSOC);$installers=[];$seen=[];foreach($workforce as $x){$tab=self::positiveId($x['installer_tab_id']);$fio=\trim((string)$x['fio']);$position=\trim((string)$x['position']);$source=\trim((string)$x['workforce_source']);$from=self::date($x['employed_from']);$to=$x['employed_to']===null?null:self::date($x['employed_to']);if($tab===null||isset($seen[$tab])||$fio===''||$position===''||$source===''||$from===null||($x['employed_to']!==null&&$to===null)||!self::rfc3339($x['workforce_source_updated_at']))throw new PilotHttpInfrastructureUnavailable();$seen[$tab]=true;$reconciliation='delivered';if($history){$provenance=[$x['reconciliation_state'],$x['last_successful_sync_run_id'],$x['last_successful_sync_at']];$legacy=\count(\array_filter($provenance,static fn(mixed $v):bool=>$v!==null))===0;if(!$legacy&&!\in_array($x['reconciliation_state'],['delivered','missing_from_delivery'],true))throw new PilotHttpInfrastructureUnavailable();if(!$legacy&&(!self::rfc3339($x['last_successful_sync_at'])||$x['last_successful_sync_run_id']!==$runId||$x['last_successful_sync_at']!==$syncAt))throw new PilotHttpInfrastructureUnavailable();$reconciliation=$legacy?'delivered':$x['reconciliation_state'];}if($reconciliation==='delivered'&&$x['employment_status']==='employed'&&$from<=$businessDate&&($to===null||$to>=$finish))$installers[]=['tabId'=>$tab,'fio'=>$fio,'position'=>$position,'source'=>$source,'updatedAt'=>(string)$x['workforce_source_updated_at']];}if(\count($installers)>500)throw new PilotHttpInfrastructureUnavailable();\usort($installers,static fn(array $a,array $b):int=>[$a['fio'],$a['tabId']]<=>[$b['fio'],$b['tabId']]);
...
final class MariaDbObjectListReader implements ObjectListReader
{
    ...
    {
        try{$rows=$this->connection->query("SELECT c.id AS case_id,c.legacy_installation_object_id,l.id AS legacy_id,l.ordadr_address,l.entrance,l.regnumber,l.workdatestart,l.workdateendadjusted,l.plan_finish_date FROM fm2_installation_cases c LEFT JOIN `{$this->prefix}fm_maintable` l ON l.id=c.legacy_installation_object_id LIMIT 501")->fetch_all(MYSQLI_ASSOC);}catch(\Throwable $e){throw new PilotHttpInfrastructureUnavailable('',0,$e);}
