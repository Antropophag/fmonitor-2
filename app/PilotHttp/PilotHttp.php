...
final class MariaDbObjectCardReader implements ObjectCardReader
{
    ...
    public function read(int $id):array|null
    {
        try{
            ...
            $pto=$legacy===null?null:self::optionalLegacyDate($legacy['ptoactdate']);if($activeProvenance!==null)unset($activeProvenance['legacyObject']);return $card+['status'=>$status,'order'=>$renderedOrder,'controlEngineer'=>$controlEngineer,'opened'=>$allOpening||$migratedActive,'actualStartDate'=>$case['actual_start_date'],'actualStartDateUnknownAtCutover'=>$migratedActive,'openedAt'=>$case['opened_at'],'openedByUserId'=>$allOpening?self::positiveId($case['opened_by_user_id']):null,'activeCaseProvenance'=>$activeProvenance,'events'=>$renderedEvents,'hasPtoAct'=>$pto!==null];
...
final class MariaDbObjectListReader implements ObjectListReader
{
    ...
    public function __construct(private readonly \mysqli $connection,private readonly string $prefix,private readonly string $processPrefix=''){}
    public function read():array{$raw=$_GET['page']??1;$page=filter_var($raw,FILTER_VALIDATE_INT,['options'=>['min_range'=>1]]);if($page===false)throw new PilotHttpInfrastructureUnavailable();$result=$this->readPage($page);foreach($result['objects']as&$object)$object['_pagination']=array_diff_key($result,['objects'=>true]);return$result['objects'];}
    public function readPage(int$page):array
    {
        if($page<1)throw new PilotHttpInfrastructureUnavailable();$offset=($page-1)*self::PAGE_SIZE;
        try{$caseCount=(int)$this->connection->query("SELECT COUNT(*) n FROM `{$this->processPrefix}fm2_installation_cases`")->fetch_assoc()['n'];$provenanceTable=$this->connection->real_escape_string($this->processPrefix.'fm2_active_case_provenance');$baselineTable=$this->connection->real_escape_string($this->processPrefix.'fm2_legacy_active_baselines');$migrationTables=(int)$this->connection->query("SELECT COUNT(*) n FROM information_schema.TABLES WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME IN ('{$provenanceTable}','{$baselineTable}')")->fetch_assoc()['n'];$migrationSelect=$migrationTables===2?'p.legacy_object_id provenance_legacy_id,p.baseline_id,p.baseline_sha256 provenance_baseline_hash,b.content_sha256 baseline_hash,b.payload_json baseline_payload':'NULL provenance_legacy_id,NULL baseline_id,NULL provenance_baseline_hash,NULL baseline_hash,NULL baseline_payload';$migrationJoin=$migrationTables===2?" LEFT JOIN `{$this->processPrefix}fm2_active_case_provenance` p ON p.installation_case_id=c.id LEFT JOIN `{$this->processPrefix}fm2_legacy_active_baselines` b ON b.id=p.baseline_id":'';$rows=$this->connection->query("SELECT c.id AS case_id,c.legacy_installation_object_id,c.process_state,l.id AS legacy_id,l.ordadr_address,l.entrance,l.regnumber,l.workdatestart,l.workdateendadjusted,l.plan_finish_date,{$migrationSelect} FROM `{$this->processPrefix}fm2_installation_cases` c LEFT JOIN `{$this->prefix}fm_maintable` l ON l.id=c.legacy_installation_object_id{$migrationJoin} ORDER BY l.workdatestart IS NULL,LEFT(l.workdatestart,10),c.legacy_installation_object_id LIMIT ".self::PAGE_SIZE." OFFSET {$offset}")->fetch_all(MYSQLI_ASSOC);}catch(PilotHttpInfrastructureUnavailable$e){throw$e;}catch(\Throwable $e){throw new PilotHttpInfrastructureUnavailable('',0,$e);}
        $objects=[];$caseIds=[];$objectIds=[];
        foreach($rows as $row){$caseId=self::positiveId($row['case_id']);$id=self::positiveId($row['legacy_installation_object_id']);$legacyId=self::positiveId($row['legacy_id']);$migrated=$legacyId===null;if($migrated){try{$payload=\json_decode((string)$row['baseline_payload'],true,flags:JSON_THROW_ON_ERROR);}catch(\Throwable){throw new PilotHttpInfrastructureUnavailable();}$legacyObject=$payload['legacyObject']??null;$classification=$payload['classification']??null;if(!\is_array($legacyObject)||!\is_array($classification)||$row['process_state']!=='working'||self::positiveId($row['baseline_id'])===null||self::positiveId($row['provenance_legacy_id'])!==$id||!\hash_equals((string)$row['provenance_baseline_hash'],(string)$row['baseline_hash'])||!\hash_equals((string)$row['baseline_hash'],\hash('sha256',(string)$row['baseline_payload']))||($classification['category']??null)!=='legacy_active'||($classification['quarantineCodes']??null)!==[])throw new PilotHttpInfrastructureUnavailable();$address=\trim((string)($legacyObject['ordadr_address']??''));$entrance=\trim((string)($legacyObject['entrance']??''));$registration=\trim((string)($legacyObject['regnumber']??''));$start=null;$finish=null;}else{$address=\trim((string)$row['ordadr_address']);$entrance=\trim((string)$row['entrance']);$registration=\trim((string)$row['regnumber']);$start=self::date($row['workdatestart']);$finish=self::date($row['workdateendadjusted'])??self::date($row['plan_finish_date']);}if($caseId===null||$id===null||(!$migrated&&$legacyId!==$id)||isset($caseIds[$caseId])||isset($objectIds[$id])||$address===''||$entrance===''||$registration===''||(!$migrated&&($start===null||$finish===null)))throw new PilotHttpInfrastructureUnavailable();$caseIds[$caseId]=true;$objectIds[$id]=true;$objects[]=['id'=>$id,'registrationNumber'=>$registration,'address'=>$address,'entrance'=>$entrance,'plannedStartDate'=>$start,'plannedFinishDate'=>$finish,'planningDatesUnknownAtCutover'=>$migrated];}
        $pages=max(1,(int)\ceil($caseCount/self::PAGE_SIZE));if($page>$pages)throw new PilotHttpInfrastructureUnavailable();return['objects'=>$objects,'page'=>$page,'pages'=>$pages,'total'=>$caseCount,'pageSize'=>self::PAGE_SIZE];
    }
    private static function date(mixed $value):?string{if(!\is_string($value)||\preg_match('/^(\d{4})-(\d{2})-(\d{2})(?:[ T].*)?$/D',$value,$m)!==1||!\checkdate((int)$m[2],(int)$m[3],(int)$m[1])||$m[1]==='0000')return null;return $m[1].'-'.$m[2].'-'.$m[3];}
    private static function positiveId(mixed $value):?int{if(!\is_int($value)&&(!\is_string($value)||\preg_match('/^[1-9][0-9]*$/D',$value)!==1))return null;$id=\filter_var($value,FILTER_VALIDATE_INT,['options'=>['min_range'=>1]]);return $id===false?null:$id;}
}
