...
require_once __DIR__.'/AccessPolicy.php';require_once __DIR__.'/PilotRouteCsp.php';require_once __DIR__.'/MariaDbPrepareEngineerDirectory.php';require_once __DIR__.'/MariaDbPrepareWorkforceDirectory.php';
...
interface PrepareFormReaderProvider { public function prepareForms():PrepareFormReader; public function hasCapability(int $userId,string $capability):bool; public function hasProcessCapability(int $userId,string $capability):bool; public function processDate():string; }
...
final class MariaDbPrepareFormReader implements PrepareFormReader
{
    ...
    public function read(int $id,string $processDate):array|null
    {
        try{
            ...
            if($cases===[]&&$legacy===[])return null;if(\count($cases)!==1||\count($legacy)!==1)throw new PilotHttpInfrastructureUnavailable();$case=$cases[0];$l=$legacy[0];if(!\in_array((string)$case['process_state'],['needs_assignment_order','working','needs_assignment_change'],true))throw new PrepareFormUnavailable();
            $address=\trim((string)$l['ordadr_address']);$entrance=\trim((string)$l['entrance']);$registration=\trim((string)$l['regnumber']);$start=self::datePrefix($l['workdatestart']);$finish=self::datePrefix($l['workdateendadjusted'])??self::datePrefix($l['plan_finish_date']);$completion=self::optionalLegacyDate($l['workdatefinish']);$pto=self::optionalLegacyDate($l['ptoactdate']);$changing=(string)$case['process_state']!=='needs_assignment_order';if(self::positiveId($case['id'])===null||self::positiveId($case['legacy_installation_object_id'])!==$id||self::positiveId($l['id'])!==$id||$address===''||$entrance===''||$registration===''||$start===null||$finish===null||(!$changing&&($case['actual_start_date']!==null||$case['opened_at']!==null||$case['opened_by_user_id']!==null))||($changing&&($case['actual_start_date']===null||$case['opened_at']===null||$case['opened_by_user_id']===null))||$completion!==null||$pto!==null)throw new PilotHttpInfrastructureUnavailable();$currentOrders=$this->many('SELECT id,version_no,status FROM fm2_assignment_orders WHERE installation_case_id=? ORDER BY version_no DESC LIMIT 1',(int)$case['id']);if((!$changing&&$currentOrders!==[])||($changing&&(\count($currentOrders)!==1||$currentOrders[0]['status']!=='registered')))throw new PilotHttpInfrastructureUnavailable();
            ...
            $workforce=(new MariaDbPrepareWorkforceDirectory($this->connection,$this->processPrefix))->read($history);$installers=[];$seen=[];foreach($workforce as$x){$tab=self::positiveId($x['installer_tab_id']);$fio=self::pickerText($x['fio'],300);$position=self::pickerText($x['position'],160);$source=\trim((string)$x['workforce_source']);$from=$x['employed_from']===null?null:self::date($x['employed_from']);$to=$x['employed_to']===null?null:self::date($x['employed_to']);if($tab===null||$tab>999999||isset($seen[$tab])||$fio===null||$position===null||$source===''||($x['employed_from']!==null&&$from===null)||($x['employed_to']!==null&&$to===null)||!self::rfc3339($x['workforce_source_updated_at']))throw new PilotHttpInfrastructureUnavailable();$seen[$tab]=true;$reconciliation='delivered';if($history){$provenance=[$x['reconciliation_state'],$x['last_successful_sync_run_id'],$x['last_successful_sync_at']];$legacy=\count(\array_filter($provenance,static fn(mixed$v):bool=>$v!==null))===0;if(!$legacy&&!\in_array($x['reconciliation_state'],['delivered','missing_from_delivery'],true))throw new PilotHttpInfrastructureUnavailable();if(!$legacy&&(!self::rfc3339($x['last_successful_sync_at'])||$x['last_successful_sync_run_id']!==$runId||$x['last_successful_sync_at']!==$syncAt))throw new PilotHttpInfrastructureUnavailable();$reconciliation=$legacy?'delivered':$x['reconciliation_state'];}if($reconciliation==='delivered'&&$x['employment_status']==='employed'&&$from!==null&&$from<=$processDate&&($to===null||$to>=$finish))$installers[]=['tabId'=>$tab,'fio'=>$fio,'position'=>$position,'source'=>$source,'updatedAt'=>(string)$x['workforce_source_updated_at']];}if(\count($installers)>500)throw new PilotHttpInfrastructureUnavailable();\usort($installers,static fn(array$a,array$b):int=>[$a['fio'],$a['tabId']]<=>[$b['fio'],$b['tabId']]);$pairs=[];foreach($installers as$installer)$pairs[$installer['source']."\0".$installer['updatedAt']]=[$installer['source'],$installer['updatedAt']];$workforceProvenance=\count($pairs)===1?['mode'=>'group','source'=>\array_values($pairs)[0][0],'updatedAt'=>\array_values($pairs)[0][1]]:['mode'=>'row'];
            $engineers=$this->eligibleEngineers(self::positiveId($l['responsstroicontrol']));$engineer=\array_values(\array_filter($engineers,static fn(array $candidate):bool=>$candidate['selected']))[0]??null;
            $selected=[];if($changing){$selectedRows=$this->many('SELECT installer_tab_id FROM fm2_order_installers WHERE assignment_order_id=? ORDER BY installer_tab_id',(int)$currentOrders[0]['id']);$selected=\array_map(static fn(array $row):int=>(int)$row['installer_tab_id'],$selectedRows);}
            return ['id'=>$id,'registrationNumber'=>$registration,'address'=>$address,'entrance'=>$entrance,'plannedStartDate'=>$start,'plannedFinishDate'=>$finish,'installers'=>$installers,'engineers'=>$engineers,'engineer'=>$engineer,'workforceProvenance'=>$workforceProvenance,'changing'=>$changing,'currentVersion'=>$changing?(int)$currentOrders[0]['version_no']:null,'selected'=>['installers'=>$selected]];
    ...
    private static function pickerText(mixed $value,int $limit):?string{if(!\is_string($value)||!\mb_check_encoding($value,'UTF-8'))return null;$normalized=\preg_replace('/[\x{0009}-\x{000D}\x{0020}]+/u',' ',\preg_replace('/^[\x{0009}-\x{000D}\x{0020}]+|[\x{0009}-\x{000D}\x{0020}]+$/u','',$value)??'');return !\is_string($normalized)||$normalized===''||\mb_strlen($normalized,'UTF-8')>$limit?null:$normalized;}
    private function eligibleEngineers(?int $legacyUserId):array
    {
        $rows=(new MariaDbPrepareEngineerDirectory($this->connection,$this->prefix,$this->processPrefix))->read();
        if(\count($rows)>100)throw new PilotHttpInfrastructureUnavailable();$engineers=[];$seen=[];
        foreach($rows as$row){$id=self::positiveId($row['id']);$fio=self::pickerText($row['name'],300);$position=self::pickerText($row['position_snapshot'],160);if($id===null||isset($seen[$id])||$fio===null||$position===null)throw new PilotHttpInfrastructureUnavailable();$seen[$id]=true;$engineers[]=['userId'=>$id,'fio'=>$fio,'position'=>$position,'selected'=>$id===$legacyUserId];}
        \usort($engineers,static fn(array$a,array$b):int=>[$a['fio'],$a['userId']]<=>[$b['fio'],$b['userId']]);return $engineers;
    }
}
...
final class MariaDbObjectCardReader implements ObjectCardReader
{
    ...
    public function read(int $id):array|null
    {
        try{
            ...
            if($orders!==[]){
                ...
                if(\count($highest)!==1||!\in_array($highest[0]['status'],['prepared','registered'],true))throw new PilotHttpInfrastructureUnavailable();
                $order=$highest[0];
            }
            $migratedActive=$activeProvenance!==null;
            ...
            $status=match($state){
                ...
                'needs_assignment_change'=>($allOpening&&$order!==null&&\in_array($order['status'],['prepared','registered'],true))?($order['status']==='prepared'?'Изменяющее распоряжение подготовлено':'Требуется изменение') :throw new PilotHttpInfrastructureUnavailable(),
            ...
            if($controlEngineer===null&&$legacy!==null){$legacyEngineer=self::positiveId($legacy['responsstroicontrol']);if($legacyEngineer!==null)$controlEngineer=$this->engineerSnapshot($legacyEngineer);}
            $pto=$legacy===null?null:self::optionalLegacyDate($legacy['ptoactdate']);$dataOrigin=$activeProvenance!==null?'migration_active':$this->nonActiveOrigin($caseId,$id);if($activeProvenance!==null)unset($activeProvenance['legacyObject']);return $card+['status'=>$status,'order'=>$renderedOrder,'controlEngineer'=>$controlEngineer,'opened'=>$allOpening||$migratedActive,'actualStartDate'=>$case['actual_start_date'],'actualStartDateUnknownAtCutover'=>$migratedActive,'openedAt'=>$case['opened_at'],'openedByUserId'=>$allOpening?self::positiveId($case['opened_by_user_id']):null,'activeCaseProvenance'=>$activeProvenance,'dataOrigin'=>$dataOrigin,'events'=>$renderedEvents,'hasPtoAct'=>$pto!==null];
    ...
    private function activeProvenance(int $caseId,int $objectId):?array
    {
        $table=$this->processPrefix.'fm2_active_case_provenance';$check=$this->connection->prepare('SELECT TABLE_NAME FROM information_schema.TABLES WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME=? LIMIT 1');$check->bind_param('s',$table);$check->execute();if($check->get_result()->fetch_assoc()===null)return null;
        $rows=$this->many('SELECT p.legacy_object_id,p.baseline_id,p.baseline_sha256,p.cutover_at,p.template_snapshot_id,p.template_snapshot_version,p.template_content_sha256,b.content_sha256 current_baseline_sha256,b.payload_json,t.snapshot_version current_template_version,t.content_sha256 current_template_sha256 FROM fm2_active_case_provenance p JOIN fm2_legacy_active_baselines b ON b.id=p.baseline_id JOIN fm2_checklist_template_snapshots t ON t.id=p.template_snapshot_id WHERE p.installation_case_id=? LIMIT 2',$caseId);
        if($rows===[])return null;if(\count($rows)!==1)throw new PilotHttpInfrastructureUnavailable();$r=$rows[0];$payload=\json_decode((string)$r['payload_json'],true,flags:JSON_THROW_ON_ERROR);$classification=$payload['classification']??null;$reasons=$classification['reasonCodes']??null;
        if(!\is_array($classification)||($classification['category']??null)!=='legacy_active'||!\is_array($reasons)||(int)$r['legacy_object_id']!==$objectId||(int)$r['baseline_id']<1||(int)$r['template_snapshot_id']<1||!\hash_equals((string)$r['baseline_sha256'],(string)$r['current_baseline_sha256'])||!\hash_equals((string)$r['template_content_sha256'],(string)$r['current_template_sha256'])||(string)$r['template_snapshot_version']!==(string)$r['current_template_version'])throw new PilotHttpInfrastructureUnavailable();
        $facts=$payload['legacyObject']??null;if(!\is_array($facts)||\trim((string)($facts['ordadr_address']??''))===''||\trim((string)($facts['entrance']??''))===''||\trim((string)($facts['regnumber']??''))==='')throw new PilotHttpInfrastructureUnavailable();return['baselineId'=>(int)$r['baseline_id'],'baselineHash'=>(string)$r['baseline_sha256'],'cutoverAt'=>(string)$r['cutover_at'],'templateSnapshotId'=>(int)$r['template_snapshot_id'],'templateVersion'=>(string)$r['template_snapshot_version'],'templateHash'=>(string)$r['template_content_sha256'],'classificationVersion'=>(string)($classification['classificationVersion']??''),'classificationReasons'=>\array_values(\array_map('strval',$reasons)),'legacyObject'=>$facts];
    }
    private function nonActiveOrigin(int$caseId,int$objectId):string
    {
        $table=$this->processPrefix.'fm2_migration_classification_provenance';$check=$this->connection->prepare('SELECT TABLE_NAME FROM information_schema.TABLES WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME=? LIMIT 1');$check->bind_param('s',$table);$check->execute();if($check->get_result()->fetch_assoc()===null)return'demo_fixture';$s=$this->connection->prepare("SELECT category FROM `{$table}` WHERE output_kind='operational_case' AND output_id=? AND legacy_object_id=? LIMIT 2");$s->bind_param('ii',$caseId,$objectId);$s->execute();$rows=$s->get_result()->fetch_all(MYSQLI_ASSOC);if($rows===[])return'demo_fixture';if(\count($rows)!==1||$rows[0]['category']!=='native_candidate')throw new PilotHttpInfrastructureUnavailable();return'migration_native';
    }
    private function engineerSnapshot(int $id):?array{$pilotTable=$this->processPrefix.'fm2_pilot_users';$table=$this->connection->prepare('SELECT TABLE_NAME FROM information_schema.TABLES WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME=? LIMIT 1');$table->bind_param('s',$pilotTable);$table->execute();if($table->get_result()->fetch_assoc()===null)return null;$s=$this->connection->prepare("SELECT full_name FROM `{$pilotTable}` WHERE user_id=? AND status=1 AND activation_state='active' LIMIT 2");$s->bind_param('i',$id);$s->execute();$rows=$s->get_result()->fetch_all(MYSQLI_ASSOC);return \count($rows)===1?['userId'=>$id,'fullName'=>(string)$rows[0]['full_name'],'position'=>'Строительный контроль']:null;}
    private function legacyCard(array $r,int $id):array|null
    {
        $address=\trim((string)$r['ordadr_address']);$entrance=\trim((string)$r['entrance']);$registration=\trim((string)($r['regnumber']??''));$factory=\trim((string)($r['zavnumber']??''));
        $start=self::date($r['workdatestart']);$finish=self::date($r['workdateendadjusted'])??self::date($r['plan_finish_date']);
        if((int)$r['id']!==$id||$address===''||$entrance==='')return null;
        return ['id'=>$id,'address'=>$address,'entrance'=>$entrance,'registrationNumber'=>$registration,'factoryNumber'=>$factory,'plannedStartDate'=>$start,'plannedFinishDate'=>$finish];
    }
...
{
    private const PAGE_SIZE=501;
    private const MAX_OBJECTS=500;
    public function __construct(private readonly \mysqli $connection,private readonly string $prefix,private readonly string $processPrefix=''){}
    public function read():array{$result=$this->readPage(1,'all');if($result['total']>self::MAX_OBJECTS||\count($result['objects'])>self::MAX_OBJECTS)throw new PilotHttpInfrastructureUnavailable();return$result['objects'];}
    public function readPage(int$page,string$origin='all'):array
    {
        if($page<1||!self::origin($origin))throw new PilotHttpInfrastructureUnavailable();$offset=($page-1)*self::PAGE_SIZE;
        try{$names=[$this->processPrefix.'fm2_active_case_provenance',$this->processPrefix.'fm2_legacy_active_baselines',$this->processPrefix.'fm2_migration_classification_provenance'];$quoted=\implode(',',\array_map(fn(string$n):string=>"'".$this->connection->real_escape_string($n)."'",$names));$full=(int)$this->connection->query("SELECT COUNT(*) n FROM information_schema.TABLES WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME IN ({$quoted})")->fetch_assoc()['n']===3;$select=$full?'p.legacy_object_id provenance_legacy_id,p.baseline_id,p.baseline_sha256 provenance_baseline_hash,b.content_sha256 baseline_hash,b.payload_json baseline_payload,m.category migration_category':'NULL provenance_legacy_id,NULL baseline_id,NULL provenance_baseline_hash,NULL baseline_hash,NULL baseline_payload,NULL migration_category';$joins=$full?" LEFT JOIN `{$names[0]}` p ON p.installation_case_id=c.id LEFT JOIN `{$names[1]}` b ON b.id=p.baseline_id LEFT JOIN `{$names[2]}` m ON m.output_kind='operational_case' AND m.output_id=c.id AND m.legacy_object_id=c.legacy_installation_object_id AND m.category='native_candidate'":'';$filter=$full?match($origin){'migration'=>' AND (p.installation_case_id IS NOT NULL OR m.id IS NOT NULL)','migration_native'=>' AND m.id IS NOT NULL','migration_active'=>' AND p.installation_case_id IS NOT NULL','demo_fixture'=>' AND p.installation_case_id IS NULL AND m.id IS NULL','local_audit'=>' AND 1=0',default=>''}:($origin==='all'||$origin==='demo_fixture'?'':' AND 1=0');$from=" FROM `{$this->processPrefix}fm2_installation_cases` c LEFT JOIN `{$this->prefix}fm_maintable` l ON l.id=c.legacy_installation_object_id{$joins} WHERE 1=1{$filter}";$caseCount=(int)$this->connection->query("SELECT COUNT(*) n{$from}")->fetch_assoc()['n'];$order=$full?'(p.installation_case_id IS NULL AND m.id IS NULL),':'';$rows=$this->connection->query("SELECT c.id AS case_id,c.legacy_installation_object_id,c.process_state,l.id AS legacy_id,l.ordadr_address,l.entrance,l.regnumber,l.zavnumber,l.workdatestart,l.workdateendadjusted,l.plan_finish_date,{$select}{$from} ORDER BY {$order}l.workdatestart IS NULL,LEFT(l.workdatestart,10),c.legacy_installation_object_id LIMIT ".self::PAGE_SIZE." OFFSET {$offset}")->fetch_all(MYSQLI_ASSOC);}catch(PilotHttpInfrastructureUnavailable$e){throw$e;}catch(\Throwable $e){throw new PilotHttpInfrastructureUnavailable('',0,$e);}
        $objects=[];$caseIds=[];$objectIds=[];
        foreach($rows as $row){$caseId=self::positiveId($row['case_id']);$id=self::positiveId($row['legacy_installation_object_id']);$legacyId=self::positiveId($row['legacy_id']);$migrated=$legacyId===null;$dataOrigin=$migrated?'migration_active':(($row['migration_category']??null)==='native_candidate'?'migration_native':'demo_fixture');if($migrated){try{$payload=\json_decode((string)$row['baseline_payload'],true,flags:JSON_THROW_ON_ERROR);}catch(\Throwable){throw new PilotHttpInfrastructureUnavailable();}$legacyObject=$payload['legacyObject']??null;$classification=$payload['classification']??null;if(!\is_array($legacyObject)||!\is_array($classification)||$row['process_state']!=='working'||self::positiveId($row['baseline_id'])===null||self::positiveId($row['provenance_legacy_id'])!==$id||!\hash_equals((string)$row['provenance_baseline_hash'],(string)$row['baseline_hash'])||!\hash_equals((string)$row['baseline_hash'],\hash('sha256',(string)$row['baseline_payload']))||($classification['category']??null)!=='legacy_active'||($classification['quarantineCodes']??null)!==[])throw new PilotHttpInfrastructureUnavailable();$address=\trim((string)($legacyObject['ordadr_address']??''));$entrance=\trim((string)($legacyObject['entrance']??''));$registration=\trim((string)($legacyObject['regnumber']??''));$factory=\trim((string)($legacyObject['zavnumber']??''));$start=null;$finish=null;}else{$address=\trim((string)$row['ordadr_address']);$entrance=\trim((string)$row['entrance']);$registration=\trim((string)($row['regnumber']??''));$factory=\trim((string)($row['zavnumber']??''));$start=self::date($row['workdatestart']);$finish=self::date($row['workdateendadjusted'])??self::date($row['plan_finish_date']);}if($caseId===null||$id===null||(!$migrated&&$legacyId!==$id)||isset($caseIds[$caseId])||isset($objectIds[$id])||$address===''||$entrance==='')throw new PilotHttpInfrastructureUnavailable();$caseIds[$caseId]=true;$objectIds[$id]=true;$objects[]=['id'=>$id,'registrationNumber'=>$registration,'factoryNumber'=>$factory,'address'=>$address,'entrance'=>$entrance,'plannedStartDate'=>$start,'plannedFinishDate'=>$finish,'planningDatesUnknownAtCutover'=>$migrated,'dataOrigin'=>$dataOrigin];}
        $pages=\max(1,(int)\ceil($caseCount/self::PAGE_SIZE));if($page>$pages)throw new PilotHttpInfrastructureUnavailable();return['objects'=>$objects,'page'=>$page,'pages'=>$pages,'total'=>$caseCount,'pageSize'=>self::PAGE_SIZE,'origin'=>$origin];
    }
    private static function origin(string$value):bool{return \in_array($value,['all','migration','migration_native','migration_active','demo_fixture','local_audit'],true);}
    private static function date(mixed $value):?string{if(!\is_string($value)||\preg_match('/^(\d{4})-(\d{2})-(\d{2})(?:[ T].*)?$/D',$value,$m)!==1||!\checkdate((int)$m[2],(int)$m[3],(int)$m[1])||$m[1]==='0000')return null;return $m[1].'-'.$m[2].'-'.$m[3];}
    private static function positiveId(mixed $value):?int{if(!\is_int($value)&&(!\is_string($value)||\preg_match('/^[1-9][0-9]*$/D',$value)!==1))return null;$id=\filter_var($value,FILTER_VALIDATE_INT,['options'=>['min_range'=>1]]);return $id===false?null:$id;}
}
class PilotHttpCoordinator
{ private string $cspMethod='GET';private string $cspPath='';
    public function __construct(private TrustedServerIdentity $identity,private PilotShellRenderer $shell,private PilotHttpDependencies $dependencies,private ?ObjectCardRenderer $cards=null,private ?ObjectCardReaderProvider $cardReaders=null,private ?ObjectListRenderer $lists=null,private ?ObjectListReaderProvider $listReaders=null,private ?PrepareFormRenderer $prepareForms=null,private ?PrepareFormReaderProvider $prepareReaders=null,private ?ProductionChecklistRenderer $checklists=null,private ?LocalObjectListAuthorization $localObjects=null,private ?LocalObjectListAuthorization $localPrepare=null){}
    public function handle(PilotHttpRequest $r):PilotHttpResponse
    {
        $this->cspMethod=$r->method;$this->cspPath=$r->path;$cardId=self::cardId($r->path);$prepareId=self::prepareId($r->path);$checklistId=self::checklistId($r->path);
        ...
        if($r->path==='/pilot/objects'){if($this->localObjects===null)return $this->response(503,"Service unavailable.\n",['Retry-After'=>'60'],$r->method);$authorization=$this->localObjects->authorize($r->server['FMONITOR_AUTH_USER_ID']??null);if($authorization->status==='AUTHENTICATION_REQUIRED')return $this->response(401,"Authentication required.\n",[],$r->method);if($authorization->status==='ACCESS_DENIED')return $this->response(403,"Access denied.\n",[],$r->method);if($authorization->status==='AUTHORIZATION_UNAVAILABLE'){$this->localObjects->reportUnavailable((string)$authorization->category,(string)$authorization->correlationId);return $this->response(503,"Service unavailable.\n",['X-Correlation-ID'=>(string)$authorization->correlationId],$r->method);}try{$configured=$this->dependencies instanceof PilotUiConfiguration&&$this->dependencies->pilotUiConfigured();$this->dependencies->css()->readBytes();if($configured)$this->dependencies->pilotCss()->readBytes();$user=$this->localObjects->user((int)$authorization->actorUserId);if($user===null||$this->listReaders===null||$this->lists===null)throw new PilotHttpInfrastructureUnavailable();$objects=$this->listReaders->objectList()->read();$html=!$configured&&$this->lists instanceof CompatibilityObjectListRenderer?$this->lists->renderCompatibility($user,$objects):$this->lists->render($user,$objects);return $this->response(200,$html,['Content-Type'=>'text/html; charset=UTF-8'],$r->method);}catch(CssAssetUnavailable|PilotHttpInfrastructureUnavailable){return $this->response(503,"Service unavailable.\n",['Retry-After'=>'60'],$r->method);}}try{$principal=$this->identity->resolve($r->serverIdentity);}catch(InvalidServerIdentity){return $this->response(401,"Authentication required.\n",[],$r->method);}
        if($prepareId!==null){if($this->localPrepare===null)return $this->prepareUnavailable($r->method);$authorization=$this->localPrepare->authorize($r->server['FMONITOR_AUTH_USER_ID']??null);if($authorization->status==='AUTHENTICATION_REQUIRED')return $this->response(401,"Authentication required.\n",[],$r->method);if($authorization->status==='ACCESS_DENIED')return $this->response(403,"Access denied.\n",[],$r->method);if($authorization->status==='AUTHORIZATION_UNAVAILABLE'){$this->localPrepare->reportUnavailable((string)$authorization->category,(string)$authorization->correlationId);return $this->response(503,"Service unavailable.\n",['Retry-After'=>'60','X-Correlation-ID'=>(string)$authorization->correlationId],$r->method);}}
        $configured=$this->dependencies instanceof PilotUiConfiguration&&$this->dependencies->pilotUiConfigured();
        try{$this->dependencies->css()->readBytes();if($configured)$this->dependencies->pilotCss()->readBytes();$user=$this->dependencies->users()->resolveActiveUser($principal);if($user===null)return $this->response(403,"Access denied.\n",[],$r->method);if($checklistId!==null){if($this->cardReaders===null||$this->checklists===null)throw new PilotHttpInfrastructureUnavailable();$card=$this->cardReaders->objectCards()->read($checklistId);if($card===null)return $this->response(404,"Not found.\n",[],$r->method);$roleAccess=$this->dependencies instanceof ChecklistAccessProvider&&$this->dependencies->canEditChecklist($user->id);return $this->response(200,$this->checklists->render($user,$card,$roleAccess),['Content-Type'=>'text/html; charset=UTF-8'],$r->method);}if($prepareId!==null){if($this->prepareReaders===null||$this->prepareForms===null)throw new PilotHttpInfrastructureUnavailable();if(!$this->prepareReaders->hasProcessCapability($user->id,'assignment_order.prepare'))return $this->response(403,"Access denied.\n",[],$r->method);$form=$this->prepareReaders->prepareForms()->read($prepareId,$this->prepareReaders->processDate());if($form===null)return $this->response(404,"Not found.\n",[],$r->method);$html=!$configured&&$this->prepareForms instanceof CompatibilityPrepareFormRenderer?$this->prepareForms->renderCompatibility($user,$form):$this->prepareForms->render($user,$form);return $this->response(200,$html,['Content-Type'=>'text/html; charset=UTF-8'],$r->method);}if($r->path==='/pilot/objects'){if($this->listReaders===null||$this->lists===null)throw new PilotHttpInfrastructureUnavailable();$objects=$this->listReaders->objectList()->read();$html=!$configured&&$this->lists instanceof CompatibilityObjectListRenderer?$this->lists->renderCompatibility($user,$objects):$this->lists->render($user,$objects);return $this->response(200,$html,['Content-Type'=>'text/html; charset=UTF-8'],$r->method);}if($cardId!==null){if($this->cardReaders===null||$this->cards===null)throw new PilotHttpInfrastructureUnavailable();$card=$this->cardReaders->objectCards()->read($cardId);if($card===null)return $this->response(404,"Not found.\n",[],$r->method);$prepareConfigured=!($this->dependencies instanceof PilotUiConfiguration)||$this->dependencies->prepareCommandConfigured();$compatibility=$this->dependencies instanceof PilotUiConfiguration&&!$configured&&!$prepareConfigured;$hasPrepareCapability=false;if(!$compatibility){if($this->prepareReaders===null)throw new PilotHttpInfrastructureUnavailable();$hasPrepareCapability=$this->prepareReaders->hasProcessCapability($user->id,'assignment_order.prepare');}$card['canPrepare']=$prepareConfigured&&$hasPrepareCapability;$html=!$configured&&$this->cards instanceof CompatibilityObjectCardRenderer?$this->cards->renderCompatibility($user,$card):$this->cards->render($user,$card);return $this->response(200,$html,['Content-Type'=>'text/html; charset=UTF-8'],$r->method);}$html=!$configured&&$this->shell instanceof CompatibilityPilotShellRenderer?$this->shell->renderCompatibility($user):$this->shell->render($user);return $this->response(200,$html,['Content-Type'=>'text/html; charset=UTF-8'],$r->method);}catch(PrepareFormUnavailable){return $this->response(409,"Формирование распоряжения недоступно для текущего состояния объекта монтажа.\n",[],$r->method);}catch(CssAssetUnavailable|PilotHttpInfrastructureUnavailable){return $prepareId!==null?$this->prepareUnavailable($r->method):$this->response(503,"Service unavailable.\n",['Retry-After'=>'60'],$r->method);}
    }
    private function prepareUnavailable(string $method):PilotHttpResponse{return $this->response(503,"Service unavailable.\n",['Retry-After'=>'60','X-Correlation-ID'=>\bin2hex(\random_bytes(6))],$method);}
    ...
    private function response(int $status,string $getBody,array $headers=[],string $method='GET'):PilotHttpResponse
    {
        $headers += [
            ...
            'Content-Security-Policy'=>PilotRouteCsp::forResponse($this->cspMethod,$this->cspPath,$status,(string)($headers['Content-Type']??'text/plain; charset=UTF-8'),$getBody),
...
final class ProductionPilotHttpDependencies implements PilotHttpDependencies,ObjectCardReaderProvider,ObjectListReaderProvider,PrepareFormReaderProvider,ChecklistAccessProvider,PilotUiConfiguration,ShlzAssetProvider
{
    ...
    public function objectCards():ObjectCardReader
    {
        ...
        return $this->objectCardReader=new MariaDbAppliedObjectCardReader($this->connection,$this->legacyTablePrefix,$this->resolveProcessPrefix(),new MariaDbObjectCardReader($this->connection,$this->legacyTablePrefix,$this->resolveProcessPrefix()));
    }
    ...
    public function canEditChecklist(int $userId):bool
    {
        ...
        try{
            return (new MariaDbChecklistRoleAccess($this->connection,$this->processTablePrefix))->allowed($userId);
    ...
    public function processDate():string{$businessDate=$this->environment->read('FMONITOR_BUSINESS_DATE');if(\is_string($businessDate)){if(\preg_match('/^(\d{4})-(\d{2})-(\d{2})$/D',$businessDate,$parts)!==1||!\checkdate((int)$parts[2],(int)$parts[3],(int)$parts[1])||$parts[1]==='0000')throw new PilotHttpInfrastructureUnavailable();return $businessDate;}$now=$this->environment->read('FMONITOR_NOW');try{return new \DateTimeImmutable(\is_string($now)?$now:'now')->setTimezone(new \DateTimeZone('Europe/Moscow'))->format('Y-m-d');}catch(\Throwable $e){throw new PilotHttpInfrastructureUnavailable('',0,$e);}}
...
final class MariaDbHttpUserDirectory implements HttpUserDirectory
{
    ...
    {
        try{$local=(new MariaDbLocalUserProfile($this->connection,$this->processPrefix))->readPrincipal($principal);if($local!==null)return$local;$identity=(new \FMonitor2\InstallationProcess\MariaDbProcessUserDirectory($this->connection,$this->processPrefix,$this->prefix))->findActiveLegacyIdentity($principal);return $identity===null?null:new HttpUser($identity['id'],$identity['name'],$identity['email']);}catch(\Throwable $e){if($e instanceof PilotHttpInfrastructureUnavailable)throw $e;throw new PilotHttpInfrastructureUnavailable('',0,$e);}
    }
}
...
class PilotHttpGateway
{
    ...
    {
        $headers=['Content-Type'=>'text/plain; charset=UTF-8','X-Content-Type-Options'=>'nosniff','Referrer-Policy'=>'no-referrer','X-Frame-Options'=>'DENY','Content-Security-Policy'=>PilotRouteCsp::BASE,'Permissions-Policy'=>'camera=(), microphone=(), geolocation=()','Cross-Origin-Opener-Policy'=>'same-origin','Cache-Control'=>'no-store']+$extra;$headers['Content-Length']=(string)\strlen($body);return new PilotHttpResponse($status,$headers,$body);
    }
}
...
final class ProductionPilotHttpGatewayFactory
{
    public static function create(EnvironmentSource $environment):PilotHttpGateway
    {
        ...
        $application=new PilotHttpCoordinator(new RemoteUserIdentity(),new ProductionPilotShellRenderer(),$dependencies,new ProductionObjectCardRenderer(),$dependencies,new ProductionObjectListRenderer(),$dependencies,new ProductionPrepareFormRenderer(),$dependencies,new ProductionChecklistRenderer(),new ProductionLocalObjectListAuthorization($environment));
...
require_once __DIR__.'/InstallerDirectoryView.php';require_once __DIR__.'/ChecklistView.php';require_once __DIR__.'/PilotSessionView.php';
