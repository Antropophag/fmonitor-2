...
final class MariaDbHttpUserDirectory implements HttpUserDirectory
{
    ...
    public function resolveActiveUser(string $principal):?HttpUser
    {
        try{
            ...
            if($table->get_result()->fetch_assoc()!==null){$authenticatedId=filter_var($_SERVER['FMONITOR_AUTH_USER_ID']??null,FILTER_VALIDATE_INT,['options'=>['min_range'=>1]]);if($authenticatedId!==false){$s=$this->connection->prepare("SELECT user_id AS id,full_name AS name,email FROM `{$pilotTable}` WHERE user_id=? AND status=1 LIMIT 1");$s->bind_param('i',$authenticatedId);}else{$s=$this->connection->prepare("SELECT user_id AS id,full_name AS name,email FROM `{$pilotTable}` WHERE BINARY email=BINARY ? AND status=1 LIMIT 2");$s->bind_param('s',$principal);}$s->execute();$rows=$s->get_result()->fetch_all(MYSQLI_ASSOC);if($rows!==[])return $this->user($rows,$principal);}
