...
class PilotHttpCoordinator
{
    ...
    public function handle(PilotHttpRequest $r):PilotHttpResponse
    {
        ...
        if(!\in_array($r->path,['/pilot','/pilot/','/pilot/assets/pilot.css','/pilot/assets/picker.js','/pilot/assets/users.js','/pilot/assets/checklist.js','/pilot/objects'],true)&&!$assetCandidate&&$cardId===null&&$prepareId===null&&$checklistId===null)return $this->response(404,"Not found.\n");
    ...
    private function response(int $status,string $getBody,array $headers=[],string $method='GET'):PilotHttpResponse
    {
        $headers += [
            ...
            'Content-Security-Policy'=>(($headers['Content-Type']??'')==='text/html; charset=UTF-8'&&\str_contains($getBody,'data-checklist'))?"default-src 'none'; style-src 'self'; script-src 'self'; img-src 'self' blob:; font-src 'self'; base-uri 'none'; form-action 'self'; frame-ancestors 'none'":"default-src 'none'; style-src 'self'; script-src 'self'; img-src 'self'; font-src 'self'; base-uri 'none'; form-action 'self'; frame-ancestors 'none'",
