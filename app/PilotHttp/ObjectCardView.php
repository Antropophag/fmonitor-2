...
final class ProductionObjectCardRenderer implements ObjectCardRenderer,CompatibilityObjectCardRenderer
{
    ...
    public function renderCompatibility(HttpUser $user,array $card):string
    {
        [$id,$body]=$this->cardBody($card);
        return (new ProductionPilotShellRenderer())->renderObjectCardCompatibility($user,$id,$body);
    }

    /** @return array{string,string} */
    private function cardBody(array $c):array
    {
        ...
        $body.='<p><a class="shlz-link" href="/pilot/objects/'.$id.'/execution">Применение и открытие работ</a></p>';return [$id,$body];
    }
}
