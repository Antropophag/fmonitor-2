...
final class PilotUserAccessHttpHandler
{
    ...
    private function ownerUserAccess(PilotHttpRequest $request,string $id,array $initialState):PilotHttpResponse
    {
        ...
        if($request->path==='/pilot/admin/users/invite'&&$request->method==='POST'&&\in_array($response->status,[201,400],true)){$state['fm2_invitation_flash']=$response->status===201&&\preg_match('#^Invitation: (/pilot/activate\?token=[A-Za-z0-9_-]{43})\n$#D',$response->body,$match)===1?['kind'=>'success','url'=>(($this->environment?->read('FMONITOR_TRUSTED_REQUEST_SCHEME')==='https'?'https':'http').'://'.$request->host.$match[1])]:['kind'=>'error'];$response=PilotUserAccessResponse::create($request,303,'',['Location'=>'/pilot/admin/users']);}
        if($response->status===200){$flash=$state['fm2_invitation_flash']??null;$body=$response->body;if(\is_array($flash)&&($flash['kind']??null)==='success'&&\is_string($flash['url']??null)){$body=PilotSessionView::withInvitationFeedback($body,$flash['url']);unset($state['fm2_invitation_flash']);}$response=PilotUserAccessResponse::create($request,200,$body,['Content-Type'=>'text/html; charset=UTF-8']);}
    ...
    private function ownerPath(string $path):bool{return \in_array($path,['/pilot/admin/users','/pilot/admin/users/invite'],true)||\preg_match('#^/pilot/admin/users/[1-9][0-9]*/roles(?:/[1-9][0-9]*)?$#D',$path)===1;}
