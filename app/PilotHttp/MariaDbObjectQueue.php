...
final readonly class MariaDbObjectQueue
{
    ...
    public function read(string $query, string $status, int $page, int $size = 50): array
    {
        ...
        $statusSql = match ($status) {
            'needs_assignment_order' => "(c.process_state='needs_assignment_order' AND o.id IS NULL AND a.application_id IS NULL) OR (c.process_state='assignment_order_prepared' AND o.status='prepared' AND a.application_id IS NULL)",
            'ready_to_open' => "c.process_state='assignment_order_prepared' AND (a.application_id IS NOT NULL OR o.status='registered')",
        ...
        $offset = ($page - 1) * $size;
        $rows = $this->db->query("SELECT c.legacy_installation_object_id,c.process_state,c.actual_start_date,c.opened_at,c.opened_by_user_id,l.ordadr_address,l.entrance,l.regnumber,l.workdatestart,l.workdateendadjusted,l.plan_finish_date,o.status order_status,a.application_id{$from} ORDER BY l.workdatestart IS NULL,LEFT(l.workdatestart,10),c.legacy_installation_object_id LIMIT {$size} OFFSET {$offset}")->fetch_all(MYSQLI_ASSOC);
        $objects = [];
        foreach ($rows as $row) {
            ...
            $label = match ((string) $row['process_state']) {
                'needs_assignment_order' => $empty && $order === null && !$applied ? 'Требуется распоряжение' : null,
                'assignment_order_prepared' => $empty && $order === 'prepared' && !$applied ? 'Распоряжение подготовлено' : ($empty && ($applied || $order === 'registered') ? 'Готов к открытию' : null),
