...
final class PilotE2ECoordinator extends PilotHttpCoordinator
{
    ...
    {
        $objects=$this->dependencies->objectList()->read();$control=[];foreach($objects as $object){$card=$this->dependencies->objectCards()->read($object['id']);if(!(bool)$card['opened'])continue;$object['controlEngineer']=$card['controlEngineer'];$object['completed']=(bool)$card['hasPtoAct'];$object['lastChecklistActivityAt']=null;$control[]=$object;}$objects=$control;
        try{[$db,$prefix]=$this->dependencies->commandResources();$table=$prefix.'fm2_checklist_operations';$q=$db->prepare('SELECT TABLE_NAME FROM information_schema.TABLES WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME=? LIMIT 1');$q->bind_param('s',$table);$q->execute();if($q->get_result()->fetch_assoc()!==null){$result=$db->query("SELECT c.legacy_installation_object_id object_id,MAX(o.device_time) last_activity_at FROM `{$prefix}fm2_checklist_operations` o JOIN `{$prefix}fm2_installation_cases` c ON c.id=o.installation_case_id GROUP BY c.legacy_installation_object_id");$activity=[];foreach($result->fetch_all(MYSQLI_ASSOC)as$row)$activity[(int)$row['object_id']]=(string)$row['last_activity_at'];foreach($objects as&$object)$object['lastChecklistActivityAt']=$activity[$object['id']]??null;unset($object);}}catch(\Throwable){}
        \usort($objects,static function(array $a,array $b):int{$aTime=$a['lastChecklistActivityAt']??null;$bTime=$b['lastChecklistActivityAt']??null;if($aTime===null&&$bTime!==null)return-1;if($aTime!==null&&$bTime===null)return 1;return[$aTime??'',$a['id']]<=>[$bTime??'',$b['id']];});
