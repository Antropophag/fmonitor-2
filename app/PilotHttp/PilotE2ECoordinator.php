...
final class PilotE2ECoordinator extends PilotHttpCoordinator
{
    ...
    public function handle(PilotHttpRequest $r):PilotHttpResponse
    {
        ...
        try{
            ...
            if($r->method==='POST')return $this->command($r,$route,$user);
    ...
    public function card(PilotHttpRequest $r,int $id,HttpUser $user):PilotHttpResponse
    {
        ...
        $card['canPrepare']=$capPrepare;$card['canRegister']=$capRegister;$card['canOpen']=$capOpen;$card['flash']=$session===null?null:$this->pullFlash($session,$r->path);$card['openedByName']=$card['opened']?$this->actorName((int)$card['openedByUserId']):null;
    ...
    {
        $objects=$this->dependencies->objectList()->read();foreach($objects as &$object){$card=$this->dependencies->objectCards()->read($object['id']);$object['status']=$card['status'];$object['nextStep']=match($card['status']){'Требуется распоряжение'=>'Сформировать распоряжение','Распоряжение подготовлено'=>'Внести номер 1С ДО','Готов к открытию'=>'Открыть работы','В работе'=>'Инженеру: провести первую инспекцию',default=>'Открыть карточку объекта монтажа'};}unset($object);
    ...
    private function preparePage(PilotHttpRequest $r,int $id,HttpUser $user):PilotHttpResponse
    {
        ...
        if($form===null)return $this->response(404,"Not found.\n",[],$r->method);[$session,$headers]=$this->session($r,$user,true);$form['csrfToken']=$this->token($session,$user,$id);$form['processRevision']=$this->revision($session,$id);$form['flash']=$this->pullFlash($session,$r->path);return $this->response(200,$this->forms->render($user,$form),['Content-Type'=>'text/html; charset=UTF-8']+$headers,$r->method);
    }
    ...
    private function command(PilotHttpRequest $r,array $route,HttpUser $user):PilotHttpResponse
    {
        ...
        $fields=$this->body($r,$allowed);if($fields===null)return $this->response(400,"Bad request.\n");
        ...
        if($route['kind']==='prepare'){
            $path=$r->path;$error=null;$installers=$fields['installerTabIds[]']??[];$engineers=$fields['controlEngineerUserId']??[];$confirm=$fields['controlEngineerConfirmed']??[];
            ...
            if($error===null){$eligible=$this->dependencies->prepareForms()->read($route['id'],$this->dependencies->businessDate());$installerIds=\array_map(static fn(array $x):string=>(string)$x['tabId'],$eligible['installers']);$engineerIds=\array_map(static fn(array $x):string=>(string)$x['userId'],$eligible['engineers']);foreach($installers as$value)if(!\in_array($value,$installerIds,true)){$error=['installers','Состав монтажников изменился. Проверьте доступных сотрудников.'];break;}if($error===null&&!\in_array($engineers[0],$engineerIds,true))$error=['engineer','Выбранный инженер больше недоступен. Выберите другого.'];}
            if($error!==null)return $this->flashRedirect($session,$path,$error[1],$error[0]);
            $result=$process->prepareAssignmentOrder($route['id'],\array_map('intval',$installers),(int)$engineers[0],$user->id);if(($result['accepted']??false)===true)return $this->flashRedirect($session,'/pilot/objects/'.$route['id'],'Распоряжение подготовлено.');return $this->violation($session,$route,$result,$path);
        }
        $version=$fields['assignmentOrderVersion']??[];if(\count($version)!==1||!self::positive($version[0])||$card['order']===null||(int)$version[0]!==$card['order']['version'])return $this->flashRedirect($session,'/pilot/objects/'.$route['id'],'Данные объекта монтажа изменились. Проверьте актуальное состояние и повторите действие.');
        if($route['kind']==='registration'){$number=$fields['registrationNumber'][0]??'';$number=\trim($number);if($number===''||\mb_strlen($number)>120||\preg_match('/[\x00-\x1F\x7F]/u',$number)===1)return $this->flashRedirect($session,'/pilot/objects/'.$route['id'],'Введите номер распоряжения в 1С ДО.','registrationNumber');$result=$process->confirmOrderRegistration($route['id'],(int)$version[0],$number,'manual',$user->id);if(($result['accepted']??false)===true)return $this->flashRedirect($session,'/pilot/objects/'.$route['id'],'Номер 1С ДО сохранён. Распоряжение зарегистрировано.');return $this->violation($session,$route,$result,'/pilot/objects/'.$route['id']);}
        $date=$fields['actualStartDate'][0]??'';$today=(new \DateTimeImmutable($now))->setTimezone(new \DateTimeZone('Europe/Moscow'))->format('Y-m-d');if(!self::date($date)||$date>$today||$date<$card['order']['orderDate'])return $this->flashRedirect($session,'/pilot/objects/'.$route['id'],'Укажите фактическую дату от даты распоряжения до сегодняшнего дня.','actualStartDate');$result=$process->openInstallation($route['id'],$date,$user->id);if(($result['accepted']??false)===true)return $this->flashRedirect($session,'/pilot/objects/'.$route['id'],'Работы открыты.');return $this->violation($session,$route,$result,'/pilot/objects/'.$route['id']);
    }
    ...
    {
        $code=$result['violations'][0]['code']??'';$map=['INSTALLER_NOT_IN_CATALOG'=>['Состав монтажников изменился. Проверьте доступных сотрудников.','installers'],'INSTALLER_NOT_EMPLOYED'=>['Состав монтажников изменился. Проверьте доступных сотрудников.','installers'],'CONTROL_ENGINEER_NOT_ELIGIBLE'=>['Выбранный инженер больше недоступен. Выберите другого.','engineer'],'CONTROL_ENGINEER_REQUIRED'=>['Выберите одного инженера строительного контроля.','engineer'],'INSTALLER_REQUIRED'=>['Выберите хотя бы одного монтажника.','installers'],'REGISTRATION_NUMBER_REQUIRED'=>['Введите номер распоряжения в 1С ДО.','registrationNumber'],'ACTUAL_START_BEFORE_ORDER_DATE'=>['Укажите фактическую дату от даты распоряжения до сегодняшнего дня.','actualStartDate']];if(isset($map[$code]))return $this->flashRedirect($session,$path,$map[$code][0],$map[$code][1]);if($code==='FORBIDDEN')return $this->response(403,"Access denied.\n");if(\in_array($code,['ORDER_HAS_PTO_ACT','ASSIGNMENT_ORDER_ALREADY_PREPARED'],true))return $this->flashRedirect($session,'/pilot/objects/'.$route['id'],'Формирование распоряжения недоступно для текущего состояния объекта монтажа.');if(\str_contains($code,'PERSISTENCE')||\str_contains($code,'RENDER_FAILED'))return $this->response(503,"Service unavailable.\n",['Retry-After'=>'60']);return $this->flashRedirect($session,'/pilot/objects/'.$route['id'],'Данные объекта монтажа изменились. Проверьте актуальное состояние и повторите действие.');
    }

    private function route(string $path):?array
    {
        foreach([['prepare','#^/pilot/objects/([1-9][0-9]*)/assignment-order/prepare$#D'],['artifact','#^/pilot/objects/([1-9][0-9]*)/assignment-orders/([1-9][0-9]*)/artifacts/(order|appendix)$#D'],['registration','#^/pilot/objects/([1-9][0-9]*)/assignment-orders/([1-9][0-9]*)/registration$#D'],['open','#^/pilot/objects/([1-9][0-9]*)/open$#D']]as[$kind,$pattern])if(\preg_match($pattern,$path,$m)===1){foreach(\array_slice($m,1)as$x)if(\ctype_digit($x)&&!self::positive($x))return null;return ['kind'=>$kind,'id'=>(int)$m[1]]+($kind==='artifact'?['version'=>(int)$m[2],'type'=>$m[3]]:[]);}return null;
    }
    ...
    private function revision(array $s,int $id):string{[$db]=$this->dependencies->commandResources();$q=$db->prepare('SELECT lock_version FROM fm2_installation_cases WHERE legacy_installation_object_id=? LIMIT 2');$q->bind_param('i',$id);$q->execute();$rows=$q->get_result()->fetch_all(MYSQLI_ASSOC);if(\count($rows)!==1)throw new PilotHttpInfrastructureUnavailable();return \hash_hmac('sha256',$id.':'.$rows[0]['lock_version'],$s['secret']);}
    private function validRequest(PilotHttpRequest $r,array $s,HttpUser $u):bool{$origin=$r->server['HTTP_ORIGIN']??null;$fetch=$r->server['HTTP_SEC_FETCH_SITE']??null;return $s['actor']===$u->id&&($origin===null||$origin==='https://'.$r->host)&&($fetch===null||$fetch==='same-origin');}
    private function body(PilotHttpRequest $r,array $allowed):?array{$type=(string)($r->server['CONTENT_TYPE']??'');$length=$r->server['CONTENT_LENGTH']??null;if(!\preg_match('#^application/x-www-form-urlencoded(?:;\s*charset=UTF-8)?$#iD',$type)||!\is_string($length)||!\ctype_digit($length)||(int)$length>16384||(int)$length!==\strlen($r->body))return null;$out=[];$nextInstaller=0;foreach(\explode('&',$r->body)as$part){if($part==='')continue;$pair=\explode('=',$part,2);$key=\rawurldecode(\str_replace('+',' ',$pair[0]));$value=\rawurldecode(\str_replace('+',' ',$pair[1]??''));if(\preg_match('/^installerTabIds\[([0-9]+)\]$/D',$key,$m)===1){if((int)$m[1]!==$nextInstaller++)return null;$key='installerTabIds[]';}if(!\in_array($key,$allowed,true)||!\mb_check_encoding($key,'UTF-8')||!\mb_check_encoding($value,'UTF-8'))return null;$out[$key][]=$value;}foreach($out as$key=>$values)if($key!=='installerTabIds[]'&&\count($values)!==1)return null;return $out;}
    private function flashRedirect(array &$s,string $path,string $message,?string $field=null):PilotHttpResponse{$s['flash'][$path]=['message'=>$message,'field'=>$field];$_SESSION=$s;return $this->redirect($path);}
