...
final class PilotE2ECoordinator extends PilotHttpCoordinator
{
    ...
    private function command(PilotHttpRequest $r,array $route,HttpUser $user):PilotHttpResponse
    {
        ...
        if($route['kind']==='prepare'){
            ...
            $result=$process->prepareAssignmentOrder($route['id'],\array_map('intval',$installers),(int)$engineers[0],$user->id);if(($result['accepted']??false)===true){if(($result['status']??null)!=='prepared'||!self::positive((string)($result['assignmentOrderVersion']??'')))return $this->response(503,"Service unavailable.\n",['Retry-After'=>'60']);return $this->flashRedirect($session,'/pilot/objects/'.$route['id'],'Распоряжение подготовлено.');}return $this->violation($session,$route,$result,$path);
        }
    ...
    private function artifactOutage(\mysqli $db,string $prefix,string $root,array $route):bool{try{$sql="SELECT a.sha256 FROM `{$prefix}fm2_order_artifacts` a JOIN `{$prefix}fm2_assignment_orders` o ON o.id=a.assignment_order_id JOIN `{$prefix}fm2_installation_cases` c ON c.id=o.installation_case_id WHERE c.legacy_installation_object_id=? AND o.version_no=? AND a.artifact_type=? LIMIT 2";$q=$db->prepare($sql);$q->bind_param('iis',$route['id'],$route['version'],$route['type']);$q->execute();$rows=$q->get_result()->fetch_all(MYSQLI_ASSOC);if(\count($rows)!==1)return false;$hash=(string)$rows[0]['sha256'];$path=$root.'/sha256/'.\substr($hash,0,2).'/'.\substr($hash,2,2).'/'.$hash;return !\is_readable($root)||(\file_exists($path)&&!\is_readable($path))||(!\file_exists($path)&&!\is_readable(\dirname($path)));}catch(\Throwable){return true;}}
