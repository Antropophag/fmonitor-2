...
final class PilotE2ECoordinator extends PilotHttpCoordinator
{
    ...
    public function handle(PilotHttpRequest $r):PilotHttpResponse
    {
        ...
        try{
            ...
            if($r->method==='POST'){try{return $this->command($r,$route,$user);}catch(\LogicException $e){[$session]=$this->session($r,$user,false);if($session===null)return $this->response(503,"Service unavailable.\n",['Retry-After'=>'60']);if($route['kind']==='open'&&$e->getMessage()==='A current installer is not employed on the actual start date.')return $this->flashRedirect($session,'/pilot/objects/'.$route['id'],'Состав монтажников изменился. Открытие работ недоступно.',null,true);return $this->flashRedirect($session,'/pilot/objects/'.$route['id'],'Данные объекта монтажа изменились. Проверьте актуальное состояние и повторите действие.');}}
