...
final class MariaDbOtizSettlement
{
    ...
    private function execute(int$actor,string$operationId,array$fingerprint,\Closure$work):array
    {
        ...
        $hash=hash('sha256',json_encode($fingerprint,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES|JSON_THROW_ON_ERROR));$transaction=$this->db->beginTransaction();
        try{
        ...
        }catch(IntegrityException){$transaction->rollBack();$saved=$this->receipt($actor,$operationId);if($saved!==false&&hash_equals((string)$saved['request_sha256'],$hash))return json_decode((string)$saved['result_json'],true,flags:JSON_THROW_ON_ERROR);$this->fail('OPERATION_CONFLICT');}
        catch(\Throwable$e){if($transaction->isActive)$transaction->rollBack();throw$e;}
    }

    private function authorize(int$actor):void{$ok=$this->db->createCommand("SELECT 1 FROM `{$this->prefix}fm2_pilot_users` u JOIN `{$this->prefix}fm2_pilot_user_roles` ur ON ur.user_id=u.user_id JOIN `{$this->prefix}fm2_pilot_roles` r ON r.role_id=ur.role_id JOIN `{$this->prefix}fm2_pilot_role_permissions` rp ON rp.role_id=r.role_id WHERE u.user_id=:a AND u.status=1 AND u.activation_state='active' AND r.status=1 AND rp.permission='otiz.manage' LIMIT 1",[':a'=>$actor])->queryScalar();if($ok===false)$this->fail('FORBIDDEN');}
    private function receipt(int$a,string$o):array|false{return$this->db->createCommand("SELECT request_sha256,result_json FROM `{$this->prefix}fm2_otiz_settlement_operations` WHERE actor_user_id=:a AND operation_id=:o FOR UPDATE",[':a'=>$a,':o'=>$o])->queryOne();}
