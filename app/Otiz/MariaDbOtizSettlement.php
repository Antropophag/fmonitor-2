...
final class MariaDbOtizSettlement
{
    ...
    private function snapshot(int$id):array{$r=$this->db->createCommand("SELECT * FROM `{$this->prefix}fm2_pilot_otiz_snapshots` WHERE id=:id FOR UPDATE",[':id'=>$id])->queryOne();if($r===false)$this->fail('NOT_FOUND');if($r['status']!=='accepted')$this->fail('SNAPSHOT_NOT_ACCEPTED');if($r['rules_version']!==PremiumCalculationV2::VERSION)$this->fail('STALE_CALCULATION');return$r;}
